...
def ChildShortDisconnect(state):
    ...
    state.wait_connected("child", "parent")
    time.sleep(5)
    AgentTest.sh("docker network disconnect gaps_hi_default gaps_hi_agent_child_1", state.output)
    time.sleep(3)
    AgentTest.sh("docker network connect --alias agent_child gaps_hi_default gaps_hi_agent_child_1", state.output)
    time.sleep(5)
    # pylint: disable-msg=W0622
...
def ChildShortRestart(state):
    ...
    state.wait_connected("child", "parent")
    time.sleep(5)
    AgentTest.sh("docker kill -s INT gaps_hi_agent_child_1", state.output)
    time.sleep(3)
    AgentTest.sh("docker start gaps_hi_agent_child_1", state.output)
    state.wait_isparent("parent")
    time.sleep(10)
    # pylint: disable-msg=W0622
    state.end_checks.append( lambda: state.check_sync("child","parent") )
...
def ParentShortDisconnect(state):
    ...
    state.wait_connected("child", "parent")
    time.sleep(5)
    AgentTest.sh("docker network disconnect gaps_hi_default gaps_hi_agent_parent_1", state.output)
    time.sleep(3)
    AgentTest.sh("docker network connect --alias agent_parent gaps_hi_default gaps_hi_agent_parent_1", state.output)
    state.wait_isparent("parent")
    time.sleep(5)
    # pylint: disable-msg=W0622
    state.end_checks.append( lambda: state.check_sync("child","parent") )
...
def ParentShortRestart(state):
    ...
    state.wait_connected("child", "parent")
    time.sleep(5)
    AgentTest.sh("docker kill gaps_hi_agent_parent_1", state.output)
    time.sleep(3)
    AgentTest.sh("docker start gaps_hi_agent_parent_1", state.output)
    state.wait_isparent("parent")
    time.sleep(10)
    # pylint: disable-msg=W0622
    state.end_checks.append( lambda: state.check_sync("child","parent") )
...
def ParentLongRestart(state):
    ...
    state.wait_connected("child", "parent")
    time.sleep(5)
    AgentTest.sh("docker kill -s INT gaps_hi_agent_parent_1", state.output)
    time.sleep(30)
    AgentTest.sh("docker start gaps_hi_agent_parent_1", state.output)
    state.wait_isparent("parent")
    time.sleep(30)
