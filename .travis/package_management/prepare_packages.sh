...
for d in ${CREATED_CONTAINERS[@]}; do
	...
	if [ -d "${RPM_BUILD_PATH}" ]; then
		...
		if [ -d "${DEB_BUILD_PATH}" ]; then
			cp "${DEB_BUILD_PATH}/netdata-dbgsym*.ddeb" "${PACKAGES_DIRECTORY}" || echo "Could not copy any .ddeb files"
			cp "${DEB_BUILD_PATH}/netdata_*.deb" "${PACKAGES_DIRECTORY}" || echo "Could not copy any .deb files"
			cp "${DEB_BUILD_PATH}/netdata_*.buildinfo" "${PACKAGES_DIRECTORY}" || echo "Could not copy any .buildinfo files"
			cp "${DEB_BUILD_PATH}/netdata_*.changes" "${PACKAGES_DIRECTORY}" || echo "Could not copy any .changes files"
		else
