...
CHART crsproxy.requests '' "CRS Proxy Requests Rate" "requests/s" crsproxy '' area 20001 $crsproxy_update_every
DIMENSION web '' incremental 1 $((1 * crsproxy_update_every))
DIMENSION native '' incremental -1 $((1 * crsproxy_update_every))
CHART crsproxy.clients '' "CRS Proxy Clients Rate" "clients/s" crsproxy '' area 20010 $crsproxy_update_every
DIMENSION web '' incremental 1 $((1 * crsproxy_update_every))
DIMENSION native '' incremental -1 $((1 * crsproxy_update_every))
DIMENSION virtual '' incremental 1 $((1 * crsproxy_update_every))
CHART crsproxy.replies '' "CRS Replies Rate" "replies/s" crsproxy '' area 20020 $crsproxy_update_every
DIMENSION ok '' incremental 1 $((1 * crsproxy_update_every))
DIMENSION failed '' incremental -1 $((1 * crsproxy_update_every))
CHART crsproxy.bconnections '' "Back-End Connections Rate" "connections/s" crsproxy '' area 20030 $crsproxy_update_every
DIMENSION ok '' incremental 1 $((1 * crsproxy_update_every))
DIMENSION failed '' incremental -1 $((1 * crsproxy_update_every))
EOF

	local x=
	echo "CHART crsproxy.commands '' 'CRS Commands Requests' 'requests/s' crsproxy '' stacked 20100 $crsproxy_update_every"
	for x in $crsproxy_cmds
	do
		echo "DIMENSION $x '' incremental 1 $((1 * crsproxy_update_every))"
	done

	echo "CHART crsproxy.commands_failed '' 'CRS Failed Commands' 'replies/s' crsproxy '' stacked 20110 $crsproxy_update_every"
	for x in $crsproxy_cmds
	do
		echo "DIMENSION $x '' incremental 1 $((1 * crsproxy_update_every))"
	done
