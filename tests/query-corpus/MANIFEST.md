...
| CASE-022/time-group-latest | time_group=latest: per-bucket last value, empty buckets stay empty; points=1+before=0 anchors at the newest sample, served from the collector cache (0 db reads, raw un-quantized value, ar=0); storage path keeps quantization + generic ar. Stock agents silently average the unknown name | **red** | n/a | |
