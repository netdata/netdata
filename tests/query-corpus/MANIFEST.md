...
| CASE-019/v1-json-name-escaping | v1 json/jsonp/csvjsonarray/datatable emit dimension names unescaped — a double-quote in a name (or label value via group_by=label) breaks the JSON; ditto the objectrows row keys and the apostrophe in google-flavored single-quoted labels; json2 escapes properly | **red** | n/a | |
