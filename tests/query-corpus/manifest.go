...
var manifest = map[string]ManifestCase{
	...
	"L6/two-pass-matrix": {
		Proves: "two-pass oracle over 10 key-chains (including cross-key union partitioning) x 7 aggregation chains with no average boundary x non-raw/raw: pass-2 values, PARTIAL propagation and group_by_label[1]; non-raw anomaly metadata is weighted by the raw metric contributors beneath pass-1 groups, while raw keeps the anomaly numerator and the old Agent-Cloud prior-group count because the same field can be a Cloud-rewritten average divisor; the result has no hidden schema field",
	},
