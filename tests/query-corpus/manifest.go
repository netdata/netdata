...
var manifest = map[string]ManifestCase{
	...
	"L6/two-pass-average-boundary": {
		Proves: "sum→average needs separate denominators across two passes: non-raw value divides by contributing pass-1 groups, while anomaly rate remains weighted by the raw metric contributors beneath those groups; raw mode deliberately leaves both the value and anomaly-rate numerators undivided and reports the raw contributor count. This contract is separated from the no-average matrix so contributor propagation cannot silently change the average value divisor",
	},
	...
	"CASE-026/totals-survive-a-plan-switch": {
		Proves: "a sum total does not change because the answer had to be assembled from two tiers. sum carries across rows the seconds a record still owes, and a plan switch is the one moment the record stream itself jumps - the engine reads ahead into the next tier and may keep or discard either side. A carry dropped or double-paid there moves the total by up to one stored record, a whole minute of data above tier 0. Asserted over a discovered, rotated tier0 head at four zooms from 1s to 300s buckets; two exact two-row seams cover a first-row join and a carried coarse record. A sparse control adds 120-second bursts separated by 1,920-second true DBENGINE page holes: forced tier0 and tier1 queries prove the exact fine burst and its overlapping coarse rollup, while two automatic 10-second grid phases place the fine page boundary on both adjacent row grids, leave every hole exactly empty, conserve every retained sample and report exact fine-tier anomaly membership once fine retention begins. Partial higher-tier anomaly metadata is not claimed because a stored rollup cannot locate its constituent timestamps",
	},
