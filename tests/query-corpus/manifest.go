...
var manifest = map[string]ManifestCase{
	...
	"CASE-026/anomaly-rate-covers-the-paid-seconds": {
		Proves: "a row's anomaly rate describes the seconds the row was paid, not only the seconds it was handed. sum is the one grouping that pays a row from a record delivered to a DIFFERENT row - a record wider than the row owes its opening seconds backwards - so a row can hold a value from a record it never received. Taking the anomaly rate from the received records alone makes such a row report a value out of nowhere and call it perfectly healthy. Asserted over a fixture where every stored second is anomalous, so every row holding a value must read 100 with nothing to weigh",
	},
	"CASE-026/totals-survive-a-plan-switch": {
		Proves: "a sum total does not change because the answer had to be assembled from two tiers. sum carries across rows the seconds a record still owes, and a plan switch is the one moment the record stream itself jumps - the engine reads ahead into the next tier and may keep or discard either side. A carry dropped or double-paid there moves the total by up to one stored record, a whole minute of data above tier 0. Asserted over a window straddling a rotated tier0 head, at four zooms from 1s to 300s buckets, on the layer-4c daemon where the boundary is discovered rather than predicted",
	},
