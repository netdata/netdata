...
program
    ...
    try {
        ...
        if (!Number.isFinite(toolTimeout) || toolTimeout <= 0) {
            console.error('Error: --tool-timeout must be positive');
            process.exit(4);
        }
        ...
        if (!Number.isFinite(topP) || topP < 0 || topP > 1) {
            console.error('Error: --top-p must be between 0 and 1');
            process.exit(4);
        }
        if (!Number.isFinite(maxToolTurns) || maxToolTurns <= 0) {
        ...
        function detectOS() {
            try {
                ...
                if (match && match[1])
                    return `${match[1]} (kernel ${os.release()})`;
            }
            catch { /* ignore */ }
            return `${os.type()} ${os.release()}`;
        }
        function expandPrompt(str, vars) {
            const replace = (s, re) => s.replace(re, (_m, name) => (name in vars ? vars[name] : _m));
            let out = str;
        ...
        const threshold = options.quiet === true ? 'error' : options.verbose === true ? 'debug' : 'info';
        ...
        const callbacks = {
            ...
            onAccounting: (entry) => {
                if (typeof accountingFile !== 'string' || accountingFile.length === 0)
                    return;
                try {
        ...
        if (!result.success) {
            ...
            if ((result.error ?? '').includes('Configuration'))
                process.exit(1);
            else if ((result.error ?? '').includes('Max tool turns exceeded'))
                process.exit(5);
            else if ((result.error ?? '').includes('tool'))
                process.exit(3);
            else
                process.exit(2);
        }
