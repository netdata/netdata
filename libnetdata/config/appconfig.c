...
void appconfig_generate(struct config *root, BUFFER *wb, int only_changed)
{
    ...
    for(i = 0; i <= 15 ;i++) {
        ...
        for(co = root->first_section; co ; co = co->next) {
            ...
            else if(!strcmp(co->name, CONFIG_SECTION_HOST_LABEL))        pri = 3;
            else if(!strcmp(co->name, CONFIG_SECTION_SQLITE))            pri = 4;
            else if(!strcmp(co->name, CONFIG_SECTION_CLOUD))             pri = 5;
            else if(!strcmp(co->name, CONFIG_SECTION_ML))                pri = 6;
            else if(!strcmp(co->name, CONFIG_SECTION_HEALTH))            pri = 7;
            else if(!strcmp(co->name, CONFIG_SECTION_STREAM))            pri = 8;
