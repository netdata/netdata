...
static inline void aral_lock(ARAL *ar) {
    if(likely(!ar->config.lockless))
        netdata_spinlock_lock(&ar->unsafe.spinlock);
}

static inline void aral_unlock(ARAL *ar) {
    if(likely(!ar->config.lockless))
        netdata_spinlock_unlock(&ar->unsafe.spinlock);
}
...
static void aral_add_page(ARAL *ar TRACE_ALLOCATIONS_FUNCTION_DEFINITION_PARAMS) {
    ARAL_PAGE *page = callocz(1, sizeof(ARAL_PAGE));
    page->size = ar->unsafe.allocation_size;
    ...
    if(unlikely(ar->config.mmap.enabled)) {
        ar->unsafe.file_number++;
        char filename[FILENAME_MAX + 1];
        snprintfz(filename, FILENAME_MAX, "%s/array_alloc.mmap/%s.%zu", *ar->config.mmap.cache_dir, ar->config.mmap.filename, ar->unsafe.file_number);
...
static inline ARAL_PAGE *aral_get_a_page_with_free_item(ARAL *ar TRACE_ALLOCATIONS_FUNCTION_DEFINITION_PARAMS) {
    ARAL_PAGE *page = ar->unsafe.pages;

    if(unlikely(!page || !page->free_list)) {
        usec_t check_ut = __atomic_load_n(&aral_globals.atomic.check_free_space_ut, __ATOMIC_RELAXED);
        if(check_ut > ar->atomic.last_full_check_time_ut) {
            page = find_page_with_free_slots_internal_check(ar);
            if (page) {
                internal_error(true, "ARAL: '%s' first page does not have any free slots, "
                ...
                internal_error(true, "ARAL: '%s' page check done - all pages full",
                               ar->config.name);
                aral_add_page(ar TRACE_ALLOCATIONS_FUNCTION_CALL_PARAMS);
            }
        }
        else
            aral_add_page(ar TRACE_ALLOCATIONS_FUNCTION_CALL_PARAMS);

        page = ar->unsafe.pages;
    }

    internal_fatal(!page->free_list,
                   "ARAL: '%s' free item to use, cannot be NULL.", ar->config.name);

    internal_fatal(page->free_list->size < ar->config.element_size,
...
void *aral_mallocz_internal(ARAL *ar TRACE_ALLOCATIONS_FUNCTION_DEFINITION_PARAMS) {
    ...
    ARAL_PAGE *page = aral_get_a_page_with_free_item(ar TRACE_ALLOCATIONS_FUNCTION_CALL_PARAMS);
...
static inline ARAL_PAGE *aral_ptr_to_page(ARAL *ar, void *ptr) {
...
void aral_freez_internal(ARAL *ar, void *ptr TRACE_ALLOCATIONS_FUNCTION_DEFINITION_PARAMS) {
    ...
    ARAL_PAGE *page = aral_ptr_to_page(ar, ptr);
...
void aral_destroy_internal(ARAL *ar TRACE_ALLOCATIONS_FUNCTION_DEFINITION_PARAMS) {
    ...
    while(ar->unsafe.pages)
        aral_del_page(ar, ar->unsafe.pages TRACE_ALLOCATIONS_FUNCTION_CALL_PARAMS);
...
ARAL *aral_create(const char *name, size_t element_size, size_t initial_page_elements, size_t max_page_elements, const char *filename, char **cache_dir, bool mmap, bool lockless) {
    ...
    ar->unsafe.allocation_size = aral_align_alloc_size(ar, (uint64_t)ar->config.element_size * ar->config.initial_page_elements);
    ar->unsafe.pages = NULL;
    ar->unsafe.file_number = 0;
    ...
    internal_error(true,
                   ...
                   , ar->unsafe.allocation_size / ar->config.element_size, ar->config.initial_page_elements
