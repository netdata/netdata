...
typedef enum __attribute__((packed)) {
    ...
    HTTP_ACL_BEARER_OPTIONAL    = (1 << 12), // allow unprotected access if bearer is not enabled in netdata
    HTTP_ACL_BEARER_REQUIRED    = (1 << 13), // allow access only if a valid bearer is used
} HTTP_ACL;

#define HTTP_ACL_DASHBOARD_ACLK_WEBRTC (HTTP_ACL_DASHBOARD | HTTP_ACL_ACLK | HTTP_ACL_WEBRTC | HTTP_ACL_BEARER_OPTIONAL)
#define HTTP_ACL_ACLK_WEBRTC_DASHBOARD_WITH_BEARER (HTTP_ACL_DASHBOARD | HTTP_ACL_ACLK | HTTP_ACL_WEBRTC | HTTP_ACL_BEARER_REQUIRED)
