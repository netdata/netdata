        ...
        if(ret == EINTR) continue;
            ...
            if (ret == EINVAL) {
                if (!einval_printed) {
                    einval_printed++;
                    error(
                        "Invalid time given to clock_nanosleep(): clockid = %d, tv_sec = %"PRId64", tv_nsec = %ld",
                        clock,
                        req.tv_sec,
                        req.tv_nsec);
                }
                ...
                if (!enotsup_printed) {
                    enotsup_printed++;
                    error(
                        "Invalid clock id given to clock_nanosleep(): clockid = %d, tv_sec = %"PRId64", tv_nsec = %ld",
                        clock,
                        req.tv_sec,
                        req.tv_nsec);
                }
                ...
                if (!eunknown_printed) {
                    eunknown_printed++;
                    error(
                        "Unknown return value %d from clock_nanosleep(): clockid = %d, tv_sec = %"PRId64", tv_nsec = %ld",
                        ret,
                        clock,
                        req.tv_sec,
                        req.tv_nsec);
