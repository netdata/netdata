...
printhelp () {
	echo "Usage: docs/generate/checklinks.sh [-r OR -f <fname>] [OPTIONS]
	-r Recursively check all mds in all child directories, except docs/generate and node_modules (which is generated by netlify)
	...
	By default, nothing is actually checked. The following options tell it what to check:
	 ...
	 -l Check relative links to the netdata repo (and replace them with links that the html static site can live with, under docs/generate/src only)
