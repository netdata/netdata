...
Use this page when you need to understand why a trap has a specific name, category, severity, message, tag, or profile-derived chart when profile metrics are enabled. For listener setup and hardening, see [Configuration](/docs/npm/snmp-traps/configuration.md).
...
For field-level query details, see [Usage and Output](/docs/npm/snmp-traps/usage-and-output.md) and [Field Reference](/docs/npm/snmp-traps/field-reference.md).
...
Only committed traps update profile metrics. For the exact update rule (dedup-suppressed and failed-write traps do not update metrics, and an OTLP export failure does not roll back an already-updated metric), see [Metrics](/docs/npm/snmp-traps/metrics.md#profile-defined-metrics).
...
- To configure listener jobs, overrides, outputs, and `profile_metrics`, see [Configuration](/docs/npm/snmp-traps/configuration.md).
- To query received traps and read profile-enriched output, see [Usage and Output](/docs/npm/snmp-traps/usage-and-output.md).
- To understand source identity, relay handling, and device context, see [Enrichment](/docs/npm/snmp-traps/enrichment.md).
- To understand every emitted trap field, see [Field Reference](/docs/npm/snmp-traps/field-reference.md).
