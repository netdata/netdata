...
For listener setup and output options, see [Configuration](/docs/npm/snmp-traps/configuration.md). For the complete field list, see [Field Reference](/docs/npm/snmp-traps/field-reference.md).
...
The host must have `journalctl` from systemd 252 or later installed, because the files use the compact journal format. Use `sudo` when the trap journal files are not readable by your user. Use `journalctl --directory` to produce JSON output that your existing log shipper or SIEM ingestion path can consume:
...
For local query examples, see [Journal and querying](/docs/npm/snmp-traps/journal-and-querying.md).
For journal retention and rotation controls, see [Configuration](/docs/npm/snmp-traps/configuration.md#direct-journal-retention).

## OTLP/gRPC export

OTLP export is disabled by default. It uses gRPC transport only (HTTP/protobuf receivers on port `4318` are not supported), takes a `host:port`/`http://`/`https://` endpoint, and preflights the receiver at job startup so an unreachable collector blocks the job. For the full `otlp` option block, defaults, endpoint/TLS rules, and preflight behavior, see [Configuration](/docs/npm/snmp-traps/configuration.md#otlpgrpc-export).
...
The batching, queue, and retry knobs are defined in [Configuration](/docs/npm/snmp-traps/configuration.md#otlpgrpc-export).
...
For the complete journal-field-to-OTLP-attribute mapping table, see [Field Reference](/docs/npm/snmp-traps/field-reference.md#otlp-mapping-notes).
...
For receiver metrics, see [Metrics](/docs/npm/snmp-traps/metrics.md); for the default health alerts, see [Alerts](/docs/npm/snmp-traps/alerts.md).

## Related pages

- [Configuration](/docs/npm/snmp-traps/configuration.md) - Configure output backends, retention, OTLP endpoint settings, and secrets.
- [Field Reference](/docs/npm/snmp-traps/field-reference.md) - Map journal fields and OTLP attributes.
- [Journal and querying](/docs/npm/snmp-traps/journal-and-querying.md) - Query and export direct-journal trap rows locally.
- [Metrics](/docs/npm/snmp-traps/metrics.md) - Validate receiver health and forwarding failures.
- [Alerts](/docs/npm/snmp-traps/alerts.md) - Default alerts on OTLP export and write failures.
