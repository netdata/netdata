...
For the byte-unit details of `max_size` and rotation, see [Configuration](/docs/npm/snmp-traps/configuration.md#direct-journal-retention).

## Shipped limits and defaults

Useful starting points and the fixed caps that bound capacity — not capacity promises. For the full configurable option list, see [Configuration](/docs/npm/snmp-traps/configuration.md#option-map).
...
For how each control reads out in the receiver counters, see [Metrics](/docs/npm/snmp-traps/metrics.md#receiver-pipeline).
...
For the full quality checklist, see [Validation and Data Quality](/docs/npm/snmp-traps/validation-and-data-quality.md).

## What's next

- [Configuration](/docs/npm/snmp-traps/configuration.md) — buffer, retention, storm-control, and OTLP options.
- [Metrics](/docs/npm/snmp-traps/metrics.md) — the receiver counters to watch during a storm.
- [Alerts](/docs/npm/snmp-traps/alerts.md) — the default storm and dedup alerts, plus the kernel UDP buffer-drop alert.
- [Troubleshooting](/docs/npm/snmp-traps/troubleshooting.md) — missing traps, silent loss, and backend failures.
