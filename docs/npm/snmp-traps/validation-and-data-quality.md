...
Work through these checks in order during first deployment. Use [Quick Start](/docs/npm/snmp-traps/quick-start.md) to send a known test trap, and use [Journal and Querying](/docs/npm/snmp-traps/journal-and-querying.md) when you need local `journalctl` or Cloud Logs verification commands.
...
Compare `TRAP_SOURCE_IP` with `TRAP_SOURCE_UDP_PEER` (equal for direct senders, different for a trusted-relay override), then inspect the `source` object inside `TRAP_ENRICHMENT` to confirm `selected`, `method`, `trusted_relay`, and any `rejected_candidates`. For the full source-identity model and the step-by-step trust checklist, see [Enrichment](/docs/npm/snmp-traps/enrichment.md#how-to-validate-source-identity). Keep `source.trusted_relays` narrow: a broad range lets senders on that path influence attribution through `snmpTrapAddress.0`.
...
For OTLP-only jobs, validate the same source relationship with OTLP attributes: `snmp.source.ip` is the selected source and `network.peer.address` is the UDP peer. Use the [Field Reference](/docs/npm/snmp-traps/field-reference.md#otlp-mapping-notes) for the journal-to-OTLP mapping.
...
- If deduplication is enabled, expect the first matching trap row plus later `TRAP_REPORT_TYPE=deduplication_summary` rows. Check `dedup.window_sec`, `dedup.cache_max_entries`, and `dedup.key_varbinds` in [Configuration](/docs/npm/snmp-traps/configuration.md) when summaries do not match expectations.
...
- Use [Journal and Querying](/docs/npm/snmp-traps/journal-and-querying.md) for `snmp:traps` Function behavior, Cloud requirements, and local `journalctl --directory` commands.
...
- Check that retention settings keep enough local history for incident investigation. See [Configuration](/docs/npm/snmp-traps/configuration.md#direct-journal-retention) for retention defaults and settings.

For OTLP export:

- Confirm `snmp.trap.errors` `otlp_export_failed` stays clear.
- Confirm the downstream OTLP receiver stores the expected records.
- Confirm records carry `service.name=netdata-snmptrap` and `service.instance.id=<job>`. See [Forwarding to SIEM](/docs/npm/snmp-traps/forwarding-to-siem.md) for the OTLP export checks.
...
For OTLP ingestion, use the OTLP mapping in [Field Reference](/docs/npm/snmp-traps/field-reference.md). Build rules against report type, selected source, UDP peer, trap OID, trap name, category, severity, version, varbind payload, decode-error details, and dedup summary attributes.
...
- [Enrichment](/docs/npm/snmp-traps/enrichment.md) - Source identity, trusted relays, reverse DNS, and enrichment audit details.
- [Configuration](/docs/npm/snmp-traps/configuration.md) - Source controls, credentials, deduplication, rate limiting, retention, and output settings.
- [Trap Profiles](/docs/npm/snmp-traps/trap-profiles.md) - How profiles turn OIDs and varbinds into names, categories, severities, labels, and profile-defined metrics.
- [Field Reference](/docs/npm/snmp-traps/field-reference.md) - Complete field meanings, population rules, sensitive-data cautions, and OTLP mapping.
- [Journal and Querying](/docs/npm/snmp-traps/journal-and-querying.md) - Local `journalctl` and Cloud Logs query workflows.
- [Forwarding to SIEM](/docs/npm/snmp-traps/forwarding-to-siem.md) - OTLP export, resource attributes, and SIEM field validation.
- [Metrics](/docs/npm/snmp-traps/metrics.md) - Receiver metrics, pipeline counters, and errors.
- [Alerts](/docs/npm/snmp-traps/alerts.md) - Default health alert behavior on these receiver metrics.
- [Troubleshooting](/docs/npm/snmp-traps/troubleshooting.md) - Failure investigation paths when validation does not match expectations.
- [Anti-Patterns](/docs/npm/snmp-traps/anti-patterns.md) - Configurations and operational habits to avoid.
