...
Use this page to read the SNMP trap receiver metrics: whether the receiver is healthy, whether traps are flowing, and what the receiver is dropping or suppressing. For the default health alerts that ship for these metrics, see [Alerts](/docs/npm/snmp-traps/alerts.md).
...
For configuration details, see the [rate limiting, deduplication, and output backend sections](/docs/npm/snmp-traps/configuration.md).
...
Profile-defined metrics convert selected committed traps into time-series. They are disabled by default and must be enabled with `profile_metrics` in [Configuration](/docs/npm/snmp-traps/configuration.md).
