...
To see real numbers, filter by one exporter and one interface. Each packet then appears in exactly one record on that interface. See the [Anti-patterns page](/docs/npm/network-flows/anti-patterns.md) for the full framing.
...
- **You're setting up the plugin for the first time** — [Installation](/docs/npm/network-flows/installation.md), then [Quick Start](/docs/npm/network-flows/quick-start.md).
- **You have data, you want to find a bandwidth hog or trace an IP** — [Investigation Playbooks](/docs/npm/network-flows/investigation-playbooks.md).
- **You want to make sure your data is trustworthy** — [Validation and Data Quality](/docs/npm/network-flows/validation.md).
- **You want to avoid the most common mistakes** — [Anti-patterns](/docs/npm/network-flows/anti-patterns.md).
