...
- The "sampling but not telling" case happens with NetFlow v7 (no rate field), v5 with rate=0, and v9 / IPFIX exporters that don't send a Sampling Options Template. Fix on the exporter side, or apply `enrichment.override_sampling_rate` per source prefix to substitute a known rate (see [Configuration → enrichment](/docs/npm/network-flows/configuration.md#enrichment) and the [Static Metadata integration card](/src/crates/netflow-plugin/integrations/static_metadata.md)).
...
The raw tier dominates. If raw-tier disk usage is approaching the `size_of_journal_files` you set, plan retention vs. capacity (see [Sizing and Capacity Planning](/docs/npm/network-flows/sizing-capacity.md)).
...
For each, read the [Anti-patterns](/docs/npm/network-flows/anti-patterns.md) page first — most "data is wrong" reports are expected behaviour misread.

## What's next

- [Plugin Health Charts](/docs/npm/network-flows/visualization/dashboard-cards.md) — The charts referenced above.
- [Anti-patterns](/docs/npm/network-flows/anti-patterns.md) — Misreadings to rule out before declaring a bug.
- [Investigation Playbooks](/docs/npm/network-flows/investigation-playbooks.md) — Concrete recipes for common questions.
- [Troubleshooting](/docs/npm/network-flows/troubleshooting.md) — Recovery for the symptoms above.
