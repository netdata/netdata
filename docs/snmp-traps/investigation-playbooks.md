...
- [Metrics](/docs/snmp-traps/metrics.md)
...
- **Inspect:** `TRAP_OID`, `TRAP_NAME`, `TRAP_CATEGORY`, and `snmp.trap.errors` `unknown_oid` first; for the full field and evidence list, see [Troubleshooting](/docs/snmp-traps/troubleshooting.md#unknown-oids-profile-load-failures-or-template-issues).
...
- **Inspect:** `TRAP_SUPPRESSED_COUNT`, `TRAP_SUPPRESSED_FINGERPRINTS`, and `TRAP_REPORT_PERIOD_SEC` first; for the full evidence list and config checks, see [Troubleshooting](/docs/snmp-traps/troubleshooting.md#rate-limiting-or-deduplication-surprise).
