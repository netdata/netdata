<!--startmeta
custom_edit_url: "https://github.com/netdata/netdata/edit/master/docs/snmp-traps/investigation-playbooks.md"
...
- For OTLP-only jobs, inspect the equivalent downstream log attributes instead of local journal field names. Use the OTLP mapping in [Field Reference](/docs/snmp-traps/field-reference.md#otlp-mapping-notes).
...
- [Field Reference](/docs/snmp-traps/field-reference.md)
- [Journal and Querying](/docs/snmp-traps/journal-and-querying.md)
- [Metrics and Alerts](/docs/snmp-traps/metrics-and-alerts.md)
...
- **Next action:** Check the receiver pipeline in order: `received`, `decoded`, `accepted`, `committed`. If `received` is flat, check sender and network delivery. If `received` rises but `committed` is flat, inspect `dropped`, `dedup_suppressed`, `write_failed`, and the matching error dimensions.
...
- **Inspect:** `TRAP_REPORT_TYPE`, `TRAP_SOURCE_IP`, `TRAP_SOURCE_UDP_PEER`, `TRAP_OID`, `TRAP_NAME`, `TRAP_CATEGORY`, `TRAP_SEVERITY`, `TRAP_VAR_*`, `TRAP_JSON`, `snmp.trap.errors` `unknown_oid`, and `snmp.trap.source_errors` `unknown_oid`.
...
- **Next action:** Collect the numeric `TRAP_OID`, the source, a minimized `TRAP_JSON` summary, and the operational meaning from vendor documentation or device evidence. Add or adjust [Trap Profiles](/docs/snmp-traps/trap-profiles.md) only after confirming the OID and expected severity/category policy.
...
- **Inspect:** `TRAP_REPORT_TYPE=decode_error`, `TRAP_DECODE_ERROR_KIND`, `TRAP_DECODE_ERROR`, `TRAP_SOURCE_IP`, `TRAP_SOURCE_UDP_PEER`, `TRAP_SOURCE_UDP_PORT`, `TRAP_ENGINE_ID`, `TRAP_PACKET_SIZE`, `TRAP_PACKET_SHA256`, `TRAP_LISTENER`, `TRAP_JSON`, `snmp.trap.errors` `auth_failures`, `usm_failures`, and `unknown_engine_id`.
...
- **Inspect:** `TRAP_SUPPRESSED_COUNT`, `TRAP_SUPPRESSED_FINGERPRINTS`, `TRAP_REPORT_PERIOD_SEC`, `TRAP_JSON`, `snmp.trap.dedup_suppressed`, `snmp.trap.pipeline` `dedup_suppressed`, and `snmp.trap.source_pipeline` `dedup_suppressed`.
