<!--startmeta
custom_edit_url: "https://github.com/netdata/netdata/edit/master/docs/snmp-traps/alerts.md"
...
The default alerts watch receiver health and high-severity trap flow. They do not assert the complete health state of the sending devices. For the underlying receiver metrics these alerts read, see [Metrics](/docs/snmp-traps/metrics.md).
...
The trap receiver also benefits from the system-level `1m_ipv4_udp_receive_buffer_errors` alert on `ipv4.udperrors`. It catches datagrams the kernel drops before they ever reach the collector, which the receiver's own pipeline metrics cannot see. That alert is routed `to: silent` by default — route it to a recipient to be notified of kernel UDP drops during a storm. See [Sizing and Capacity](/docs/snmp-traps/sizing-and-capacity.md#kernel-udp-buffer-drops).
...
- [Metrics](/docs/snmp-traps/metrics.md) - Read the receiver pipeline, processing errors, dedup counters, and source metrics these alerts watch.
- [Troubleshooting](/docs/snmp-traps/troubleshooting.md) - Investigate the failures that raise these alerts.
- [Configuration](/docs/snmp-traps/configuration.md) - Tune allowlists, rate limits, deduplication, and output backends that change alert behavior.
