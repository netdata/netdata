...
- Shared clients now attach to the MCP SDK’s `onclose` hook. If the stdio child exits or a remote transport disconnects unexpectedly, we treat it the same as a failed health probe and enter the restart loop immediately, so the next tool call never pays a full timeout just to discover a dead connection.
...
- LLM error classification: `mapError` combines structured fields (HTTP status, error name/code) with **message-pattern classification applied once at the provider entry**; the rest of the app consumes structured kinds only.
- Fallback policy: any non‑200 HTTP response skips the current provider/model pair; status‑specific fallback behavior remains unchanged for 200 responses.
...
- Turn/session success/failure: a turn succeeds only if **finalization readiness** is achieved (final report plus any required plugin META blocks) or at least one non-progress/batch tool is executed. **Executed** means the tool passed validation and execution started (transport‑level); schema/unknown‑tool rejections do **not** count, while timeouts/transport errors after execution do. Progress/batch still return responses but do not satisfy success. Missing META after a final report sets `invalid_response` and triggers META-only retries; exhaustion can synthesize `reason: "final_meta_missing"`. Retry exhaustion fails the session immediately (single ERR + synthetic session report). Each failed turn emits one WRN with slugged reasons and full response (capped ~128 KB). Collapse logic may only shrink `maxTurns`.
...
- XML transport (single xml-final path) is handled by `XmlToolTransport` (`src/xml-transport.ts`): provider tool definitions stay native (tool_calls) while the final report and plugin META wrappers travel via XML. META wrappers are extracted before XML tool parsing, and XML-NEXT always pairs FINAL guidance with META guidance, switching to META-only mode when the final report is locked. Streaming suppresses locked-final retries to avoid duplicate output. Accounting uses `source: xml`; final-report command recorded as `agent__final_report_xml`; context guard still applies based on XML message size and tool outputs.
...
- Leaked tool-call fallback can parse XML-like tool-name tags (e.g., `<agent__task_status>...</agent__task_status>`) with `<parameter name="x">value</parameter>` and task_status bare tags, plus Minimax wrapper format `<minimax:tool_call><invoke name="tool">...</invoke></minimax:tool_call>`; boolean strings are coerced to booleans, repeated fields join with newlines, malformed `</x>` closes are accepted, and extraction is limited to tools allowed for the current turn.
