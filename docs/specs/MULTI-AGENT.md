...
- **How it differs from sub-agent tools**:
  ...
  - Orchestration runs are captured in opTree as `session` ops under `steps[]` with attributes `{ provider: "orchestration", kind: "advisors" | "router_handoff" | "handoff" }`, so snapshots include their LLM requests/responses.
- **Flow**:
