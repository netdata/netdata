...
- **XML transport (tooling.transport = xml | xml-final)**:
  - `xml`: provider tool list hidden; toolChoice forced to `auto`. Each turn injects XML-NEXT (nonce, slots, tool schemas, progress slot when enabled, final-report slot) and optionally XML-PAST (prior turn tool results). Model must emit tags like `<ai-agent-<nonce>-0001 tool=\"tool_name\">{json}</ai-agent-<nonce>-0001>`; native `tool_calls` are ignored with a warning.
  - `xml-final`: provider tools stay native (tool_calls) but the final report must be wrapped in the XML tag. XML-NEXT reminds the model to call tools natively; progress slot suppressed; XML-PAST omitted.
  ...
  - CLI override: `--tooling-transport native|xml|xml-final` wins over config/frontmatter when you need to toggle XML quickly.
