...
- **Tool failures**: `ToolsOrchestrator.executeWithManagement` never retries a tool. Failures are logged (`severity='WRN'`, `type='tool'`) and inserted into the conversation as tool-result messages so the LLM can course-correct. MCP tool parameters are validated against their JSON schemas via AJV; invalid args surface as `(tool failed: Validation error: ...)` without calling the MCP server, while un-compilable schemas log a warning and skip validation. Unknown tool names return a tool failure instructing the model to use the exact tool name (typically `<namespace>__<tool>`). If the model calls an unqualified tool name that exactly matches a tool suffix and the payload validates against that tool’s schema, the agent auto-corrects the tool name (logs a warning and rewrites the tool call in history). When the SDK injects a missing tool result, the synthetic tool message is tagged with `metadata.injectedToolResult=true` and `metadata.injectedReason` (`tool_missing` or `xml_wrapper`); auto-correction only re-executes when the reason is `tool_missing`. Output that exceeds `toolResponseMaxBytes` **or** would overflow the context budget is stored under the per-run tool_output root (`/tmp/ai-agent-<run-hash>/session-<uuid>/...`) and replaced with a handle message instructing the model to call `tool_output`. Handles are relative paths like `session-<uuid>/<file-uuid>`. Truncation now happens only inside the `tool_output` module (as a fallback), not in the main loop. Storage warnings include `handle`, `reason` (`size_cap|token_budget|reserve_failed`), `bytes`, `lines`, and `tokens`.
...
- **Reasoning management**: Frontmatter/CLI `reasoning` now accepts `none|minimal|low|medium|high|default|unset`. `none` **or** `unset` disables reasoning (sets `reasoningValue = null`), while omitting the key (or writing `default`) lets the global fallback (`defaults.reasoning` or `--default-reasoning`) decide. These settings plus `reasoningTokens` feed `ProviderReasoningMapping` before each turn. Anthropic-style thinking streams arrive via `onEvent(type='thinking')` and surface as `THK` log entries plus grey stderr text. Each LLM attempt also triggers `onEvent(type='turn_started')` with `{ turn, attempt, isRetry, isFinalTurn, forcedFinalReason?, retrySlugs? }` so headends can render `Turn X, Attempt Y` headers even when no reasoning/progress chunks arrive.
- **Event stream (`onEvent`)**: All output/log/progress/final_report/handoff signals flow through `onEvent(event, meta)`. `meta.isFinal` is **authoritative only for `final_report` events**; treat it as informational for all other event types. When a handoff is configured or selected, the upstream payload is emitted as `type='handoff'` (not `final_report`).
- **Context-window guard**: `AIAgentSession` tracks `currentCtxTokens`, `pendingCtxTokens`, `newCtxTokens`, and `schemaCtxTokens`. If projected totals exceed provider limits it enters “forced final turn” mode: tool calls disabled, XML-NEXT carries the final-turn instruction (no extra system notices), and `logExit('EXIT-TOKEN-LIMIT', ...)` recorded.
- **System prompt composition**: `agent-loader` flattens `.ai` content (resolves `include:`), strips frontmatter, then `applyFormat` injects `${FORMAT}` description. Runtime appends the native tool instruction block; XML-NEXT provides per-turn nonce/wrapper guidance (and forced-final instructions). Schemas ride in XML-NEXT for the final report, not in the system prompt.
- **Injected user messages**:
  - Only two notice types are injected: XML-NEXT (ephemeral, per attempt) and TURN-FAILED (persistent retry feedback).
  - XML-NEXT is the single injection point for per-turn guidance, including forced-final instructions; no separate final-turn or tool reminder messages are appended.
- **Internal tools**: `agent__final_report` is always available; the LLM must call it exactly once to conclude. `agent__batch` enables parallel tool execution for models without native parallel support (see below). `tool_output` is exposed when oversized tool results are stored; call it with a `handle` + `extract` to retrieve the needed data. Sub-agents register as `agent__<toolName>`. REST imports register as `rest__<toolName>`.
- **XML transport (single path)**:
  - Provider tools stay native (tool_calls) but the final report must be wrapped in the XML tag. XML-NEXT carries the nonce + final slot and per-turn guidance (including forced-final instructions); XML-PAST omitted.
...
- **Exit codes & summaries**: `AIAgentSession.logExit` emits standardized tags (`EXIT-FINAL-ANSWER`, `EXIT-TOOL-FAILURE`, `EXIT-TOKEN-LIMIT`, `EXIT-UNCAUGHT-EXCEPTION`, etc.) whose severity mirrors `fatal`. Synthetic failures keep `fatal=true`/`severity='ERR'`, while true final answers set `fatal=false` and remain `VRB/FIN`. `FIN` summaries follow even on failure, and telemetry spans capture `ai.agent.success` plus turn counts.
...
- **Turn success rule**: A turn only succeeds if an accepted/valid final report is produced or at least one non-progress/batch tool executes. Progress and batch always return results but do not satisfy success. Retry exhaustion fails the session immediately; failed turns emit a single WRN with slugged reasons and full LLM response (capped ~128 KB). Sessions emit one ERR on failure; synthetic final reports are produced only on session failure.
...
| `toolResponseMaxBytes` | bytes | `12288` | Over-limit responses are stored and replaced with a `tool_output` handle (no core truncation). |
| `toolOutput` | object | unset | Override tool_output settings (`enabled`, `storeDir`, `maxChunks`, `overlapPercent`, `avgLineBytesThreshold`, `models`). |
...
- **Accounting flushes**: `onEvent(type='accounting_flush')` appends newline-delimited JSON entries to `billingFile`. Each entry includes `timestamp`, `status`, `latency`, and either LLM token counts or tool character counts. These files are append-only; truncate with care.
