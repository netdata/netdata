...
- **Slack Block Kit formatting**: `slack-block-kit` requires Slack *mrkdwn* inside `section`/`context` blocks (not GitHub Markdown). Avoid `#` headings and Markdown tables; use header blocks or bold lines instead, format links as `<url|text>`, escape `& < >`, and use `section.fields` for 2‑column layouts.
