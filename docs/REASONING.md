...
- When a provider emits `<think>...</think>` inside normal content (e.g., OpenAI-compatible streams), the block is routed to `onThinking` and excluded from `onOutput` and XML parsing; leading whitespace before `<think>` is tolerated.
