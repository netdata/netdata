...
2. **OpenTelemetry (OTLP) log ingestion** — Follow [Send OpenTelemetry data to Netdata](/docs/opentelemetry/send-data-to-netdata.md) to receive logs through OTLP/gRPC and index them for fast querying in the Logs tab (the `otel-logs` source). This method does not depend on a local systemd installation.
