...
1. **Create your script** — write a `.ps1` file that outputs status text with optional performance data and exits with code `0` (OK), `1` (WARNING), `2` (CRITICAL), or `3` (UNKNOWN). Save it somewhere accessible (e.g. `C:\Netdata\checks\`):
