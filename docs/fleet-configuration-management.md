...
As infrastructures grow to thousands of nodes across mixed environments (Linux, Kubernetes, Windows, macOS, FreeBSD), managing observability agents becomes one of the most painful operational tasks.
...
|------------------------------------|-------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------|----------------|----------------|----------------------------------------------------------------------------------------|
| **Auto-deploy**                    | ✅ [kickstart.sh](/packaging/installer/methods/kickstart.md)¹ | ✅ [Helm](/packaging/installer/methods/kubernetes.md) | ✅ kickstart.sh | ✅ kickstart.sh | ✅ [MSI](/packaging/windows/WINDOWS_INSTALLER.md)² (silent) |
...
3. **Manual**: No built-in auto-update mechanism yet; requires PowerShell/SCCM/GPO automation (see [Windows Automatic Updates](/packaging/windows/WINDOWS_INSTALLER.md#automatic-updates) for the scheduled-task recipe)
...
| **Hybrid (Best)** | Most organizations                    | Base config in Git, credentials/thresholds via [UI](/docs/netdata-agent/configuration/dynamic-configuration.md) |

## Deployment at Scale

Netdata's fleet deployment strategy minimizes operational overhead while maximizing flexibility:
...
**Strong backwards compatibility**: upgrades don't break existing configurations or data, maintaining compatibility across versions (see [Netdata Infrastructure](/docs/welcome-to-netdata.md) for architectural details).

## Configuration Management at Scale

Netdata supports both IaC and Dynamic Configuration, minimizing manual configuration through extensive auto-discovery.
...
For organizations with established DevOps practices, Netdata fully supports configuration management through traditional IaC tools like [Ansible](/packaging/installer/methods/ansible.md), allowing version-controlled, auditable deployments.

### Dynamic Configuration via the dashboard

Through [Netdata Cloud and the Dynamic Configuration Manager](/docs/netdata-agent/configuration/dynamic-configuration.md) and authenticated Netdata Agent and Parent dashboards (the user must sign-in to the dashboard), users can manage collector configurations and alert rules across their entire fleet without touching configuration files, restarting or redeploying agents.
...
1. **[Dynamic Configuration (DynCfg)](/docs/netdata-agent/configuration/dynamic-configuration.md)** - Runtime configurations via Netdata Cloud UI or API
2. **[User Configuration](/docs/netdata-agent/configuration/README.md)** - Files in `/etc/netdata` or `/opt/netdata/etc/netdata`
...
### [Process and Application Monitoring](/src/collectors/apps.plugin/metadata.yaml) (apps.plugin)
...
This gives comprehensive monitoring even for software without specific collectors, an essential first line of observability.

### [Applications](/src/collectors/REFERENCE.md) (go.d.plugin)
...
- **[Docker container discovery](/src/go/plugin/go.d/collector/docker/metadata.yaml) (dockersd)** - Discovers applications running in Docker containers
- **[Kubernetes service discovery](/packaging/installer/methods/kubernetes.md) (k8ssd)** - Discovers services running in Kubernetes pods
- **[SNMP device discovery](/src/go/plugin/go.d/collector/snmp/metadata.yaml) (snmpsd)** - Discovers and profiles SNMP-enabled network devices
...
Users can supply missing credentials or parameters through the Netdata Cloud UI, letting collectors retry without manual file editing or a restart.

Supported applications include databases (MySQL, PostgreSQL, Redis, MongoDB), web servers (NGINX, Apache, HAProxy), message queues (RabbitMQ, Kafka), SNMP, and [many more](/src/collectors/REFERENCE.md).
...
**Update Management** (manual - no built-in auto-updater yet):
...
**[Windows-Specific Auto-Discovery](/packaging/windows/WINDOWS_INSTALLER.md)**:
...
- **[Cluster state](/packaging/installer/methods/kubernetes.md)** (k8s_state): Monitors nodes, pods, containers, deployments, services
- **[Kubelet metrics](/packaging/installer/methods/kubernetes.md) (k8s_kubelet)**: Container and pod resource usage, volume statistics
- **[Control plane](/packaging/installer/methods/kubernetes.md)** (kube-proxy, kube-scheduler, kube-controller-manager): When accessible

**[Service Discovery](/src/collectors/SERVICE-DISCOVERY.md)** (k8ssd):
...
Most Kubernetes deployments require no configuration beyond the initial [Helm chart installation](https://github.com/netdata/helmchart/blob/master/charts/netdata/README.md).

## Comparison with Other Platforms

Useful context for planning migrations or hybrid deployments:
...
- [Parent-Child Configuration Reference](/src/streaming/README.md)
- [Parent Configuration Best Practices](/docs/observability-centralization-points/best-practices.md)
- [Streaming and Routing Reference](/docs/streaming-routing.md)
- [Dynamic Configuration Manager](/docs/netdata-agent/configuration/dynamic-configuration.md)
