...
While charts that update every second with new metrics are useful for understanding the immediate state of a node, true
monitoring and troubleshooting happens with a closer look. That's why every chart in the Netdata dashboard is fully
interactive, and all interactions get synchronized across every chart for a consistent view of your node.
...
values presented as dimensions. If you then click on the chart, it becomes locked to this timeframe, which is useful if
you want to jump to a different chart to look for possible correlations.
...
Dragging your mouse/finger to the right pans backward through time, while dragging to the left pans forward in time.
Think of it like pushing the current timeframe off the screen so you can see what came before or after.
...
If you've performed any of the above interactions and want to return to the default auto-updating state, double-click on
the chart.
...
families](/docs/dashboards/chart-dimensions-contexts-families.mdx) to complete your understanding of how Netdata
organizes its dashboards.

If you feel comfortable with the [dashboard](/docs/dashboard/how-dashboard-works.mdx) and interacting with charts, we
recommend moving in to learning about [configuration](/docs/configure/nodes.md). While Netdata doesn't _require_ a
complicated setup process or a query language to create charts, there are a lot of ways to tweak the experience to match
your needs.

[![analytics](https://www.google-analytics.com/collect?v=1&aip=1&t=pageview&_s=1&ds=github&dr=https%3A%2F%2Fgithub.com%2Fnetdata%2Fnetdata&dl=https%3A%2F%2Fmy-netdata.io%2Fgithub%2Fdocs%2Fdashboard%2Finteract-charts&_u=MAC~&cid=5792dfd7-8dc4-476b-af31-da2fdb9f93d2&tid=UA-64295674-3)](<>)
