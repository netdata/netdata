...
While [panning through time and zooming in/out](/docs/dashboard/interact-charts.mdx) from charts it is helpful
...
With this, we aim to bring more clarity and allow you to distinguish if the content you are looking at is live or historical and also allow you
 to always refresh the content of the page when the tabs are on the background. 

Main use cases for **Force Play**:
* You use a terminal or deployment tools to do changes in your infra and want to see immediately, Netdata is on the background, the impact 
of these changes
...
With the timezone select you have the ability to change the timezone on Netdata Cloud. More often than not are 
distributed in different timezones and they need to collaborate.

Our goal is to make it easier for you and your teams to troubleshoot on your timezone preference and communicate easily 
timezones and timeframes without need to be concerned about the specificity of them.
