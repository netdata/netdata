...
By default, Netdata's web server listens on port `19999`. You can change this to any available port by editing the `[web]` section in `netdata.conf`.

### Change the Listening Port

To change the default port from `19999` to a custom port (for example, `800`):

1. Edit `netdata.conf` using the `edit-config` script:
...
2. Find the `[web]` section and modify the `default port` setting:
...
3. Save the file and restart Netdata:

   ```bash
   sudo systemctl restart netdata
   ```

After the restart, Netdata will be accessible at `http://NODE:800` (replace `NODE` with your node's IP address or hostname).

:::note

The `default port` option sets the port Netdata uses when no specific port is defined in the `bind to` setting. For advanced configurations with multiple ports or IP addresses, see the [Web Server Configuration Reference](/src/web/server/README.md#binding-netdata-to-multiple-ports).
