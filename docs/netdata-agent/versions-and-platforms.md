...
When platforms are removed from the [Binary Distribution Packages](#binary-distribution-packages) list, they default to install or update Netdata to a static build. This may mean that after platforms become EOL, Netdata on them may lose some of its features. We recommend to upgrade the operating system before it becomes EOL, to continue using all the features of Netdata.
