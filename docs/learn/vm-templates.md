...
- Backup up in `/var/lib/netdata/cloud.d/cloud.conf` (Netdata Cloud authentication)
- Never changes
...
The kickstart script automatically creates `/etc/netdata/claim.conf` if called with claiming information (e.g. when the kickstart command is copied fron Netdata Cloud).
...
# cloud-init config
runcmd:
  - bash -c "curl https://get.netdata.io/kickstart.sh -- --claim-token TOKEN --claim-rooms ROOM_ID" | sh
```
