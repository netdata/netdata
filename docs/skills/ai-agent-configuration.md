...
| `toolOutput` | object | unset | Override tool_output settings (`enabled`, `maxChunks`, `overlapPercent`, `avgLineBytesThreshold`, `models`). `storeDir` is ignored; root is always `/tmp/ai-agent-<run-hash>`. |
