...
- **[3.4 Expressions, Operators, and Functions](/docs/alerts/alert-configuration-syntax/4-expressions-operators-functions.md)** explains the expression language used in `warn`, `crit`, and `calc` lines: arithmetic, comparisons, logical operators, and helper functions
- **[3.5 Variables and Special Symbols](/docs/alerts/alert-configuration-syntax/5-variables-and-special-symbols.md)** lists the variables you can use inside those expressions (`$this`, `$status`, `$now`, and chart/context variables) and how to inspect them via the `alarm_variables` API