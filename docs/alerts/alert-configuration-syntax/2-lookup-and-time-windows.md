...
- **3.3 Calculations and Transformations (`calc`, `absolute`, `percentage`, `anomaly-bit`)** explains how to transform lookup results (or build alerts without explicit lookups) using `calc` and related flags
- **6.5 Trend and Capacity Alerts** gives concrete examples of how different time windows change alert behavior in practice
- **9.3 Querying Alert Variables and Metadata** for using the `alarm_variables` API to inspect available dimensions when building lookups