...
By default, Netdata uses a database engine that uses RAM to store recent metrics. For long-term metrics storage, the
database engine uses a "spill to disk" feature that also takes advantage of available disk space and keeps RAM usage
low.
