...
whether the receiver or transmitter in a streaming setup. You need properly-labeled metrics archiving so you can sort,
...
receiver-transmitter status, and more.
...
-   Whether the system is a streaming receiver or transmitter
...
You may have noticed the `_is_receiver` and `_is_transmitter` automatic labels from above. Host labels are also now
streamed from a transmitter to its receiver node, which concentrates an entire infrastructure's OS, hardware, container,
and virtualization information in one place: the receiver.

Now, if you'd like to remind yourself of how much RAM a certain transmitter node has, you can simply access
`http://localhost:19999/host/RECEIVER/api/v1/info` and reference the automatically-generated host labels from the
transmitter system. It's a vastly simplified way of accessing critical information about your infrastructure.

> ⚠️ Because automatic labels for transmitter nodes are accessible via API calls, and contain sensitive information like
...
You can also use `_is_receiver`, `_is_transmitter`, and any other host labels in both health entities and metrics
...
In a streaming configuration where a receiver node is triggering alarms for its transmitters, you could create health
entities that apply only to transmitters:

```yaml
 host labels: _is_transmitter = true
```
