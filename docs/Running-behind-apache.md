...
```
    Header always set Content-Security-Policy "default-src 'unsafe-inline' http://localhost:19999 https: 'self' 'unsafe-eval'; script-src 'unsafe-inline' https: 'self' 'unsafe-eval'; style-src https: 'self' 'unsafe-inline'"
```
