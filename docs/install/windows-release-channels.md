...
> `Driver failed to start: ERROR_INVALID_IMAGE_HASH (577). This usually indicates a driver signature verification failure. The driver binary may be corrupted, unsigned, or signed with an untrusted certificate.`

**What this means**: 64-bit Windows requires kernel-mode drivers to carry a valid digital signature. The Netdata hardware metrics driver (`netdata_driver.sys`) ships **unsigned** — no driver-signing step exists in the Netdata build pipeline. The catalog file (`netdata_driver.cat`) referenced by the driver INF is generated by Inf2Cat during development builds, but it is never digitally signed, and the catalog-creation step is commented out in the release packaging script (`packaging/windows/package-windows.sh`). Windows therefore refuses to load the driver under default signature enforcement.
...
**What to do**: Reinstalling Netdata will not resolve this — the driver is unsigned in every release. You have two options:
...
2. **Enable Windows test signing** (advanced): If CPU temperature metrics are required, enable test signing mode to allow unsigned kernel drivers to load:
...
> **Warning**: Test signing mode lowers system security by allowing any unsigned or test-signed kernel driver to load — not just Netdata's. Enable it only if you understand this trade-off. To disable it later, run `bcdedit /set testsigning off` and reboot.
