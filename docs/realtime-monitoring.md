...
- **The Performance Sweet Spot:** One second is highly efficient and universally safe. Sub-second collection is not supported — 1 second is the minimum collection interval by design, so polling more frequently than once per second (for example every 500 ms) is not possible.
