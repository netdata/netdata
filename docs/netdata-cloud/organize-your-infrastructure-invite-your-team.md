...
2. Assign a role to each invited user. Space-level roles determine what someone can do across the Space:
    ...
    - **Admin** — full control of the Space, including managing users, Rooms, nodes, notifications, and billing, plus access to every Room. Assign **Admin** to make the invited user a co-administrator of the Space alongside you.
   ...
   Which roles you can assign depends on your plan. To add a co-administrator, assign **Admin** — only an existing Admin can grant the Admin role, so you must already be an Admin to add another one. Role assignments take effect immediately.
