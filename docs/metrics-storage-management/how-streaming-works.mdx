...
When one node streams metrics to another, the node receiving metrics can can visualize them on the
[dashboard](/docs/visualize/interact-dashboard-charts.md), run health checks to [trigger
...
- **Child**: A node, running Netdata, that sends metric data in real time.
...
**The streaming connection between nodes is _not_ HTTP, and thus cannot be proxied by web proxies, like Nginx or
Apache.**

## What's next?

Now that you understand the fundamentals of streaming metrics between nodes, go ahed and [enable
