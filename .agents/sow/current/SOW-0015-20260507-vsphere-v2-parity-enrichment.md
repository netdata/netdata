...
### 2026-05-20 Label-Enrichment Config Cleanup Decision

The user directed removal of `collect_inventory_path_label` and
`vm_guest_labels` before merge. The remaining label-enrichment options stay flat
as top-level `tag_categories` and `custom_attributes` allowlists.

Removal scope:

- remove the `InventoryPathLabel` and `VMGuestLabels` config fields;
- remove inventory-path label construction and VM guest hostname/IP/OS label
  construction from chart labels;
- remove the VM guest-label enum validation and readiness rows;
- remove `collect_inventory_path_label` and `vm_guest_labels` from
  `config_schema.json`, `metadata.yaml`, stock `go.d/vsphere.conf`, generated
  integration docs, and test fixtures;
- keep vSphere tag category and custom-attribute label allowlists as flat
  `[]string` options.

Evidence and reason:

- `inventory_path` duplicates hierarchy labels and exposes internal naming.
- VM guest hostname, IP, and OS labels are sensitive and marginal enough to
  exclude from this PR.
- Tag categories and custom attributes are user-defined metadata names, so
  glob-pattern lists remain the right shape.
- DYNCFG and metadata already visually group label options, and the remaining
  two fields are too small to justify a one-off nested `labels:` config object.
...
15. Sensitive enrichment labels
   ...
   User concern 2026-05-08: Netdata is a monitoring system; users need labels to filter and group by vSphere metadata. The implementation keeps opt-in vSphere tag and custom-attribute labels with explicit allowlists.

   Superseding user decision 2026-05-20: remove `collect_inventory_path_label` and `vm_guest_labels` before merge. Inventory-path labels, VM guest hostname/IP/OS labels, and more sensitive device identity labels such as MACs, IQNs, WWNs, and datastore paths remain excluded from this PR pending a separate product decision.
...
| P29 | Inventory paths, guest hostname/IP, MAC/IQN/WWN identity | Feasible in subsets, removed from this PR | Datadog filters, Telegraf IP option, OTel inventory-path attributes, Broadcom guest/device/storage objects | Out of scope for this PR by 2026-05-20 user decision. Sensitive identity labels require a future product decision. |
...
- Added opt-in user-metadata label enrichment:
  - `tag_categories` allowlists vSphere tag categories with one glob pattern per YAML list item;
  - `custom_attributes` allowlists vSphere custom attributes with one glob pattern per YAML list item;
  - `collect_inventory_path_label` and `vm_guest_labels` were removed before merge by user decision.
...
- Added opt-in vSphere user metadata labels:
  - `tag_categories` defaults to an empty allowlist and matches vSphere tag category names with one glob pattern per YAML list item;
...
- `go test ./plugin/go.d/collector/vsphere -run 'TestCollector_(AddsInventoryPathLabel|AddsVMGuestLabels|Init_ReturnsFalseIfInvalidVMGuestLabel|V2CompatibilitySurface)$' -count=1` passed from `src/go/` after adding opt-in inventory-path and VM guest labels. These label options and their tests were deleted by the 2026-05-20 label-enrichment cleanup decision.
...
- Specs: added `.agents/sow/specs/vsphere-v1-compatibility-manifest.md` and `.agents/sow/specs/vsphere-parity-matrix.md`; updated the compatibility manifest to record the accepted chart-ID break, additive V2 `id` instance label, non-powered-on discovery/property-only semantics, fail-soft empty host/VM performance cycles, selector keys being optional in DynCfg schema, additive cluster DRS/HA property-status charts, static inventory-count chart, opt-in vSphere tag/custom-attribute label keys/config, opt-in VM disk capacity/performance, opt-in VM network-interface performance, opt-in host physical network-interface performance, opt-in host disk/LUN/device performance, opt-in host storage-adapter performance, opt-in host storage-path performance, opt-in host CPU-instance performance, opt-in host/VM power metrics, opt-in datastore cluster contexts/config, `collect_network_topology`, and the cached Function contract; updated parity rows P01/P02/P06/P07/P08/P09/P10/P11/P12/P13/P14/P15/P16/P17/P18/P19/P20/P21/P22/P28/P29/P30/P31/P32 to implemented, implemented-subset, or out-of-scope status.
- End-user/operator docs: updated `README.md` and `metadata.yaml` for new snapshot metrics, snapshot alerts, datastore aggregate metrics, VM/host power-state metrics, VM/host property-status metrics, cluster DRS/HA property-status metrics, inventory-count metrics, opt-in VM disk capacity/performance metrics, opt-in VM network-interface performance metrics, opt-in host physical network-interface performance metrics, opt-in host disk/LUN/device performance metrics, opt-in host storage-adapter performance metrics, opt-in host storage-path performance metrics, opt-in host CPU-instance performance metrics, opt-in host/VM power metrics, opt-in datastore cluster metrics, opt-in vSphere tag/custom-attribute labels, readiness/topology Functions, `collect_network_topology`, power-state discovery/performance behavior, zero no-snapshot semantics, inaccessible datastore capacity semantics, and the `id` label; expanded the stock `go.d/vsphere.conf` comments so file-based configuration is self-explanatory. Chart-ID continuity loss remains accepted and not emphasized in end-user docs.
...
- Added `.agents/sow/specs/vsphere-v1-compatibility-manifest.md` as the current v1 compatibility contract for the migration, including the accepted chart-ID break, V2 `id` label, additive default-safe metrics, optional vSphere tag/custom-attribute label enrichment, optional VM disk capacity/performance, optional VM network-interface performance, optional host physical network-interface performance, optional host disk/LUN/device performance, optional host storage-adapter performance, optional host storage-path performance, opt-in host CPU-instance performance, optional datastore cluster metric contracts, `collect_network_topology`, and cached Function contracts.
- Added `.agents/sow/specs/vsphere-parity-matrix.md` as the current parity/default-policy contract for implementation planning, including implemented rows P06-P22/P30-P32, explicitly excluded identity-label row P29, and explicitly excluded event row P27.
...
- Snapshot, datastore aggregate, power-state, VM/host property-status, cluster DRS/HA property-status, inventory-count, optional VM disk capacity/performance, optional VM network-interface performance, optional host physical network-interface performance, optional host disk/LUN/device performance, optional host storage-adapter performance, optional host storage-path performance, optional host CPU-instance performance, optional host/VM power metrics, optional datastore cluster, optional vSphere tag/custom-attribute label, readiness/topology Function, and optional Network/DVPG topology docs are now updated in `README.md` and `metadata.yaml`; stock `go.d/vsphere.conf` now documents selectors, secrets, vnode behavior, power-state discovery/performance behavior, opt-in labels, opt-in high-cardinality groups, opt-in power metrics, optional network topology discovery, and safe discovery defaults inline.
