...
| [otel-offline-wal-sfst-query.md](otel-offline-wal-sfst-query.md) | Offline WAL/SFST OTel-log query contract (`sfsq-cli`): directory resolution, SFST/WAL discovery, SFST-wins dedup, stream filter, time window, and NDJSON output. | `sfsq-cli` work, offline forensic log inspection, `sfsq` engine consumers. |
| [otel-stream-identity.md](otel-stream-identity.md) | OTel log-stream identity contract: `ServiceStream`, absent==empty `service.namespace`, the canonical `ns_hash`, file partitioning, and the WAL/SFST candidate filters. | otel-ingestor stream extraction, `file-registry` hashing, WAL/SFST registry candidate filters, `sfsq` query library, query-planner stream filter, offline stream-query CLIs. |
