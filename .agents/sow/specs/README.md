...
| [otel-file-lifecycle.md](otel-file-lifecycle.md) | As-built OTel logs file lifecycle: WAL→SFST→catalog→remote→read-cache artifact/authority map, the I5 identity model (machine GUID + per-process instance_id, SeqKey vs bare-seq), ingest time bounds, rotation cadences + loss windows, own-machine LIST discipline (D6=B), the fail-closed P7 startup restore + D-P8.1 corrupt-catalog heal, and the restore/migration contract. | `file-lifecycle`/`otel-ledger`/`otel-ingestor` lifecycle work, restore/migration behavior, operator remote-storage docs. |
