...
- The OIDs `snmpTrapAddress.0` (`.1.3.6.1.6.3.18.1.3.0`) is checked — that's the SNMPv2c "where did this trap originate" varbind, used to handle proxied / forwarded traps where the UDP source IP is not the device IP.
