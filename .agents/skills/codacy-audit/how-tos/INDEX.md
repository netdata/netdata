...
- [handle-malformed-local-json](handle-malformed-local-json.md) -- handle `analyze-local.sh` dumps that have a `.json` suffix but contain Codacy tool-runner logs instead of parseable JSON, including runs where the wrapper still returns zero.
