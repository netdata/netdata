...
| projected `modules[].metrics.profile_coverage`                                 | object                        | generated only                 | exact semantic-view/runtime-chart parity                                                         | learn / www generated metrics catalogue              | `integrations/prometheus_profile_docs.py` adds this detached in-memory projection after schema validation. Projected charts contain only context, title, family hierarchy, units, dimensions, selectors, and entity scope; profile-design questions remain internal. Do not author it inside a module; YAML merge anchors otherwise share nested mappings between inherited modules. |
