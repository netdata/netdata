...
this file covers the integration-catalog side only.
...
- **Documentation projection:** `integrations/prometheus_profile_docs.py` joins each mapped profile's
  ...
  module
  after schema validation. Generation fails if a stock profile is unmapped or a semantic view and runtime chart differ.
  The rows are grouped by the profile's top-level family (Prometheus metric, Netdata family and chart title, dimension,
  unit, entity scope); a view's `question` stays internal. Do not author `metrics.profile_coverage` inside a module:
  YAML
  merge anchors would share the nested mapping between inherited modules.
...
2. Choose and record exactly one public-page disposition: add or update a Prometheus application module when no
   equivalent entry exists; update or cross-link the existing first-class integration when it already documents the
   same endpoint (never a duplicate catalog entry); or keep generic-only documentation, recording the concrete product
   reason.
