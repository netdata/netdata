...
| Retiring a collector integration completely | [retiring-collector-integration](retiring-collector-integration.md) | Source, runtime wiring, discovery, installed-stock cleanup, generated artifacts, taxonomy, replacement ownership, and Learn redirects. |
| Relating stock Prometheus profiles to integration metadata | [prometheus-profile-metadata](prometheus-profile-metadata.md) | Explains why profiles do not generate metadata, the catalog-aware disposition, operator-model brief, and regeneration checks. |
| Regenerating the NPM catalog (`gen_npm_catalog.py`) | [npm-catalog-generation](npm-catalog-generation.md) | The second-order generator chain (profiles -> `metadata.yaml` -> pages), its source-first CI ordering, the ruamel YAML-anchor sharing trap that made non-SNMP producers render SNMP setup, and how to verify a generator change did not disturb the other ~990 entries. |
