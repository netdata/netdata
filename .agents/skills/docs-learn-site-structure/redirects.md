...
2. `gate_legacy_redirects` classifies every catalogue entry: resolved (source is a published page), retained (an
   existing `static.toml` or tracked rule already covers the route; reported), retired (listed under
   `legacy_catalogue_retirements` in `config/redirect-policy.json`), or fatal. A fatal entry raises
   `LegacyRedirectGateError`, ingest exits 3, and nothing below is written. A catalogue value that is an absolute
   `http(s)` URL is treated as an external target (`_is_external_target`), never resolved through the map.
3. `append_entries_to_json` adds the moved entries to the catalogue; `clean_redirects` drops rules whose source is a
   live route and collapses chains; `write_netlify_config` writes the file.
...
- Delete: `docs/.map/README.md#unpublishing-files`. Repoint the catalogue value to the replacement page's GitHub blob
  ...
  404. Run ingest and commit the regenerated `netlify.toml` with the catalogue change; the gate rejects a catalogue
  target that conflicts with the committed rule. Re-publishing a deleted page later needs no further surgery: the
