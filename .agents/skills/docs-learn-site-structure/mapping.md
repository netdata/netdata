...
- `docs/.map/validate_map_schema.py` (hand-run from the repository root with an interpreter that has `ruamel.yaml`
...
`learn_status: Published`, plus one sentinel row per `integration_placeholder` node. `learn_rel_path` is the joined
labels of the ancestors that have `items` (`docs/.map/README.md#path-reconstruction`); `meta.path` replaces the
...
`learn_rel_path`, `sidebar_label`, `custom_edit_url` (case-folded), writing the expanded table to
...
agent notification). Logs integrations additionally get `learn_rel_path + "/" + sidebar_label`. Most splice blocks
index the sentinel unguarded: removing a placeholder from `map.yaml` for a kind that has pages raises `IndexError`
and aborts the run.

## The join key

`insert_and_read_hidden_metadata_from_doc` looks a source file up by the URL
`https://github.com/netdata/<repo>/edit/<branch>/<repo-relative-path>`, owner hard-coded to `netdata`, branch `main`
for `.github` and `master` otherwise (`default_repos`). Consequences:

- A source file with no matching row is skipped with no message; `--debug` prints the list at the end of the run.
...
`keywords`. `sanitize_page` later turns the first `<!--` and `-->` into `---` (`./mdx-rules.md`).
