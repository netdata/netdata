...
One structural superset fixture MAY combine optional capabilities that no
deployment enables simultaneously when the purpose is to materialize every
independent chart. Label that fixture as a synthetic union, not a captured
scrape.

Use separate fixtures or cases when combining surfaces would:
...
- Use the source-complete synthetic fixture for the authoritative validator
  `PASS` proving every authored chart/dimension can materialize and every
  source-defined series is routed.
