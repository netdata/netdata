...
# the same .env entries the triage-sonarqube skill uses. If they're missing,
# this script prints what's needed and exits 1.
#
# Sourcing strategy: this script is part of repo-pr-reviews, but it leans on
# the triage-sonarqube skill's `_lib.sh` helpers (sq_load_env, sq_paginate)
...
# shellcheck disable=SC1091
source "$(dirname "$0")/../../triage-sonarqube/scripts/_lib.sh"

PR="${1:?usage: $0 <pr-number>}"
pr_require_numeric "${PR}"

# sq_load_env reads <repo-root>/.env; same .env the repo-pr-reviews scripts use.
sq_load_env
