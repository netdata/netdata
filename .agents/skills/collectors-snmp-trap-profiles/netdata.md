...
**Reference**: `.agents/skills/collectors-snmp-trap-profiles/decisions/0001-go-process-and-trapwriter.md`
...
1. **Go process / writer backend** — **ACCEPTED (SOW-0035 M1, ADR-0001; amended through 2026-08-12 for SDK `go/v0.8.1`, Netdata log directory placement, raw-payload serialization, and internal output ownership)**: Standard in-process go.d collector V2 module with a thin SDK-backed Go journal adapter over `github.com/netdata/systemd-journal-sdk/go/journal` `go/v0.8.1`. No separate process, no CGo, no subprocess bridge. See `.agents/skills/collectors-snmp-trap-profiles/decisions/0001-go-process-and-trapwriter.md`.
...
| Configuration | `dyncfg` for runtime job config; profile changes apply after Agent restart or recreation of all trap jobs (no runtime MIB compilation) |
...
| 4. MIB management divergence is extreme | **Ship comprehensive vendor packs derived from public MIBs (803 stock vendor profiles / 6,121 MIB entries / 150,755 traps in the OOB pack) + offline conversion tooling for operator MIBs (no runtime compilation).** Targets the LibreNMS-to-Datadog coverage band with the SNMP polling plugin's stock+override pattern. Profile changes apply on Agent restart or after all trap jobs are recreated. |
...
| **SOW-0039** | **Collector consistency bundle**: `metadata.yaml` + `config_schema.json` + stock `.conf` + `health.d/snmp_traps.conf` + `README.md` (passes `check-markdown.yml`; the collector taxonomy is dormant and no longer a CI gate). **Embedded SNMP traps logs Function** in `go.d.plugin`, exposed as `snmp:traps`, with direct-journal jobs selected through `__logs_sources` and trap-specific default facets. **End-user AI skill `query-snmp-traps`** (`docs/netdata-ai/skills/query-snmp-traps/`) + how-tos catalog. **User documentation** for the offline MIB-to-YAML conversion workflow (per §7). **SOW-0032 research/comparison/comparative-analysis.md closeout**. **Final merge gate** — single PR sequence ending here. | Mergeable, CI-passing, documented |
...
**Status: Accepted (SOW-0035 M1, ADR-0001, reviewer round 5 findings folded in).** The Go interface definition and `TrapEntry` struct are recorded in `.agents/skills/collectors-snmp-trap-profiles/decisions/0001-go-process-and-trapwriter.md` §3-4. Key design decisions:
...
The interface contract is defined formally in `.agents/skills/collectors-snmp-trap-profiles/decisions/0001-go-process-and-trapwriter.md` §3-4 (SOW-0035 M1, ADR-0001). The SOW-0038 M3 OTLP exporter implements the same interface as the SOW-0035 M4 journal writer.
