...
- This skill: the content of every collector `metadata.yaml` field: what it answers, in what order, in what shape,
  ...
  sources and never edit the generated file: `module.yaml` carries the product identity (name, link, icon,
  categories) and the overview `description`; `contexts.yaml` carries the metric rows and scope names. Everything
  else on an ibm.d page (method, permissions, default behavior, keywords, the option rows, prerequisites, examples,
  troubleshooting) is hardcoded in docgen's metadata template (`src/go/plugin/ibm.d/docgen/main.go`), so applying
  these rules to an ibm.d module means changing that template (`.agents/skills/integrations-lifecycle/ibm-d.md`).
  `config.go` drives `config_schema.json` and the README, not the metadata options table.
