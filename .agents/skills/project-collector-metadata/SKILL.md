...
- This skill: the content of every collector `metadata.yaml` field: what it answers, in what order, in what shape,
  for whom. Rules are cross-plugin. ibm.d metadata is generated by `docgen`, so ibm.d authors apply the rules to its
  sources and never edit the generated file: `module.yaml` for the overview and identity families, `contexts.yaml`
  for metrics, `config.go` for option rows; prerequisites, examples, and troubleshooting are fixed by docgen's template
  today (`.agents/skills/integrations-lifecycle/ibm-d.md`), so changing them means changing the template.
...
- The template owns the heading hierarchy (h1 title; h2 Overview, Setup, Metrics, Alerts, Troubleshooting; h3 to h6
