---
name: project-collector-metadata
description: Content rules for collector metadata.yaml, field by field, for every collector family (go.d, C plugins, scripts.d, Rust plugins, ibm.d via its module.yaml source). Use when writing or reviewing a collector's metadata.yaml; when asked what goes on the integration page, in the overview, metrics description, method description, auto-detection, limits, performance impact, permissions, prerequisites, the config options table, examples, troubleshooting, metrics scopes, or alerts; when a generated integration page reads as a wall of text or says something false about defaults or discovery. Not for the generator pipeline, JSON schemas, taxonomy, or generated outputs (integrations-lifecycle), and not for config_schema.json (project-go-collector-design/config-schema.md).
---
...
- This skill: the content of every collector `metadata.yaml` field: what it answers, in what order, in what shape,
  for whom. Rules are cross-plugin; ibm.d authors apply them to the `module.yaml` source fields, never to the
...
- `.agents/skills/project-go-collector-design/config-schema.md`: the DynCfg form. Option wording shared between the
  form and the options table has one owner there (sections 7 and 8); this skill points to it.
- `.agents/skills/project-go-collector-design/operator-surface.md`: which options exist and what an operator decides.
...
- The template owns the heading hierarchy (h1 title, h2 Overview, Setup, Metrics, Alerts, Troubleshooting; h3 and h4
  inside them). Field text is inserted under those headings as-is.
...
| Alerts and meta | `alerts`, `meta.monitored_instance`, categories, keywords, icon, `related_resources`, `info_provided_to_referring_integrations` | `alerts-and-meta.md` |
...
- Learn's ingest escapes only bare `{`, the operators `<=`, `%<`, `<->`, and `<details><summary>`; nothing in this
  repository validates the rest. The failure appears on the next Learn deploy preview, not in this repo's CI.
...
1. Regenerate and read the page: `python3 integrations/gen_integrations.py`, then open the collector's file under
   `integrations/`. Read it as the operator, top to bottom, once.
...
7. Markdown safety items above; then the pipeline validation in `integrations-lifecycle/description-authoring.md`
