...
| Stock contribution policy for `autogen.selector` and the relabel grammar (allowlists, exact denies, wildcard drop and rewrite forms, name provenance, identity preservation) | `src/go/plugin/go.d/collector/prometheus/profile-format.md#autogenselector` ("Stock contribution policy"); every rule has a finding code in `src/go/tools/prometheus-profile-validation/README.md#what-pass-establishes` |
| Chart template: groups, `chart_defaults`, charts, `instances.by_labels` and `optional_by_labels`, dimensions and naming modes, `label_promotion`, `aggregation`, `lifecycle`, `options`, selectors, the validation rules, engine-derived behavior (ID derivation, per-dimension algorithm, priority `<= 0` to `70000`, family composition, multiplier and divisor `0` to `1`) | `src/go/plugin/framework/charttpl/README.md#field-reference`, `src/go/plugin/framework/charttpl/README.md#validation-rules`, `src/go/plugin/framework/charttpl/README.md#engine-derived-behavior` (the design roles and consequences of `label_promotion` and `optional_by_labels` are in `chart-design.md`) |
| Relabel actions, the ordered stages, histogram and summary safety, profile precedence on the shared stream | `src/go/pkg/relabel/README.md` |
