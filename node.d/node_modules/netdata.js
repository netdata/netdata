...
var netdata = {
	...
	service: function(service) {
		...
		service._processResponse = function(response, callback) {
			this.running = false;
			this.ended = new Date().getTime();

			if(netdata.options.DEBUG === true) netdata.debug(this.module.name + ': ' + this.name + ': processing response (received it in ' + (this.ended - this.started).toString() + ' ms)');

			callback(this, response);

			this.module.active--;
			if(this.module.active < 0) {
				this.module.active = 0;
				if(netdata.options.DEBUG === true) netdata.debug(this.module.name + ': active module counter below zero.');
			}

			if(this.module.active === 0) {
				// check if we run under configure
				if(this.module.configure_callback !== null) {
					if(netdata.options.DEBUG === true) netdata.debug(this.module.name + ': configuration finish callback called from processResponse().');
					var ccallback = this.module.configure_callback;
					this.module.configure_callback = null;
					ccallback();
				}
			}
		};
		...
		service.execute = function(callback) {
			...
			if(netdata.options.DEBUG === true) netdata.debug(this.module.name + ': ' + this.name + ': making request: ' + netdata.stringify(this.request));

			var req = http.request(this.request, function(response) {
				...
				if(response.statusCode !== 200) {
					if(end === false) {
						...
						service._processResponse(null, callback);
					}
				}
				...
				response.on('end', function() {
					if(end === false) {
						...
						service._processResponse(data, callback);
					}
				});
			});
			...
			if(typeof this.postData !== 'undefined' && this.request.method === 'POST') {
				if(netdata.options.DEBUG === true) netdata.debug(this.module.name + ': ' + this.name + ': posting data: ' + this.postData);
