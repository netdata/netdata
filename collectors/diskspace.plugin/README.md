...
Netdata will try to detect mounts that are duplicates (i.e. from the same device), or binds, and will not display charts for them since most likely the device is already monitored. If you find that some devices are missing, switch `ignore duplicate devices and binds` to `no`.
...
[plugin:proc:diskspace]
    ...
    # ignore duplicate devices and binds = yes
