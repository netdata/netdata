...
The SNMP plugin supports Counter64 metrics with the only limitation that the `offset` parameter should not be defined. Due to the way Javascript handles large numbers and the fact that the offset is applied to metrics inside the plugin, defining an offset will erroneously add a 0 to the end of the hex string, effectively mutliplying the metric by 16.
</details> 
<br>
