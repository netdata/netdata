...
#define APPS_PLUGIN_FUNCTIONS() do { \
        fprintf(stdout, PLUGINSD_KEYWORD_FUNCTION " \"processes\" %d \"%s\" \"top\" \"0x%"PRIx64"\" %d\n", \
                PLUGINS_FUNCTIONS_TIMEOUT_DEFAULT, APPS_PLUGIN_PROCESSES_FUNCTION_DESCRIPTION,         \
                (uint64_t)(HTTP_ACCESS_SIGNED_IN|HTTP_ACCESS_VIEW_SENSITIVE_DATA), RRDFUNCTIONS_PRIORITY_DEFAULT / 10); \
    } while(0)

#define APPS_PLUGIN_GLOBAL_FUNCTIONS() do { \
        fprintf(stdout, PLUGINSD_KEYWORD_FUNCTION " GLOBAL \"processes\" %d \"%s\" \"top\" \"0x%"PRIx64"\" %d\n", \
                PLUGINS_FUNCTIONS_TIMEOUT_DEFAULT, APPS_PLUGIN_PROCESSES_FUNCTION_DESCRIPTION,                \
                (uint64_t)(HTTP_ACCESS_SIGNED_IN|HTTP_ACCESS_VIEW_SENSITIVE_DATA), RRDFUNCTIONS_PRIORITY_DEFAULT / 10); \
    } while(0)
