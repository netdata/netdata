...
nvme_get() {
  ...
  for i in $nvme_list;do
    ...
    critical_warning=$critical_warning"SET $h = $(sudo -n nvme smart-log $i | awk -F: '/critical_warning/ {gsub(/ /,""); print $2 }')"$'\n'
	
    # read the percentage used in the nvme	
    percentage_used=$percentage_used"SET $h = $(sudo -n nvme smart-log $i | awk -F: '/percentage_used/ {gsub(/ /,""); print $2}' | awk -F% '{print $1}')"$'\n'
	
    # read the temperature in the nvme
    temperature=$temperature"SET $h = $(sudo -n nvme smart-log $i | awk -F: '/temperature/ { print $2 }' | awk '{ print $1 }')"$'\n'
	
    # read the power cycles in the nvme	
    power_cycles=$power_cycles"SET $h = $(sudo -n nvme smart-log $i | awk -F: '/power_cycles/ {gsub(/ /,""); print $2 }')"$'\n'
	
    # read the power on hours in the nvme
    power_on_hours=$power_on_hours"SET $h = $(sudo -n nvme smart-log $i | awk -F: '/power_on_hours/ { print $2 }' | awk -F, '{gsub(/ /,""); print $1 }')"$'\n'
