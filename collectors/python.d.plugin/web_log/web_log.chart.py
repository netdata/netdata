...
class Web:
    ...
    def check(self):
        raw_data = read_last_lines(self.log_path)
        last_lines = filter_data(raw_data=raw_data, pre_filter=self.pre_filter)
        last_line = next(last_lines)
        if not last_line:
            return False
...
# Snipped from https://stackoverflow.com/questions/2301789/how-to-read-a-file-in-reverse-order
def read_last_lines(filename, buf_size=8192):
