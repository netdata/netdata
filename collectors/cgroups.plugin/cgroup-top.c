...
int cgroup_function_cgroup_top(BUFFER *wb, int timeout __maybe_unused, const char *function __maybe_unused,
    ...
    double max_cpu = 0.0;
    double max_received = 0.0;
    double max_sent = 0.0;
    double max_ram = 0.0;

    for(struct cgroup *cg = cgroup_root; cg ; cg = cg->next) {
        if(unlikely(!cg->enabled || cg->pending_renames || !cg->st_cpu_rd_user || !cg->st_cpu_rd_system))
            continue;
    ...
    buffer_json_member_add_object(wb, "columns");
    {
        ...
        // CPU
        buffer_rrdf_table_add_field(wb, field_id++, "CPU", "CGROUP CPU",
        ...
        // RAM
        buffer_rrdf_table_add_field(wb, field_id++, "RAM", "CGROUP RAM",
                RRDF_FIELD_TYPE_BAR_WITH_INTEGER, RRDF_FIELD_VISUAL_BAR, RRDF_FIELD_TRANSFORM_NUMBER,
        ...
        // Received
        buffer_rrdf_table_add_field(wb, field_id++, "Received", "CGROUP Received Bandwidth",
                RRDF_FIELD_TYPE_BAR_WITH_INTEGER, RRDF_FIELD_VISUAL_BAR, RRDF_FIELD_TRANSFORM_NUMBER,
                2, "kbps", max_received, RRDF_FIELD_SORT_DESCENDING, NULL,
                RRDF_FIELD_SUMMARY_SUM, RRDF_FIELD_FILTER_RANGE,
                RRDF_FIELD_OPTS_VISIBLE,
                NULL);

        // Sent
        buffer_rrdf_table_add_field(wb, field_id++, "Sent", "CGROUP Received Bandwidth",
                RRDF_FIELD_TYPE_BAR_WITH_INTEGER, RRDF_FIELD_VISUAL_BAR, RRDF_FIELD_TRANSFORM_NUMBER,
                2, "kbps", max_sent, RRDF_FIELD_SORT_DESCENDING, NULL,
                RRDF_FIELD_SUMMARY_SUM, RRDF_FIELD_FILTER_RANGE,
                RRDF_FIELD_OPTS_VISIBLE,
                NULL);
    }
