...
case "${BUILDARCH}" in
    armv6l)
        ...
        export INSTALLER_ARGS="--disable-plugin-systemd-journal --disable-plugin-otel-metrics"
        ;;
    *)
        export NETDATA_CMAKE_OPTIONS="-DENABLE_LIBBACKTRACE=On"
        export INSTALLER_ARGS="--enable-plugin-systemd-journal --internal-systemd-journal --enable-plugin-otel-metrics"
        ;;
esac
