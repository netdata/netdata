...
Homebrew installs Netdata but does not start the Agent automatically, so the local dashboard at `http://localhost:19999` is not available until you start it yourself. Start the Agent as a background service:

```sh
brew services start netdata
```

This starts the Agent now and registers it to launch at login. To check that it is running:

```sh
brew services info netdata
```

If you change your Netdata configuration, restart the Agent to apply it:
