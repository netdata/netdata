...
detect_existing_install() {
  ...
  while [ -n "${searchpath}" ]; do
    ...
    if [ -z "${ndpath}" ]; then
      ...
      fatal "Multiple installs of Netdata agent detected (located at '${ndpath}' and '${_ndpath}'). Such a setup is not generally supported. If you are certain you want to operate on one of them despite this, use the '--install-prefix' option to specifiy the install you want to operate on." F0516
    fi
