...
prepare_cmake_options() {
  ...
  if [ -n "${NETDATA_ENABLE_LTO}" ]; then
    if [ "${NETDATA_ENABLE_LTO}" -eq 1 ]; then
      NETDATA_CMAKE_OPTIONS="${NETDATA_CMAKE_OPTIONS} -DCMAKE_INTERPROCEDURAL_OPTIMIZATION=On"
    else
      NETDATA_CMAKE_OPTIONS="${NETDATA_CMAKE_OPTIONS} -DCMAKE_INTERPROCEDURAL_OPTIMIZATION=Off"
    fi
  fi
