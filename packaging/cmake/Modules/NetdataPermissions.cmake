...
if(CREATE_USER)
  if(OS_LINUX)
    message(CHECK_START "Looking for systemd-sysusers")
    find_program(SYSUSERS_CMD systemd-sysusers NO_CMAKE_INSTALL_PREFIX)
    if(SYSUSERS_CMD STREQUAL "SYSUSERS_CMD-NOTFOUND")
      ...
      message(CHECK_PASS "${SYSUSERS_CMD}")
    endif()
  elseif(OS_FREEBSD)
    message(CHECK_START "Looking for getent")
    find_program(GETENT_CMD getent NO_CMAKE_INSTALL_PREFIX)
...
function(_nd_perms_mark_path_filecaps var path capset)
  set(tmp_var "${${var}}chown -f root:$NETDATA_GROUP '${path}'\n")
  set(tmp_var "${tmp_var}chmod -f 0750 '${path}'\n")
  set(tmp_var "${tmp_var}if ! setcap '${capset}' '${path}' 2>/dev/null; then\n")

  if(capset MATCHES "perfmon")
    string(REPLACE "perfmon" "sys_admin" capset2 "${capset}")
    set(tmp_var "${tmp_var}    if ! setcap '${capset2}' '${path}' 2>/dev/null; then\n")
    set(tmp_var "${tmp_var}        chmod -f 4750 '${path}'\n")
...
function(nd_perms_generate_cmake_install_hook path component perms)
  ...
  if(perms STREQUAL "restrict" OR PACKAGE_TYPE STREQUAL "docker")
    _nd_add_install_hook("${CMAKE_SOURCE_DIR}/packaging/cmake/install-perms-hook.sh \$ENV{DESTDIR}/\${CMAKE_INSTALL_PREFIX}/${path} ${NETDATA_GROUP} 0750" "${component}")
  elseif(USE_FILE_CAPABILITIES AND NOT "${perms}" STREQUAL "suid")
    list(TRANSFORM perms PREPEND "cap_")
    list(JOIN perms " " caps)

    _nd_add_install_hook("${CMAKE_SOURCE_DIR}/packaging/cmake/install-linux-caps-hook.sh \$ENV{DESTDIR}/\${CMAKE_INSTALL_PREFIX}/${path} ${NETDATA_GROUP} ${caps}" "${component}")
  else()
    _nd_add_install_hook("${CMAKE_SOURCE_DIR}/packaging/cmake/install-perms-hook.sh \$ENV{DESTDIR}/\${CMAKE_INSTALL_PREFIX}/${path} ${NETDATA_GROUP} 4750" "${component}")
  endif()
endfunction()
...
function(netdata_install_extra_permissions)
  if(OS_WINDOWS OR PACKAGE_TYPE STREQUAL "windows")
    return()
  endif()
