...
REGISTRY_MACHINE_URL *registry_machine_url_allocate(REGISTRY_MACHINE *m, STRING *u, time_t when) {
    netdata_log_debug(D_REGISTRY,
                      "REGISTRY: allocating machine URL (machine_guid_bytes=%zu, url_bytes=%zu, object_bytes=%zu)",
                      strlen(m->guid), string_strlen(u), sizeof(REGISTRY_MACHINE_URL));
    ...
    netdata_log_debug(D_REGISTRY, "REGISTRY: indexing machine URL (machine_guid_bytes=%zu, url_bytes=%zu)",
                      strlen(m->guid), string_strlen(u));
...
REGISTRY_MACHINE *registry_machine_allocate(const char *machine_guid, time_t when) {
    netdata_log_debug(D_REGISTRY, "REGISTRY: creating machine (guid_bytes=%zu, object_bytes=%zu)",
                      strlen(machine_guid), sizeof(REGISTRY_MACHINE));
...
REGISTRY_MACHINE_URL *registry_machine_link_to_url(REGISTRY_MACHINE *m, STRING *url, time_t when) {
    netdata_log_debug(D_REGISTRY, "REGISTRY: searching for machine URL (machine_guid_bytes=%zu, url_bytes=%zu)",
                      strlen(m->guid), string_strlen(url));

    REGISTRY_MACHINE_URL *mu = registry_machine_url_find(m, url);
    if(!mu) {
        netdata_log_debug(D_REGISTRY, "REGISTRY: machine URL not found (machine_guid_bytes=%zu, url_bytes=%zu)",
                          strlen(m->guid), string_strlen(url));
        mu = registry_machine_url_allocate(m, url, when);
        registry.machines_urls_count++;
    }
    else {
        netdata_log_debug(D_REGISTRY, "REGISTRY: machine URL found (machine_guid_bytes=%zu, url_bytes=%zu)",
                          strlen(m->guid), string_strlen(url));
        mu->usages++;
    ...
    if(mu->flags & REGISTRY_URL_FLAGS_EXPIRED) {
        netdata_log_debug(D_REGISTRY,
                          "REGISTRY: re-enabling expired machine URL (machine_guid_bytes=%zu, url_bytes=%zu)",
                          strlen(m->guid), string_strlen(url));
