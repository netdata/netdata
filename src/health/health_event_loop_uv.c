...
static void health_cleanup_log(struct health_event_loop_config *config) {
    ...
    if (!config->next_cleanup_time)
        config->next_cleanup_time = nd_time_t_add_saturating(now, HEALTH_CLEANUP_FIRST_RUN_DELAY);

    // Check if it's time to run cleanup, and not already running
    if (now < config->next_cleanup_time || config->cleanup_running)
        return;

    config->next_cleanup_time = nd_time_t_add_saturating(now, HEALTH_CLEANUP_INTERVAL);
...
static inline bool check_if_resumed_from_suspension(struct health_event_loop_config *config) {
    ...
    if (config->last_realtime && config->last_monotonic &&
        realtime > config->last_realtime && monotonic >= config->last_monotonic) {
        usec_t realtime_delta = realtime - config->last_realtime;
        usec_t monotonic_delta = monotonic - config->last_monotonic;
...
static void health_batch_work_cb(uv_work_t *req) {
    ...
    for (;;) {
        ...
        time_t host_next_run =
            nd_time_t_add_saturating(config->batch.now, health_globals.config.run_at_least_every_seconds);
