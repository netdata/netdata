...
valid on its own**. If any of them is missing `on`, is missing its update frequency, has no
`lookup`/`calc`/`warn`/`crit`, or has a non-finite delay multiplier, the whole set is rejected and
the alert is not created at all — the daemon log names the offending rule. This matters for the
common per-OS pattern, where one alert name carries one rule per `host labels: _os=...` value.
