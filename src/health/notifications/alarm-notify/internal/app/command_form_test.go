	...
	for provider := range map[string]struct{}{"prowl": {}, "kavenegar": {}} {
		for name, test := range map[string]struct{ source, status string }{
			...
			t.Run(provider+"/"+name, func(t *testing.T) {
				...
				unused["api_key"] = "${env:UNSET_UNUSED_FORM_KEY}"
				...
				assert.NotContains(t, stderr.String(), formTestDestination(provider)["api_key"].(string))
			})
		}
	}
}
	...
	for provider := range map[string]struct{}{"prowl": {}, "kavenegar": {}} {
		for name, test := range map[string]struct {
			...
			t.Run(provider+"/"+name, func(t *testing.T) {
				...
				assert.NotContains(t, stderr.String(), dst["api_key"].(string))
				assert.NotContains(t, stderr.String(), server.URL)
			})
		}
	}
		...
		for name, test := range map[string]struct{ field, value, err string }{
			...
			t.Run(provider+"/"+name, func(t *testing.T) {
				...
				if test.field == "api_key" {
					dst["api_key"] = "${env:FORM_INVALID_SECRET}"
				} else {
					dst["api_url"] = "${env:FORM_INVALID_SECRET}"
...
func TestRunProwlPayloadLimit(t *testing.T) {
	...
	cfg := testConfig{
		Version:      1,
		Destinations: map[string]map[string]any{"prowl": dst, "webhook": {"type": "webhook", "url": server.URL + "/webhook"}},
		Routing:      notifier.Routing{Roles: map[string][]string{"ops": {"prowl", "webhook"}}},
	}
	data, err := yaml.Marshal(cfg)
	require.NoError(t, err)
	event := testutil.ExpectedEvent()
