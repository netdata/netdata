			...
			for status := range map[string]struct{}{"WARNING": {}, "CRITICAL": {}, "CLEAR": {}} {
				t.Run(provider+"/"+source+"/"+status, func(t *testing.T) {
					...
					if provider == "matrix" {
						...
						fields = map[string]string{"url": "api_url", "token": "access_token"}
					} else {
					...
					for name, value := range fields {
						...
						case "file":
							...
							require.NoError(t, os.WriteFile(path, []byte(dst[value].(string)+"\n"), 0600))
							dst[value] = "${file:" + path + "}"
						}
					}
					second, unused := maps.Clone(dst), maps.Clone(dst)
					if provider == "matrix" {
						second["room_id"] = "!second:example.org"
						second["access_token"] = "second-synthetic-token"
...
func TestRunMatrixRoomEscaping(t *testing.T) {
	for room, escaped := range map[string]string{"!opaque/hash+id": "%21opaque%2Fhash+id", "!room/?#%:example.org": "%21room%2F%3F%23%25:example.org", "!κ:example.org": "%21%CE%BA:example.org"} {
		t.Run(room, func(t *testing.T) {
			...
			dst["api_url"], dst["room_id"] = server.URL, room
			config, err := yaml.Marshal(testConfig{Version: 1, Destinations: map[string]map[string]any{"target": dst}})
			require.NoError(t, err)
			var stdout, stderr bytes.Buffer
			require.Zero(
				t,
	...
	for name, test := range map[string]struct{ provider, field, value, err string }{
		...
		t.Run(name, func(t *testing.T) {
			...
			if test.provider == "msteams" {
				...
				dst["api_url"] = "${env:CHAT_INVALID_SECRET}"
				dst["access_token"] = "${file:" + filepath.Join(t.TempDir(), "absent") + "}"
			} else {
				dst["api_url"] = server.URL
				dst["access_token"] = "${env:CHAT_INVALID_SECRET}"
			}
			config, err := yaml.Marshal(testConfig{Version: 1, Destinations: map[string]map[string]any{"target": dst}})
			require.NoError(t, err)
			var stdout, stderr bytes.Buffer
