...
func TestSendTwilio(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			defer server.Close()
			dst := map[string]any{"type": "twilio", "account_sid": twilioTestSID, "auth_token": "synthetic-private-value", "from": "+15005550006", "to": "+15005550009", "api_url": server.URL + "/proxy/"}
			if test.secret != "" {
				sid, token, api := dst["account_sid"].(string), dst["auth_token"].(string), dst["api_url"].(string)
				switch test.secret {
				case "bad-sid":
					sid += "/../secret"
				case "bad-token":
				...
				for _, field := range []struct {
					...
					{"SID", sid, "account_sid"}, {"TOKEN", token, "auth_token"}, {"API", api, "api_url"},
				} {
					if test.secret == "file" {
			...
			cfg := testConfig{Version: 1, Destinations: map[string]map[string]any{
				...
				"unused":     {"type": "twilio", "account_sid": "${env:NOTIFIER_UNUSED_SID}", "auth_token": "${env:NOTIFIER_UNUSED_TOKEN}", "from": "12345", "to": "+15005550009"},
			}, Routing: notifier.Routing{Roles: map[string][]string{"ops": {"sms", "slack", "discord", "telegram", "pushover", "pushbullet", "archive"}, "dba": {"sms"}}}}
			if test.multiple {
