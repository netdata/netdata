...
func snsHelperDestination(t *testing.T, mode string) (map[string]any, string) {
	t.Helper()
	dst := snsDestination(t)
	dir := t.TempDir()
	path := filepath.Join(dir, "aws"+mode)
	require.NoError(t, os.Symlink(dst["executable"].(string), path))
	dst["executable"] = path
	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			case "subject":
				event.Node += "\n"
			case "message":
				dst["message_template"] = "{{info}}"
				event.Info = ""
			case "missing":
				dst["executable"] = bad["executable"]
			}
			cfg := testConfig{Version: 1, Destinations: map[string]map[string]any{"target": dst, "bad": bad}, Routing: notifier.Routing{Roles: map[string][]string{"nobody": {}, "mixed": {"bad", "target", "target"}}}}
...
func TestRunSNSCancellation(t *testing.T) {
	for name, test := range map[string]struct{ deadline bool }{"cancel": {}, "deadline": {true}} {
		t.Run(name, func(t *testing.T) {
			...
			require.NoError(t, os.WriteFile(filepath.Join(filepath.Dir(dst["executable"].(string)), "control-address"), []byte(listener.Addr().String()), 0600))
			cfg := testConfig{Version: 1, Destinations: map[string]map[string]any{"target": dst}}
			timeout := "10s"
			if test.deadline {
				timeout = "2s"
...
func snsDestination(t *testing.T) map[string]any {
	t.Helper()
