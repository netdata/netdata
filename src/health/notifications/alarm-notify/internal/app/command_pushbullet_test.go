...
func TestSendPushbullet(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			defer server.Close()
			dst := map[string]any{"type": "pushbullet", "access_token": "synthetic-private-value", "email": "ops@example.com",
				"source_device_id": "test-source-device", "api_url": server.URL + "/proxy/"}
			if test.secret != "" {
				token, api := dst["access_token"].(string), dst["api_url"].(string)
				switch test.secret {
				case "bad-token":
					token += "\nextra"
				case "empty-token":
				...
				for _, field := range []struct {
					...
					{"TOKEN", token, "access_token"}, {"API", api, "api_url"},
				} {
					if test.secret == "file" {
			...
			other["email"], other["channel_tag"], other["source_device_id"] = "", "test-alerts", ""
			...
			cfg := testConfig{Version: 1, Destinations: map[string]map[string]any{
				...
				"unused": {
					"type":         "pushbullet",
					"access_token": "${env:NOTIFIER_UNUSED_TOKEN}",
					"email":        "unused@example.com",
				},
			}, Routing: notifier.Routing{Roles: map[string][]string{"ops": {"push", "slack", "discord", "telegram", "pushover", "archive"}, "dba": {"push"}}}}
			if test.channel {
