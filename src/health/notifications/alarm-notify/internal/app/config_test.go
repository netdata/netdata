...
func TestReadConfig(t *testing.T) {
	base := testutil.Document[webhook.Config]{
		Version: 1,
		Destinations: map[string]webhook.Config{
			"dev": {URL: "https://example.com/notify"},
		},
	}
	routed := base
	routed.Routing = notifier.Routing{
		Default: []string{"dev"},
		Roles:   map[string][]string{"sysadmin": {"dev"}, "muted": {}},
	}
	tests := map[string]struct {
		input string
		want  testutil.Document[webhook.Config]
		err   string
	}{
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			got, err := readWebhookConfig(strings.NewReader(test.input))
			if test.err != "" {
...
func TestConfigSecretReferences(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			input := "version: 1\ndestinations:\n  dev:\n    type: webhook\n    url: '" + test.value + "'\n"
			got, err := readWebhookConfig(strings.NewReader(input))
			if !test.valid {
				require.Error(t, err)
				return
			}
			require.NoError(t, err)
			assert.Equal(t, testutil.Document[webhook.Config]{Version: 1, Destinations: map[string]webhook.Config{
				"dev": {URL: test.value},
			}}, got)
		})
	}
...
func readWebhookConfig(r io.Reader) (testutil.Document[webhook.Config], error) {
	return testutil.ReadConfig(r, "webhook", func(c webhook.Config) error { _, err := webhook.New(c, http.DefaultClient); return err })
}
