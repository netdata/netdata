			...
			for name, test := range map[string]struct {
				...
				t.Run(name, func(t *testing.T) {
					...
					gotify := map[string]any{
						"type":      "gotify",
						"api_url":   server.URL + "/gotify/",
						"app_token": "synthetic-private-value",
					}
					ntfy := map[string]any{
						"type":         "ntfy",
					...
					cfg := testConfig{Version: 1, Destinations: map[string]map[string]any{
						...
						"unused-gotify": {
							"type":      "gotify",
							"api_url":   "${env:UNUSED_GOTIFY_URL}",
							"app_token": "${env:UNUSED_GOTIFY_TOKEN}",
						},
						"unused-ntfy": {
							"type":         "ntfy",
							"url":          "${env:UNUSED_NTFY_URL}",
							"access_token": "${env:UNUSED_NTFY_TOKEN}",
						},
					}, Routing: notifier.Routing{Roles: map[string][]string{"ops": {"gotify", "ntfy", "archive"}, "dba": {"gotify", "ntfy"}}}}
					if test.secret != "" {
						dst := cfg.Destinations[provider]
						fields := map[string]string{"TOKEN": "app_token", "URL": "api_url"}
						if provider == "ntfy" {
							fields = map[string]string{"TOKEN": "access_token", "URL": "url"}
							if test.auth == "basic" {
					...
					for _, name := range names {
						...
						if name == "gotify" {
							...
							payload, err := os.ReadFile(fixturePath("gotify-full.json"))
							require.NoError(t, err)
							if test.content {
						...
						if name == "ntfy" {
							...
							payload, err := os.ReadFile(fixturePath("ntfy-full.json"))
							require.NoError(t, err)
							var fixture struct {
								Headers http.Header
								Body    string
