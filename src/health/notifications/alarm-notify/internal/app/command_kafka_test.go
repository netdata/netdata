...
func TestRunKafka(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			defer server.Close()
			dst := map[string]any{"type": "kafka", "url": server.URL + "/kafka/synthetic-private-value?key=a%26b&key=second", "sender_ip": "192.0.2.1"}
			...
			cfg := testConfig{Version: 1, Destinations: map[string]map[string]any{
				...
				"unused":  {"type": "kafka", "url": "${env:UNUSED_KAFKA_URL}", "sender_ip": "192.0.2.2"},
			}, Routing: notifier.Routing{Roles: map[string][]string{"ops": {"bridge", "archive"}, "dba": {"bridge"}}}}
			if test.second {
				cfg.Destinations["other"] = map[string]any{"type": "kafka", "url": server.URL + "/other", "sender_ip": "2001:db8::1"}
				cfg.Routing.Roles["dba"] = append(cfg.Routing.Roles["dba"], "other")
			}
			config, err := yaml.Marshal(cfg)
			require.NoError(t, err)
			input := strings.Replace(testutil.ValidEvent, `"version": 1`, `"version": 1, "duration": 0, "non_clear_duration": 123`, 1)
...
func TestRunKafkaCancellation(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			var stdout, stderr bytes.Buffer
			assert.Equal(t, 1, Run(ctx, args, strings.NewReader(testutil.ValidEvent), &stdout, &stderr))
