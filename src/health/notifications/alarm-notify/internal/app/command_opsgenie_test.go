	...
	for source := range map[string]struct{}{"literal": {}, "env": {}, "file": {}} {
		for status, previous := range map[string]string{"WARNING": "CLEAR", "CRITICAL": "WARNING", "CLEAR": "CRITICAL"} {
			t.Run(source+"/"+status, func(t *testing.T) {
				...
				second["api_key"] = "second-synthetic-key"
				...
				for _, key := range []string{opsgenieTestKey, second["api_key"].(string)} {
					assert.Equal(
						t,
						request{
							"POST",
		...
		for name, test := range map[string]struct {
			...
			t.Run(status+"/"+name, func(t *testing.T) {
				...
				code := Run(
					context.Background(),
					args,
					strings.NewReader(strings.ReplaceAll(testutil.ValidEvent, "WARNING", status)),
					&stdout,
					&stderr,
				)
				if test.withSuccess {
...
func TestRunOpsgenieSecretsAndLimits(t *testing.T) {
	for name, test := range map[string]struct{ field, value, status, err string }{
		...
		t.Run(name, func(t *testing.T) {
			...
			switch test.field {
			case "api_key":
				dst["api_key"] = "${env:OPSGENIE_INVALID_SECRET}"
			case "api_url":
				dst["api_url"] = "${env:OPSGENIE_INVALID_SECRET}"
				dst["api_key"] = "${file:" + filepath.Join(t.TempDir(), "unread-key") + "}"
			case "info":
