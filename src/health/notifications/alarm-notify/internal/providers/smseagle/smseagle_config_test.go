	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if test.err != "" {
				...
				require.NoError(t, err)
				var want testutil.Document[Config]
			...
			for name, test := range map[string]struct {
				...
				t.Run(mode+"/"+field+"/"+name, func(t *testing.T) {
					...
					if allowed && test.positive {
						require.NoError(t, err)
						want := smseagleTestConfig()
						want.Recipients, want.MessageType = []string{"15005550009"}, mode
						n := configfield.Integer(7)
						if field == "call_duration" {
							want.CallDuration = &n
						} else {
							want.VoiceID = &n
						}
						assert.Equal(t, testutil.Document[Config]{Version: 1, Destinations: map[string]Config{"target": want}}, got)
					} else {
						require.Error(t, err)
						assert.Equal(t, testutil.Document[Config]{}, got)
					}
