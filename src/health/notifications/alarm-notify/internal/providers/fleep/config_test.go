...
func TestChatWebhookConfiguration(t *testing.T) {
	...
	for name, test := range map[string]struct {
		...
		"Fleep default sender": {dst: Config{URL: "https://example.com/hook"}},
		"Fleep sender":         {dst: Config{URL: file, Sender: "Netdata's \"警告\" bot"}},
		"relative URL":         {dst: Config{URL: "/hook"}, err: "absolute HTTP(S)"},
		"relative file":        {dst: Config{URL: "${file:relative}"}, err: "absolute path"},
		"blank sender":         {dst: Config{URL: file, Sender: " \t"}, err: "literal name"},
		"sender controls":      {dst: Config{URL: file, Sender: "bot\n"}, err: "literal name"},
		"sender reference":     {dst: Config{URL: file, Sender: "${env:SENDER}"}, err: "literal name"},
	} {
