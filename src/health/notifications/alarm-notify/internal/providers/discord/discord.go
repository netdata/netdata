...
func sendDiscord(ctx context.Context, dst Config, event notifyevent.Event, client *http.Client) error {
...
func postJSON(ctx context.Context, client *http.Client, dst Config, message any) error {
	...
	headers := http.Header{}
	response, err := httpclient.PostJSON(ctx, client, "discord", endpoint, headers, message)
	if err != nil {
		return err
	}
...
func (s *Sender) Send(ctx context.Context, event notifyevent.Event) error {
	return sendDiscord(ctx, s.config, event, s.client)
}
