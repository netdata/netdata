...
func pagerDutyTestConfig(version int64) Config {
	n := configfield.Integer(version)
	return Config{IntegrationKey: pagerDutyTestKey, APIVersion: &n}
}
...
func TestRenderPagerDuty(t *testing.T) {
	...
	for version := range map[int64]struct{}{1: {}, 2: {}} {
		for name, test := range map[string]struct{ variant, status, previous string }{
			...
			t.Run(fmt.Sprintf("v%d/%s", version, name), func(t *testing.T) {
				...
				data, err := json.Marshal(renderPagerDuty(pagerDutyTestConfig(version), event))
				require.NoError(t, err)
	...
	assert.Equal(
		t,
		renderPagerDuty(pagerDutyTestConfig(1), full),
		renderPagerDuty(Config{IntegrationKey: pagerDutyTestKey}, full),
	)
}
	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			assert.Equal(
				...
				renderPagerDuty(pagerDutyTestConfig(1), event).(pagerDutyV1Event).IncidentKey,
			)
			assert.Equal(
				t,
				pagerDutyTestIncidentKey,
				renderPagerDuty(pagerDutyTestConfig(2), event).(pagerDutyV2Event).DedupKey,
			)
		})
	}
}
...
func TestPagerDutyTitleLimits(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			one := renderPagerDuty(pagerDutyTestConfig(1), event).(pagerDutyV1Event)
			two := renderPagerDuty(pagerDutyTestConfig(2), event).(pagerDutyV2Event)
