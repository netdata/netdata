...
func (dst Config) pagerDutyVersion() int64 {
...
func validatePagerDutyField(version int64, name, value string) error {
	if name == "api_url" {
		for _, host := range []string{"events.pagerduty.com", "events.eu.pagerduty.com"} {
			if err := configfield.APIBase(value, "pagerduty", host); err != nil {
				return err
			}
		}
		return nil
	}
	if err := configfield.Token(value, "pagerduty", name); err != nil {
		return err
	}
	if len(value) != 32 {
...
func renderPagerDuty(dst Config, event notifyevent.Event) any {
