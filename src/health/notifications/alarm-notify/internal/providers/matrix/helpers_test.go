...
func readConfig(r io.Reader) (testutil.Document[Config], error) {
	return testutil.ReadConfig(r, "matrix", func(cfg Config) error { _, err := New(cfg, http.DefaultClient); return err })
}

func testDestination() Config {
	return Config{APIURL: "https://example.com/matrix/", AccessToken: "synthetic-token", RoomID: "!room:example.org"}
}
