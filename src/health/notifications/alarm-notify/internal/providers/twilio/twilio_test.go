...
func TestRenderTwilio(t *testing.T) {
	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			got, err := url.ParseQuery(renderTwilio(Config{From: test.from, To: test.to}, test.event).Encode())
			require.NoError(t, err)
...
func TestTwilioPlainText(t *testing.T) {
	for name, text := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			got, err := url.ParseQuery(renderTwilio(Config{From: "12345", To: "+15005550009"}, event).Encode())
			require.NoError(t, err)
			assert.Equal(t, want, got)
		})
	}
