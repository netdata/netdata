...
func New(config Config, runner *commandexec.Runner) (*Sender, error) {
	...
	config.Recipients = slices.Clone(config.Recipients)
	if config.PlainTextOnly != nil {
		config.PlainTextOnly = new(*config.PlainTextOnly)
	}
	if config.Threading != nil {
		config.Threading = new(*config.Threading)
	}
...
func renderEmail(dst Config, event notifyevent.Event) ([]string, []byte, error) {
