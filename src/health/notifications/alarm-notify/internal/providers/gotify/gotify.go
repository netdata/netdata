...
func validateGotifyField(name, value string) error {
	if name == "api_url" {
		...
		return field.APIBase(value, "gotify", "")
	}
	return field.Token(value, "gotify", "app_token")
}

func sendGotify(ctx context.Context, dst Config, event notifyevent.Event, client *http.Client) error {
...
func New(cfg Config, client *http.Client) (*Sender, error) {
	if err := cfg.validateGotify(); err != nil {
		return nil, err
	}
	if client == nil {
		return nil, errors.New("gotify HTTP client is required")
	}
	return &Sender{config: cfg, client: client}, nil
}
func (s *Sender) Send(ctx context.Context, event notifyevent.Event) error {
	return sendGotify(ctx, s.config, event, s.client)
}
