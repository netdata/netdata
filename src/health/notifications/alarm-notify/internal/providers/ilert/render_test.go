...
func TestRenderIncidentEvents(t *testing.T) {
	for name, test := range map[string]struct{ status, previous, ilertType string }{
		"warning":  {"WARNING", "CLEAR", "ALERT"},
		"critical": {"CRITICAL", "WARNING", "ALERT"},
		"clear":    {"CLEAR", "CRITICAL", "RESOLVE"},
	} {
		t.Run(name, func(t *testing.T) {
			...
			for provider, p := range map[string]struct{ message any }{
				"ilert": {renderIlert(Config{IntegrationKey: "synthetic-key"}, event)},
			} {
	...
	for name, test := range map[string]struct{ node, alert, summary, url string }{
		...
		t.Run(name, func(t *testing.T) {
			...
			for provider, p := range map[string]struct{ got, want any }{
				"ilert": {renderIlert(Config{IntegrationKey: "synthetic-key"}, event), ilert},
			} {
				t.Run(provider, func(t *testing.T) {
