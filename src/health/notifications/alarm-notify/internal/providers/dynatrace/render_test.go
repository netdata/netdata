...
func TestRenderMonitoringEvents(t *testing.T) {
	for name, test := range map[string]struct{ status, previous, severity string }{
		...
		t.Run(name, func(t *testing.T) {
			...
			dynatrace, err := renderDynatrace(Config{EntitySelector: `type(HOST),tag("netdata")`}, event)
			require.NoError(t, err)
			for provider, message := range map[string]any{"dynatrace": dynatrace} {
			...
			for status := range map[string]struct{}{"WARNING": {}, "CRITICAL": {}, "CLEAR": {}} {
				t.Run(status, func(t *testing.T) {
					...
					dynatrace, err := renderDynatrace(
						Config{EntitySelector: test.selector, Source: test.source, EventType: test.eventType},
						event,
					)
					require.NoError(t, err)
					assert.Equal(
...
func TestDynatracePropertyLimits(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			event, dst := testutil.ExpectedEvent(), Config{EntitySelector: "type(HOST)"}
...
func TestMonitoringAcknowledgments(t *testing.T) {
	for provider, p := range map[string]struct {
		...
		"dynatrace": {readDynatraceResponse, dynatraceTestAck},
	} {
		t.Run(provider, func(t *testing.T) {
			for name, test := range map[string]struct {
				status            int
