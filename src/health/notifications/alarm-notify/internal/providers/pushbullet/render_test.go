...
func TestRenderPushbullet(t *testing.T) {
	...
	dst := Config{Email: "ops@example.com", SourceDeviceID: "test-source-device"}
	for name, test := range map[string]struct {
		...
		"channel note and unknown values": {event: minimal, dst: Config{ChannelTag: "test-alerts"}, fixture: "pushbullet-minimal.json"},
	} {
...
func TestPushbulletPlainText(t *testing.T) {
	for name, text := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			encoded, err := json.Marshal(renderPushbullet(Config{Email: "ops@example.com"}, event))
			require.NoError(t, err)
