...
func TestPrepareSelection(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"unsupported alongside valid":   {config: []string{`DISCORD_WEBHOOK_URL=https://example.org; DEFAULT_RECIPIENT_DISCORD=channel; SLACK_WEBHOOK_URL=https://example.org; DEFAULT_RECIPIENT_SLACK=channel`}, err: "legacy slack"},
		"unsupported filtered":          {config: []string{`SLACK_WEBHOOK_URL=https://example.org; DEFAULT_RECIPIENT_SLACK='channel|nowarn'`}},
