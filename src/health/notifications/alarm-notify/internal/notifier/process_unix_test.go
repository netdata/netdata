...
func testCommandDestination(t *testing.T, mode string) (Destination, string) {
	t.Helper()
	executable, err := os.Executable()
	require.NoError(t, err)
	capture := filepath.Join(t.TempDir(), "capture.json")
	return Destination{Type: "command", Executable: executable, Env: map[string]string{
		"NOTIFIER_TEST_COMMAND_HELPER": mode, "NOTIFIER_TEST_CAPTURE": capture, "GORACE": "atexit_sleep_ms=0",
	}}, capture
}

func writeCommandConfig(t *testing.T, cfg Config) string {
	t.Helper()
	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if dst.Type == "smstools3" {
				dst.To, dst.Args = "15005550009", nil
				wantArgs = []string{dst.To, renderSMSTools3(event)}
				wantInput = ""
			}
...
func TestRunCommandCancellation(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			dst.Env["NOTIFIER_TEST_ADDRESS"] = listener.Addr().String()
			cfg := Config{Version: 1, Destinations: map[string]Destination{"target": dst}}
