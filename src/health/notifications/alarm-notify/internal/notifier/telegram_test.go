...
func TestRenderTelegram(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"warning": {event: full, dst: fullDestination, fixture: "telegram-full.json"},
		"critical": {
			event:        critical,
			dst:          fullDestination,
		...
		"unknown values and no topic": {
			event:   minimal,
			dst:     Destination{ChatID: "@example_alerts"},
...
type telegramTestBody struct {
	io.Reader
	closed bool
}

func (body *telegramTestBody) Close() error { body.closed = true; return nil }
