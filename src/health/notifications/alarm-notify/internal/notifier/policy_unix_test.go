...
func TestRunCommandStatusPolicies(t *testing.T) {
	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if test.missing {
				dst.Env["TOKEN"] = "${env:NOTIFIER_POLICY_MISSING}"
			}
			cfg := Config{Version: 1, Destinations: map[string]Destination{"dev": dst},
				Routing: Routing{Policies: map[string]*DestinationPolicy{"dev": {NoWarn: true, NoClear: true}}}}
			event := expectedEvent()
