...
func TestSendPushover(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if test.secret != "" {
				app, user, api := dst.AppToken, dst.UserKey, dst.APIURL
				switch test.secret {
				case "bad-app":
				...
				for _, field := range []struct {
					...
					{"APP", app, &dst.AppToken}, {"USER", user, &dst.UserKey}, {"API", api, &dst.APIURL},
				} {
					if test.secret == "file" {
			...
			other := dst
			other.UserKey = strings.Repeat("V", 30)
			...
			cfg := Config{Version: 1, Destinations: map[string]Destination{
				...
				"unused": {
					Type:     "pushover",
					AppToken: "${env:NOTIFIER_UNUSED_APP}",
					UserKey:  "${env:NOTIFIER_UNUSED_USER}",
				},
			}, Routing: Routing{Roles: map[string][]string{"ops": {"push", "slack", "discord", "telegram", "archive"}, "dba": {"push"}}}}
			if test.second {
			...
			for i, want := range expected {
				...
				if test.second && i == len(expected)-1 {
					payload = bytes.ReplaceAll(payload, []byte(pushoverTestUser), []byte(other.UserKey))
				}
				if test.oversized {
