	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			dst := Destination{
				...
				Originator: "+15005550006",
				Recipient:  "+15005550009",
				APIURL:     server.URL + "/proxy/",
			}
			if test.secret != "" {
				token, api := dst.AccessKey, dst.APIURL
				switch test.secret {
				case "bad-token":
					token += "\nextra"
				case "empty-token":
				...
				for _, field := range []struct {
					...
					{"TOKEN", token, &dst.AccessKey}, {"API", api, &dst.APIURL},
				} {
					if test.secret == "file" {
			...
			cfg := Config{Version: 1, Destinations: map[string]Destination{
				...
				"unused": {
					Type:       "messagebird",
					AccessKey:  "${env:NOTIFIER_UNUSED_TOKEN}",
					Originator: "12345",
					Recipient:  "+15005550009",
				},
			}, Routing: Routing{Roles: map[string][]string{"ops": {"sms", "slack", "discord", "telegram", "pushover", "pushbullet", "twilio", "archive"}, "dba": {"sms"}}}}
			if test.multiple {
