	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			data, err := json.Marshal(
				renderPushover(Destination{AppToken: pushoverTestToken, UserKey: pushoverTestUser}, test.event),
			)
...
func TestPushoverEscaping(t *testing.T) {
	...
	}, renderPushover(Destination{AppToken: pushoverTestToken, UserKey: pushoverTestUser}, event))
}
