...
func ircHelperDestination(t *testing.T, mode string) (Destination, string) {
	t.Helper()
...
func TestRunIRC(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			for _, private := range []string{"synthetic-private-value", "must-not-be-inherited", dst.Host, dst.Channel, dst.Executable} {
			...
			assert.Equal(t, commandCapture{Args: []string{"irc.example.com", "6667"}, Env: []string{"GORACE=atexit_sleep_ms=0", "NOTIFIER_TEST_CAPTURE=" + capture, "NOTIFIER_TEST_COMMAND_HELPER=" + test.mode, "PATH=" + commandDefaultPath, "TOKEN=synthetic-private-value"}, Input: got.Input}, got)
			if test.code != 0 {
				return
			}
			...
			assert.Equal(t, renderIRC(event), content.String())
			if test.info == "" {
...
func TestRunIRCSelection(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			bad.Env = nil
			cfg := Config{Version: 1, Destinations: map[string]Destination{"target": dst, "bad": bad}, Routing: Routing{Roles: map[string][]string{"mixed": {"bad", "target", "target"}, "bad": {"bad"}}}}
