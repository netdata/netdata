			...
			for name, test := range map[string]struct {
				...
				t.Run(name, func(t *testing.T) {
					...
					cfg := Config{Version: 1, Destinations: map[string]Destination{
						"ilert": {
							Type:           "ilert",
							APIURL:         server.URL + "/ilert/synthetic-private-value/",
							IntegrationKey: "synthetic-key",
						},
						"signl4": {
							Type: "signl4",
							URL:  server.URL + "/signl4/synthetic-private-value?keep=a%26b&keep=second",
						},
						"archive": {Type: "webhook", URL: server.URL + "/archive"},
						"unused_ilert": {
							Type:           "ilert",
							IntegrationKey: "${env:UNUSED_INCIDENT_KEY}",
							APIURL:         "${env:UNUSED_INCIDENT_API}",
						},
						"unused_signl4": {Type: "signl4", URL: "${env:UNUSED_INCIDENT_URL}"},
					}, Routing: Routing{Roles: map[string][]string{"ops": {"ilert", "signl4", "archive"}, "dba": {"signl4", "ilert"}}}}
					...
					if test.secret != "" {
						...
						if provider == "ilert" {
							dst.APIURL, dst.IntegrationKey = urlRef, keyRef
						} else {
							dst.URL = urlRef
						}
					}
					...
					for _, target := range selected {
						...
						if target == "archive" {
							...
							payload, err = os.ReadFile(filepath.Join("testdata", target+"-full.json"))
							require.NoError(t, err)
							if target == "ilert" {
								request.path += "/events"
							} else {
...
func TestRunIncidentLifecycle(t *testing.T) {
	for name, test := range map[string]struct{ id, ilertKey string }{
		...
		t.Run(name, func(t *testing.T) {
			...
			defer server.Close()
			cfg := Config{Version: 1, Destinations: map[string]Destination{
				"ilert":  {Type: "ilert", IntegrationKey: "synthetic-key", APIURL: server.URL + "/api"},
				"signl4": {Type: "signl4", URL: server.URL + "/webhook"},
			}, Routing: Routing{Roles: map[string][]string{"ops": {"ilert", "signl4"}}}}
			config, err := yaml.Marshal(cfg)
			require.NoError(t, err)
			...
			for step, status := range []string{"WARNING", "CRITICAL", "CLEAR"} {
				...
				for _, want := range []struct {
					...
					{"/api/events", ilertEvent{IntegrationKey: "synthetic-key", EventType: ilertType, AlertKey: test.ilertKey, Summary: title, Details: text, CustomDetails: event}},
					{"/webhook", signl4Event{Title: title, Message: text, Severity: status, ExternalID: test.id, Status: signl4Status, SourceSystem: "Netdata"}},
				} {
					got := <-requests
