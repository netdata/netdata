...
func TestReadConfig(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"unknown field":    {input: validConfig + "    password: synthetic-private-value\n", err: "invalid YAML"},
		"wrong scalar type": {
			input: strings.Replace(validConfig, "version: 1", "version: synthetic-private-value", 1),
			err:   "invalid YAML",
		},
		"duplicate key":  {input: validConfig + "    url: https://example.com/other\n", err: "invalid YAML"},
		...
		"unsupported provider": {
			input: strings.Replace(validConfig, "webhook", "slack", 1),
			err:   "other providers are not implemented",
		},
		"second document":       {input: validConfig + "---\n" + validConfig, err: "exactly one YAML"},
		...
		"URL credentials": {
			input: strings.Replace(validConfig, "example.com", "user:synthetic-private-value@example.com", 1),
		...
		"invalid scheme": {input: strings.Replace(validConfig, "https:", "ftp:", 1), err: "absolute HTTP(S)"},
		"empty host":     {input: strings.Replace(validConfig, "example.com", "", 1), err: "absolute HTTP(S)"},
		"bearer line break": {
			input: validConfig + "    bearer_token: \"synthetic-private-value\\r\\n\"\n",
			err:   "line breaks",
		},
		"command secret": {
			input: strings.Replace(validConfig, "https://example.com/notify", "${cmd:/synthetic-private-value}", 1),
