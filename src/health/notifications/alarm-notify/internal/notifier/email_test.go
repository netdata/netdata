...
func TestEmailConfig(t *testing.T) {
	for name, test := range map[string]struct {
		...
		"defaults": {},
		"display names and sender": {change: func(d *Destination) {
			d.Recipients = []string{"Ops Team <ops@example.com>", "root"}
			d.From = "Παρακολούθηση <notify@example.com>"
		}},
		"local sender":          {change: func(d *Destination) { d.From = "netdata" }},
		"quoted mailbox":        {change: func(d *Destination) { d.From = `"alert bot"@example.com` }},
		"explicit modes":        {change: func(d *Destination) { d.PlainTextOnly = new(true); d.Threading = new(false) }},
		"environment reference": {change: func(d *Destination) { d.Env = map[string]string{"HOME": "${env:UNREAD_EMAIL_HOME}"} }},
		"no recipients":         {change: func(d *Destination) { d.Recipients = nil }, err: "at least one recipient"},
		"relative executable":   {change: func(d *Destination) { d.Executable = "synthetic-private-value" }, err: "absolute path"},
		"foreign args":          {change: func(d *Destination) { d.Args = []string{"-X", "synthetic-private-value"} }, err: "fields for another provider"},
		"invalid env":           {change: func(d *Destination) { d.Env = map[string]string{"BAD=KEY": "synthetic-private-value"} }, err: "env names"},
	} {
		t.Run(name, func(t *testing.T) {
			dst := Destination{Type: "email", Executable: filepath.Join(t.TempDir(), "sendmail"), Recipients: []string{"root"}}
			if test.change != nil {
				test.change(&dst)
			}
		...
		t.Run(name, func(t *testing.T) {
			...
			if test.invalid {
				...
				for field := range map[string]struct{}{"recipient": {}, "from": {}} {
					...
					checkFormConfig(t, dst, "expected a literal mailbox")
				}
				return
			}
...
func TestEmailFieldIsolation(t *testing.T) {
	fields := reflect.TypeFor[Destination]()
	allowed := map[string]bool{"Type": true, "Executable": true, "Env": true, "Recipients": true, "From": true, "PlainTextOnly": true, "Threading": true}
	...
	for provider := range map[string]struct{}{
		...
		"opsgenie": {}, "msteams": {}, "matrix": {}, "command": {}, "smstools3": {}, "syslog": {}, "awssns": {}, "kafka": {},
	} {
		for field := range map[string]struct{}{"plain_text_only": {}, "threading": {}} {
			t.Run(provider+"/"+field, func(t *testing.T) {
				...
				if field == "plain_text_only" {
					dst.PlainTextOnly = new(false)
				} else {
					dst.Threading = new(false)
				}
				checkFormConfig(t, dst, "plain_text_only and threading require email")
			})
		}
	}
}
	...
	for field := range map[string]struct{}{"plain_text_only": {}, "threading": {}} {
		for name, test := range map[string]struct {
			...
			t.Run(field+"/"+name, func(t *testing.T) {
				...
				require.NoError(t, err)
				want := Destination{Type: "email", Executable: cfg.Destinations["mail"].Executable, Recipients: []string{"root"}}
				if field == "threading" {
...
func TestRenderEmail(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			dst := Destination{Type: "email", Recipients: []string{"Ops Team <ops@example.com>", "root"}, From: test.from, PlainTextOnly: new(test.plain), Threading: test.threading}
			event := expectedEvent()
...
func TestEmailEscapingAndLongLines(t *testing.T) {
	for name, text := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			event.Node = text
			event.Info = text
			dst := Destination{Type: "email", Recipients: []string{"ops@example.com"}}
	...
	for name, display := range map[string]string{"ASCII": strings.Repeat("a", 3000), "Unicode": strings.Repeat("名", 1000)} {
		t.Run("display name/"+name, func(t *testing.T) {
			dst := Destination{Type: "email", Recipients: []string{display + " <ops@example.com>"}, From: display + " <notify@example.com>"}
			_, raw, err := renderEmail(dst, expectedEvent())
			require.NoError(t, err)
...
func TestEmailThreadIdentity(t *testing.T) {
	base := expectedEvent()
	dst := Destination{Type: "email", Recipients: []string{"root"}}
...
func TestEmailOptionalContent(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			_, raw, err := renderEmail(Destination{Type: "email", Recipients: []string{"root"}, PlainTextOnly: new(true)}, event)
			require.NoError(t, err)
			header, parts := parseEmail(t, raw)
