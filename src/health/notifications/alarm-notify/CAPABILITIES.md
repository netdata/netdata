...
| Results | Per-target failures and Bash's any-success invocation result | Implemented for all current Go providers; IRC synchronization and subprocess acceptance do not guarantee recipient delivery |
...
The first twenty-four implementation PRs cover the foundation, routing/fan-out, Slack app webhooks, Discord, Telegram, Pushover,
Pushbullet, Twilio, MessageBird, Gotify, ntfy, Rocket.Chat, Flock, Fleep, ilert, SIGNL4, Alerta, Dynatrace, Prowl, Kavenegar,
SMSEagle, PagerDuty, Opsgenie, Teams, Matrix, custom commands, SMS Server Tools 3, syslog, AWS SNS, Kafka HTTP bridges, email and IRC. Windows command delivery remains
