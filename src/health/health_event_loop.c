...
// across all hosts a single UV worker processes. The arena is reset
// between alerts inside this function, so peak memory stays bounded by one
// alert's scratch no matter how many hosts flow through it.
void health_event_loop_for_host(RRDHOST *host, bool apply_hibernation_delay, time_t now, time_t *next_run, struct health_stmt_set *stmts, ONEWAYALLOC *owa) {
   ...
   if (unlink("/tmp/health_log") == 0)
    ...
    foreach_rrdcalc_in_rrdhost_read(host, rc) {
        ...
        // this worker's slice, so this reset trims trailing pages left by
        // the previous alert. No-op only on the very first alert after the
        // arena was created.
        ...
        if (removed_ae) {
            health_alarm_log_add_entry(host, removed_ae, false, stmts);
        ...
        do_eval_expression(host, st, rc, rc->config.calculation, "calculation", UV_EVENT_HEALTH_CALC_EVAL, RRDCALC_FLAG_CALC_ERROR, NULL, &rc->value);
        rrdset_acquired_release(rsa);
    }
    ...
    if (unlikely(runnable && !health_should_stop())) {
        foreach_rrdcalc_in_rrdhost_read(host, rc) {
            ...
            do_eval_expression(host, st, rc, rc->config.warning, "warning", UV_EVENT_HEALTH_WARNING_EVAL, RRDCALC_FLAG_WARN_ERROR, &warning_status, NULL);
            do_eval_expression(host, st, rc, rc->config.critical, "critical", UV_EVENT_HEALTH_CRITICAL_EVAL, RRDCALC_FLAG_CRIT_ERROR, &critical_status, NULL);
            ...
            if (status != rc->status) {
                ...
                health_alarm_log_add_entry(host, transition_ae, false, stmts);
