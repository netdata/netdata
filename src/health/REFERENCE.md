...
This section provides the complete reference for all variables available in Netdata alert notifications. These variables can be used in custom notification integrations and message formats.

:::

### Variable Reference

The following variables are available in alert notifications:
...
### Duration vs non_clear_duration

The `duration` and `non_clear_duration` variables serve different purposes:

- **`${duration}`**: The time in seconds the alert spent in the **previous** state before transitioning to the current state.
- **`${non_clear_duration}`**: The **total** time the alert has been in a non-clear state (WARNING or CRITICAL).

For **repeating notifications** (when an alert remains in WARNING or CRITICAL state and continues to notify), Netdata sets `${non_clear_duration}` equal to `${duration}` rather than the total non-clear time.

### Integration-specific Variables

Some notification integrations may support additional variables beyond the standard set. Check the specific integration documentation for details.
