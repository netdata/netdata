...
int run_test(struct test *test)
{
    ...
    for(c = 0 ; c < max ; c++) {
        ...
        fprintf(stderr, "    %s/%s: checking position %lu (at %llu secs), expecting value " CALCULATED_NUMBER_FORMAT ", found " CALCULATED_NUMBER_FORMAT ", %s\n",
            test->name, rd->name, c+1,
            (long long)((rrdset_first_entry_t(st) + c * st->update_every) - time_start),
