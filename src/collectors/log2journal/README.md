...
By combining these tools you can build log processing pipelines that send any kind of structured text logs to systemd-journald.
...
Rewrite rules accept named groups in them, like in the main pattern, to extract sub-fields from them, which you can then use in variable substitution. You can use rewrite rules to anonymize the URLs, e.g to remove customer IDs or transaction details from them.
...
systemd-journald employs several techniques to optimize disk footprint, like deduplication of log entries, shared indexes for fields and their values, and compression of long log entries. Still, journal files index every field of every entry, so they are bigger than the same logs stored as compressed plain text.

Storing journal files on a compressed filesystem reduces the footprint. In our tests, a compressed filesystem can save up to 75% of the space required by journal files. How much disk journals are allowed to use, and for how long they are kept, is controlled with journald's disk-usage settings — see [Log Storage and Retention](/docs/logs/log-storage-and-retention.md).
...
Another limitation of the `systemd-journald` ecosystem is the uncompressed transmission of logs across systems. `systemd-journal-remote` up to version 254 that we tested, accepts encrypted, but uncompressed data. This means that when centralizing logs to a logs server, the logs travel uncompressed and the bandwidth required is proportional to the raw size of the log entries.
