...
[KEY FEATURES](#key-features) | [PREREQUISITES](#prerequisites) | [JOURNAL SOURCES](#journal-sources) | [JOURNAL FIELDS](#journal-fields) | [VISUALIZATION](#visualization-capabilities) | [PLAY MODE](#play-mode) | [FULL TEXT SEARCH](#full-text-search) | [EXAMPLE COMMANDS](#example-commands) | [QUERY PERFORMANCE](#query-performance) | [PERFORMANCE AT SCALE](#performance-at-scale) | [BEST PRACTICES](#best-practices-for-better-performance) | [CONFIGURATION](#configuration-and-maintenance) | [FAQ](#faq) | [HOW TO TROUBLESHOOT COMMON ISSUES](#how-to-troubleshoot-common-issues) | [HOW TO VERIFY SETUP](#how-to-verify-setup)
...
| `libsystemd` | The plugin relies on libsystemd to read journal files |
| Netdata Cloud account | Required to use Netdata Functions including this plugin |
| **Not supported in**: | Alpine Linux Docker containers or static builds (use Debian-based containers instead) |
...
| Multiple patterns | Separate with `|` (e.g., `error|warning`) |
...
Here are some useful commands to assist with troubleshooting or managing the `systemd` journal plugin.
...
### Test journal access directly with `journalctl`

```bash
journalctl --no-pager --output=short-monotonic
```

Example: Filter by priority and time range:

```bash
journalctl -p warning --since "1 hour ago"
```

### Verify Netdata plugin status

Check if the plugin is listed and active:

```bash
ps aux | grep systemd-journal.plugin
```

### Restart the Netdata Agent (if necessary)
...
```bash
docker restart <netdata-container-name>
```

### Check Netdata logs for plugin errors

```bash
sudo journalctl -u netdata.service --since "10 minutes ago"
```

Or view the Agent log file directly (if not using systemd):
...
If the plugin is missing or not working as expected, confirm that your Netdata build includes `libsystemd`. Static builds and older Alpine-based containers may not support this plugin.
