...
The increased privileges are primarily used for building the process tree in memory, iterating over running processes, collecting metrics, enriching local cgroup metadata, and sending data to Netdata. APPS_LOOKUP requests are local-only, size-bounded by the netipc payload limit, and limited to 8192 PIDs per request. On Linux, the `processes` Function exposes the same per-PID cgroup/container/service fields used by network connections, including cgroup status/path/name, container name, orchestrator, systemd unit kind, and actor kind.
