        ...
        if(unlikely(debug_enabled || errno != ENOENT)) {
            if(unlikely(debug_enabled || !(p->log_thrown & log))) {
                ...
                switch(log) {
                    case PID_LOG_IO:
                        #if defined(__FreeBSD__) || defined(__APPLE__)
                        netdata_log_error("Cannot fetch process %d I/O info (command '%s')", p->pid, string2str(p->comm));
                        #else
                        netdata_log_error("Cannot process %s/proc/%d/io (command '%s')",
                            netdata_configured_host_prefix, p->pid, string2str(p->comm));
                        #endif
                        break;

                    case PID_LOG_STATUS:
                        #if defined(__FreeBSD__) || defined(__APPLE__)
                        netdata_log_error("Cannot fetch process %d status info (command '%s')", p->pid, string2str(p->comm));
                        #else
                        netdata_log_error("Cannot process %s/proc/%d/status (command '%s')",
                            netdata_configured_host_prefix, p->pid, string2str(p->comm));
                        #endif
                        break;
                    ...
                    case PID_LOG_CMDLINE:
                        #if defined(__FreeBSD__) || defined(__APPLE__)
                        netdata_log_error("Cannot fetch process %d command line (command '%s')", p->pid, string2str(p->comm));
                        #else
                        netdata_log_error("Cannot process %s/proc/%d/cmdline (command '%s')",
                            netdata_configured_host_prefix, p->pid, string2str(p->comm));
                        #endif
                        break;
                    ...
                    case PID_LOG_FDS:
                        #if defined(__FreeBSD__) || defined(__APPLE__)
                        netdata_log_error("Cannot fetch process %d files (command '%s')", p->pid, string2str(p->comm));
                        #else
                        netdata_log_error("Cannot process entries in %s/proc/%d/fd (command '%s')",
                            netdata_configured_host_prefix, p->pid, string2str(p->comm));
                        #endif
                        break;
                    ...
                    case PID_LOG_LIMITS:
                        ...
                        ;
                        #else
                        netdata_log_error("Cannot process %s/proc/%d/limits (command '%s')",
                            netdata_configured_host_prefix, p->pid, string2str(p->comm));
                        #endif
                    ...
                    default:
                        netdata_log_error("unhandled error for pid %d, command '%s'",
                            p->pid, string2str(p->comm));
                        break;
                }
            }
        }
            ...
            if(unlikely(!p->target && p->parent && p->parent->target)) {
                ...
                if(debug_enabled || (p->target && p->target->debug_enabled))
                    debug_log_int("TARGET INHERITANCE: %s is inherited by %d (%s) from its parent %d (%s).",
                        p->target->name, p->pid, string2str(p->comm), p->parent->pid, string2str(p->parent->comm));
            }
        }
    }
                ...
                if(unlikely(p->target && !p->parent->target)) {
                    ...
                    if(debug_enabled || (p->target && p->target->debug_enabled))
                        debug_log_int("TARGET INHERITANCE: %s is inherited by %d (%s) from its child %d (%s).",
                            p->target->name, p->parent->pid, string2str(p->parent->comm), p->pid, string2str(p->comm));
                }
...
static inline void aggregate_pid_on_target(struct target *w, struct pid_stat *p, struct target *o) {
    ...
    if(unlikely(!w)) {
        netdata_log_error("pid %d %s was left without a target!", p->pid, string2str(p->comm));
        return;
    }
    ...
    if(unlikely(debug_enabled || w->debug_enabled)) {
        debug_log_int("aggregating '%s' pid %d on target '%s' utime=" KERNEL_UINT_FORMAT ", stime=" KERNEL_UINT_FORMAT ", gtime=" KERNEL_UINT_FORMAT ", cutime=" KERNEL_UINT_FORMAT ", cstime=" KERNEL_UINT_FORMAT ", cgtime=" KERNEL_UINT_FORMAT ", minflt=" KERNEL_UINT_FORMAT ", majflt=" KERNEL_UINT_FORMAT ", cminflt=" KERNEL_UINT_FORMAT ", cmajflt=" KERNEL_UINT_FORMAT "",
            string2str(p->comm), p->pid, w->name, p->utime, p->stime, p->gtime, p->cutime, p->cstime, p->cgtime, p->minflt, p->majflt, p->cminflt, p->cmajflt);
    ...
    for(p = root_of_pids; p ; p = p->next) {
        ...
        if(likely(p->user_target && p->user_target->uid == p->uid))
            ...
            if(unlikely(debug_enabled && p->user_target))
                debug_log("pid %d (%s) switched user from %u (%s) to %u.", p->pid, string2str(p->comm), p->user_target->uid, p->user_target->name, p->uid);
        ...
        if(likely(p->group_target && p->group_target->gid == p->gid))
            ...
            if(unlikely(debug_enabled && p->group_target))
                debug_log("pid %d (%s) switched group from %u (%s) to %u.", p->pid, string2str(p->comm), p->group_target->gid, p->group_target->name, p->gid);
