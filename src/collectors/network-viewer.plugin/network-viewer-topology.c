...
// Shared topology:network-connections implementation for network-viewer.plugin.
// Compiled on every OS that builds the plugin (see network-viewer-topology.h).
// Windows drives it through the local-sockets-windows.h backend; POSIX through
// the Linux/FreeBSD/macOS backends. The network-connections Function and the
// per-OS mains live in network-viewer.c (POSIX) and network-viewer-windows.c
// (Windows).
...
// Process display names can be longer than TASK_COMM_LEN (e.g. Windows
// exe base names); the actor/link buffers use this bounded width.
#define NV_TOPOLOGY_PROCESS_MAX 256
...
typedef struct {
    ...
    uint64_t starttime;
    uint64_t sockets;
    char process[NV_TOPOLOGY_PROCESS_MAX];
...
static void topology_actor_id_for_process(
    ...
    if(ctx && ctx->options.group_by == NV_TOPOLOGY_GROUP_BY_CONTAINER) {
        ...
        char encoded_process[((NV_TOPOLOGY_PROCESS_MAX) * 3) + 1];
...
static void local_sockets_cb_to_topology(LS_STATE *ls, const LOCAL_SOCKET *n, void *data) {
    ...
    // Windows backend provides the full exe base name separately from comm
    // (LOCAL_SOCKET.comm is truncated to TASK_COMM_LEN).
    if(n->win_comm[0])
        process_name = n->win_comm;
#endif
...
#if defined(OS_WINDOWS)
    // Windows has no uid namespace; the backend resolves the SID account name.
    else if(n->win_username[0])
        snprintf(username, sizeof(username), "%s", n->win_username);
#endif
