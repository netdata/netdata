...
func pktBuf(pkt []byte) *C.char {
	if len(pkt) == 0 {
		return nil
	}
	return (*C.char)(unsafe.Pointer(&pkt[0]))
}
...
func TestDNSParseRawPacketBoundaries(t *testing.T) {
	rt := C.netdata_dns_alloc_test_runtime()
	if rt == nil {
		t.Fatal("alloc test runtime returned nil")
	}
	defer C.netdata_dns_free_test_runtime(rt)
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			got := C.netdata_dns_test_parse_raw_packet(rt, pktBuf(tc.pkt), C.int(len(tc.pkt)))
			gotBool := got != 0
			if gotBool != tc.want {
				t.Errorf("parse_raw_packet(%d bytes) = %v, want %v", len(tc.pkt), gotBool, tc.want)
			}
		})
	}
}
...
func TestDNSReadNameBoundaries(t *testing.T) {
	...
	callReadName := func(msg []byte, offset, outSize int) int {
		return int(C.netdata_dns_test_read_name(
			msgBuf(msg), C.int(len(msg)),
			C.int(offset),
			&out[0], C.int(outSize),
		))
	}
	...
	t.Run("empty message", func(t *testing.T) {
		if got := callReadName(nil, 0, 256); got != 0 {
	...
	t.Run("offset at end of message", func(t *testing.T) {
		msg := []byte{0x00}
		if got := callReadName(msg, 1, 256); got != 0 {
			t.Errorf("got %d, want 0", got)
		}
	})

	t.Run("compression pointer — second byte missing", func(t *testing.T) {
		// 0xC0 alone: pointer flag set but no second byte
		if got := callReadName([]byte{0xC0}, 0, 256); got != 0 {
			t.Errorf("got %d, want 0", got)
		}
	})

	t.Run("self-referential compression pointer loop", func(t *testing.T) {
		// 0xC0 0x00 at offset 0 points back to offset 0 — infinite loop without jump limit
		if got := callReadName([]byte{0xC0, 0x00}, 0, 256); got != 0 {
			t.Errorf("got %d, want 0", got)
		}
	})

	t.Run("label length exceeds remaining message bytes", func(t *testing.T) {
		// Says 0x10 (16) chars follow, but only 2 bytes in message
		if got := callReadName([]byte{0x10, 'a', 'b'}, 0, 256); got != 0 {
			t.Errorf("got %d, want 0", got)
		}
	})

	t.Run("label length > 63 (RFC max)", func(t *testing.T) {
		if got := callReadName([]byte{0x40, 0x00}, 0, 256); got != 0 {
			t.Errorf("got %d, want 0", got)
		}
	})
	...
	t.Run("out buffer too small for label", func(t *testing.T) {
		...
		if got := callReadName([]byte{0x04, 'f', 'o', 'o', 'b', 0x00}, 0, 4); got != 0 {
			t.Errorf("got %d, want 0", got)
		}
	})

	t.Run("root label only — empty name", func(t *testing.T) {
		if got := callReadName([]byte{0x00}, 0, 256); got != 1 {
			t.Errorf("got %d, want 1", got)
		}
	})

	t.Run("valid single-label foo", func(t *testing.T) {
		// \x03foo\x00 → 5 bytes consumed, out = "foo"
		if got := callReadName([]byte{0x03, 'f', 'o', 'o', 0x00}, 0, 256); got != 5 {
			t.Errorf("got %d, want 5", got)
		}
	})

	t.Run("valid two-label name go.dev", func(t *testing.T) {
		// \x02go\x03dev\x00 → 9 bytes consumed, out = "go.dev"
		msg := []byte{0x02, 'g', 'o', 0x03, 'd', 'e', 'v', 0x00}
		if got := callReadName(msg, 0, 256); got != 8 {
