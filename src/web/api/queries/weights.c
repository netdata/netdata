                                                  ...
                                                  struct query_weights_data *qwd)
{
    ...
    if (num_threads <= 1 || active_hosts <= 1 || !dbengine_work_available(netdata_conf_dbengine_engine)) {
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        if (!dbengine_enq_work(netdata_conf_dbengine_engine, &thread_data[i].work)) {
