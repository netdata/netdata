    ...
    struct web_client *w, char *s, size_t length, bool *incomplete)
{
    *incomplete = false;
    ...
    if(matched->mode == HTTP_REQUEST_MODE_STREAM &&
       !SSL_connection(&w->ssl) && http_is_using_ssl_force(w)) {
        ...
        netdata_log_error(
            "The server is configured to always use encrypted connections, please enable the SSL on child with hostname '%s'.",
            hostname);
        return NULL;
    }
        ...
        if(likely(*s++ == '\n')) {
            if(unlikely(*s == '\r' && s[1] == '\n')) {
                ...
                if(web_client_check_conn_tcp(w) && netdata_ssl_web_server_ctx &&
                   !w->ssl.conn && (http_is_using_ssl_force(w) || http_is_using_ssl_default(w)) &&
...
int web_client_request_unittest(void) {
    ...
    // The adversarial packetization case must remain linear and complete only on the final byte.
    CLEAN_CHAR_P *fragmented_max_request = mallocz(NETDATA_WEB_REQUEST_MAX_SIZE + 1);
    memcpy(fragmented_max_request, buffer_tostring(w->response.data), NETDATA_WEB_REQUEST_MAX_SIZE);
    web_client_reuse_from_cache(w);
    for(size_t i = 0; i < NETDATA_WEB_REQUEST_MAX_SIZE; i++) {
        buffer_strncat(w->response.data, &fragmented_max_request[i], 1);
        HTTP_VALIDATION expected =
            i + 1 == NETDATA_WEB_REQUEST_MAX_SIZE ? HTTP_VALIDATION_OK : HTTP_VALIDATION_INCOMPLETE;
    ...
    // The transport boundary owns the size policy; the parser remains reusable for already-bounded inputs.
    web_client_reuse_from_cache(w);
    ...
    // Once headers are complete, body fragments use stored offsets and lengths.
    web_client_reuse_from_cache(w);
    ...
    for(size_t i = 0; i < sizeof("hello") - 1; i++) {
        ...
        HTTP_VALIDATION expected =
            i + 1 == sizeof("hello") - 1 ? HTTP_VALIDATION_OK : HTTP_VALIDATION_INCOMPLETE;
        if(http_request_validate(w) != expected ||
    ...
    // Exercise a maximum-size body through many fragments to guard against request-size-dependent rescanning.
    web_client_reuse_from_cache(w);
    char large_headers[128];
    size_t large_body_length = NETDATA_WEB_REQUEST_MAX_SIZE;
    size_t large_headers_length = (size_t)snprintfz(
        large_headers, sizeof(large_headers),
        "POST /api HTTP/1.1\r\nContent-Length: %zu\r\n\r\n", large_body_length);
    large_body_length = NETDATA_WEB_REQUEST_MAX_SIZE - large_headers_length;
    large_headers_length = (size_t)snprintfz(
        large_headers, sizeof(large_headers),
        "POST /api HTTP/1.1\r\nContent-Length: %zu\r\n\r\n", large_body_length);
    if(large_headers_length + large_body_length != NETDATA_WEB_REQUEST_MAX_SIZE)
        errors++;

    buffer_strncat(w->response.data, large_headers, large_headers_length);
    if(http_request_validate(w) != HTTP_VALIDATION_INCOMPLETE)
        errors++;

    char body_fragment[1024];
    memset(body_fragment, 'x', sizeof(body_fragment));
    size_t remaining = large_body_length;
    while(remaining) {
        size_t fragment_length = MIN(remaining, sizeof(body_fragment));
        buffer_strncat(w->response.data, body_fragment, fragment_length);
        remaining -= fragment_length;
        HTTP_VALIDATION expected = remaining ? HTTP_VALIDATION_INCOMPLETE : HTTP_VALIDATION_OK;
        if(http_request_validate(w) != expected) {
            errors++;
            break;
        }
    }
    if(buffer_strlen(w->response.data) != NETDATA_WEB_REQUEST_MAX_SIZE || !w->payload ||
       buffer_strlen(w->payload) != large_body_length)
        errors++;

    // Keep master framing semantics: first Content-Length wins and Transfer-Encoding is not interpreted here.
    static const char *master_compatible_requests[] = {
        "POST / HTTP/1.1\r\nContent-Length: 5\r\nContent-Length: 6\r\n\r\nhello",
        "POST / HTTP/1.1\r\nTransfer-Encoding: chunked\r\nContent-Length: 5\r\n\r\nhello",
    };
    for(size_t i = 0; i < _countof(master_compatible_requests); i++) {
        web_client_reuse_from_cache(w);
        buffer_strcat(w->response.data, master_compatible_requests[i]);
        if(http_request_validate(w) != HTTP_VALIDATION_OK || !w->payload ||
           strcmp(buffer_tostring(w->payload), "hello"))
            errors++;
    }

    // Missing, malformed, and compact Content-Length retain master's incomplete-request behavior.
    static const char *incomplete_requests[] = {
        "POST / HTTP/1.1\r\n\r\n",
    ...
    for(size_t i = 0; i < _countof(incomplete_requests); i++) {
        web_client_reuse_from_cache(w);
        buffer_strcat(w->response.data, incomplete_requests[i]);
    ...
    if(http_request_validate(w) != HTTP_VALIDATION_NOT_SUPPORTED || web_client_has_wait_receive(w))
        errors++;
    web_client_prepare_request_summary(w, "unsupported request method");
