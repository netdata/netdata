...
Netdata serves static files from `[directories].web` using the Agent process's filesystem permissions. It does not apply separate owner or group checks, so deployment ownership and permissions determine which content the web server can read and who can replace it.
