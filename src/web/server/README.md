...
You can modify web server behavior by editing the `[web]` section in `netdata.conf` using the [`edit-config` script](/docs/netdata-agent/configuration/README.md#edit-configuration-files).
...
| `mode`                             | `static-threaded`                                                                                                                                                                      | Turns on (`static-threaded`) or off (`none`) the static-threaded Web Server. See the [example](#examples) to turn off the Web Server and disable the dashboard                                                                                                                                                                                                                                          |
...
| `uri prefix`                       | empty                                                                                                                                                                                  | An optional secret path prefix that must be present at the start of every URL. When set, requests not matching `/<prefix>/...` return HTTP 404, hiding the dashboard and API from automated scanners. Example: `uri prefix = m5UY19cbG39DzKo0ieP` makes the dashboard available only at `https://host:19999/m5UY19cbG39DzKo0ieP/`. Streaming connections from child agents are not affected. Use only alphanumeric characters, hyphens, and underscores in the prefix; URL-reserved characters may cause browser or proxy issues. **Note:** This is security-through-obscurity and should complement, not replace, proper authentication and network-level access controls |
...
For information how to configure the child to use TLS, check [securing the communication](/src/streaming/README.md#secure-communications) in the streaming documentation. There you will find additional details on the expected behavior for client and server nodes, when their respective TLS options are enabled.
