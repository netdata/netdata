...
class NetdataMCPChat {
    ...
    async retryLLMRequest(chatId) {
        ...
        try {
            ...
            const provider = createLLMProvider(providerApiType, proxyProvider.proxyUrl, modelName, proxyProvider.availableProviders?.[providerType]);
            ...
            case 'tool-result':
                ...
                if (event.subChatId) {
                    ...
                    if (!existingSubChatDom) {
                        // Sub-chat might not exist if loading from storage or if it was cleaned up
                        // We can still create the DOM element using the information from the tool result
                        if (!subChat) {
                            console.warn(`[processRenderEvent] Sub-chat ${event.subChatId} not found, creating placeholder DOM`);
                        } else {
                        ...
                        // Render sub-chat DOM element - it will handle missing sub-chat data gracefully
        ...
        if (confirmed) {
            ...
            if (this.getActiveChatId() === chatId) {
                ...
                if (this.chats.size === 0) {
                    ...
                    // Only reset dropdown buttons when no chats remain
                    ...
                    if (mostRecentChat) {
                        // Force a full render when switching after deletion
                        // loadChat will handle updating all controls properly
                        this.loadChat(mostRecentChat, true);
                    }
                }
    ...
    async sendMessage(chatId, messageParam = null, isResume = false) {
        ...
        const provider = window.createLLMProvider(providerApiType, proxyProvider.proxyUrl, modelName, proxyProvider.availableProviders?.[providerType]);
        provider.onLog = (logEntry) => {
            const prefix = logEntry.direction === 'sent' ? 'llm-request' : 'llm-response';
            const providerName = providerType.charAt(0).toUpperCase() + providerType.slice(1);
            this.addLogEntry(`${prefix}: ${providerName}`, logEntry);
        ...
        // Final guard: ensure Ollama contextWindow is set and clamped before any request
        if (providerType === 'ollama' || providerApiType === 'ollama') {
            try {
                const maxCtx = this.getDefaultContextWindow(chat);
                const userCtx = chat?.config?.model?.params?.contextWindow;
                const base = (typeof userCtx === 'number' && userCtx > 0) ? userCtx : 32768;
                const effective = Math.min(base, typeof maxCtx === 'number' && maxCtx > 0 ? maxCtx : base);
                if (!chat.config.model.params) chat.config.model.params = {};
                if (chat.config.model.params.contextWindow !== effective) {
                    chat.config.model.params.contextWindow = effective;
                }
            } catch (e) {
                console.warn('[sendMessage] Failed to enforce Ollama context window clamp:', e);
            }
        }
    ...
    editSystemPromptInline(contentDiv, originalPrompt, chatId) {
        ...
        // Create floating save/cancel/reset buttons
        const buttonsDiv = document.createElement('div');
        buttonsDiv.className = 'edit-actions-floating';
        buttonsDiv.innerHTML = `
            <button class="btn btn-small btn-primary save-btn" data-tooltip="Save & Restart Chat (Enter)"><i class="fas fa-check"></i></button>
            <button class="btn btn-small btn-secondary cancel-btn" data-tooltip="Cancel (Escape)"><i class="fas fa-times"></i></button>
            <button class="btn btn-small btn-secondary reset-btn" data-tooltip="Reset to Default"><i class="fas fa-undo"></i></button>
        `;
        ...
        // Handle reset button
        const reset = () => {
            contentDiv.textContent = this.defaultSystemPrompt;
            // Also reset the lastSystemPrompt so new chats get the default
            this.lastSystemPrompt = this.defaultSystemPrompt;
            localStorage.removeItem('lastSystemPrompt');
        };

        buttonsDiv.querySelector('.save-btn').onclick = save;
        buttonsDiv.querySelector('.cancel-btn').onclick = cancel;
        buttonsDiv.querySelector('.reset-btn').onclick = reset;
    ...
    async createSubChatForTool(parentChat, toolCall, toolResult) {
        ...
        // Determine sub-chat model - need to include params for context window
        let subChatModel;
        if (parentChat.config.optimisation.toolSummarisation.model) {
            // Use the configured tool summarisation model with its params
            subChatModel = parentChat.config.optimisation.toolSummarisation.model;
            // Normalize if legacy string format slipped in
            if (typeof subChatModel === 'string') {
                try {
                    subChatModel = ChatConfig.modelConfigFromString(subChatModel);
                } catch (e) {
                    console.warn('[createSubChatForTool] Invalid tool summarisation model string; falling back to parent model:', e);
                    subChatModel = null;
                }
            }
        } else {
            // Fall back to parent chat's model with its params (use config, not legacy string)
            subChatModel = {
                provider: parentChat.config?.model?.provider || 'anthropic',
                id: parentChat.config?.model?.id || 'claude-3-haiku-20240307',
                params: parentChat.config?.model?.params || {}
            };
        }

        // Ensure the model has params object
        if (subChatModel && !subChatModel.params) {
            subChatModel.params = {};
        }

        // CRITICAL: Always apply parent's configured context window to sub-chat (even if tool model has its own value)
        // Then clamp to sub-chat model's maximum
        if (subChatModel && subChatModel.provider === 'ollama') {
            try {
                const parentRaw = parentChat.config?.model?.params?.contextWindow;
                const parentCtx = typeof parentRaw === 'string' ? parseInt(parentRaw, 10) : parentRaw;
                const parentEffective = (typeof parentCtx === 'number' && parentCtx > 0)
                    ? parentCtx
                    : 32768; // reasonable default if parent not set

                const maxCtx = this.getDefaultContextWindow({ config: { model: subChatModel } });
                const clamped = Math.min(parentEffective, typeof maxCtx === 'number' && maxCtx > 0 ? maxCtx : parentEffective);

                subChatModel.params.contextWindow = clamped;
                console.log(`[createSubChatForTool] Applying parent context to sub-chat: parent=${parentEffective}, modelMax=${maxCtx}, effective=${clamped}`);
            } catch (e) {
                console.warn('[createSubChatForTool] Failed to set sub-chat context window from parent:', e);
                // Fallback: at least ensure some default
                if (!subChatModel.params.contextWindow) {
                    subChatModel.params.contextWindow = 32768;
                }
            }
        }
    ...
    async processSubChat(subChatId, parentChatId, _toolCallId) {
        ...
        try {
            ...
            // If there's an error, propagate it to the parent
            if (errorMessage) {
                console.log('[processSubChat] Sub-chat encountered error, propagating to parent');
                // Return the error content with a clear indication it's an error
                return `ERROR: Tool request succeeded, but processing its output failed with this error: ${errorMessage.content}\nPlease retry the request while trying to address the issue.`;
            }
            ...
            // Return the error message so the parent can see what went wrong
            return `ERROR: Tool request succeeded, but processing its output failed with this error: ${error.message}\nPlease retry the request while trying to address the issue.`;
        }
    }
     ...
     */
    updateSubChatStatus(parentChatId, toolCallId, newStatus, originalSize = null, summarizedSize = null) {
        const container = this.getChatContainer(parentChatId);
        if (!container) {
        ...
        // If we have sizes and status is success, add it to the tool-info
        if (originalSize !== null && originalSize > 0 && summarizedSize !== null && newStatus === 'success') {
            const toolInfo = subChatItem.querySelector('.tool-info');
            if (toolInfo) {
                // Check if size metric already exists
                let sizeMetric = toolInfo.querySelector('.tool-metric.response-size');

                // Format original size
                let originalSizeInfo;
                if (originalSize < 1024) {
                    originalSizeInfo = `${originalSize} bytes`;
                } else if (originalSize < 1024 * 1024) {
                    originalSizeInfo = `${(originalSize / 1024).toFixed(1)} KB`;
                } else {
                    originalSizeInfo = `${(originalSize / (1024 * 1024)).toFixed(1)} MB`;
                }

                // Format summarized size
                let summarizedSizeInfo;
                if (summarizedSize < 1024) {
                    summarizedSizeInfo = `${summarizedSize} bytes`;
                } else if (summarizedSize < 1024 * 1024) {
                    summarizedSizeInfo = `${(summarizedSize / 1024).toFixed(1)} KB`;
                } else {
                    summarizedSizeInfo = `${(summarizedSize / (1024 * 1024)).toFixed(1)} MB`;
                }

                // Create the size display showing reduction
                const sizeDisplay = `${originalSizeInfo} → ${summarizedSizeInfo}`;

                if (!sizeMetric) {
                    // Create new size metric
                    sizeMetric = document.createElement('span');
                    sizeMetric.className = 'tool-metric response-size';
                    sizeMetric.innerHTML = `<i class="fas fa-box"></i> ${sizeDisplay}`;
                    toolInfo.appendChild(sizeMetric);
                } else {
                    // Update existing size metric
                    sizeMetric.innerHTML = `<i class="fas fa-box"></i> ${sizeDisplay}`;
                }
            }
        }
     ...
     */
    renderSubChatAsItem(parentChatId, subChatId, toolCallId, status = 'success', originalSize = null, summarizedSize = null) {
        const parentContainer = this.getChatContainer(parentChatId);
        const subChat = this.chats.get(subChatId);

        if (!parentContainer) {
            console.error(`[renderSubChatAsItem] Parent container not found for parentChatId: ${parentChatId}`);
            return;
        }

        // Allow rendering even if sub-chat doesn't exist (for loaded chats)
        // Create a minimal sub-chat object if needed
        const displaySubChat = subChat || {
            messages: [],
            parentToolName: 'Tool',
            parentChatId,
        ...
        // If sizes not provided and status is success, try to retrieve them from the sub-chat's result
        let finalOriginalSize = originalSize;
        let finalSummarizedSize = summarizedSize;
        if ((finalOriginalSize === null || finalSummarizedSize === null) && status === 'success') {
            // Find the summarized result from the parent chat's tool results
            const parentChat = this.chats.get(parentChatId);
            if (parentChat) {
                for (const message of parentChat.messages) {
                    if (message.role === 'tool-results' && message.toolResults) {
                        const toolResult = message.toolResults.find(tr => tr.toolCallId === toolCallId);
                        if (toolResult && toolResult.wasProcessedBySubChat && toolResult.result) {
                            // Get original size from stored value or calculate from result
                            if (finalOriginalSize === null) {
                                finalOriginalSize = toolResult.originalResponseSize || null;
                            }
                            // Calculate summarized size from the actual result
                            if (finalSummarizedSize === null) {
                                finalSummarizedSize = new Blob([toolResult.result]).size;
                            }
                            break;
                        }
                    }
        ...
        subChatHeader.innerHTML = `
            ...
            <span class="tool-info">
                ...
                <span class="tool-metric"><i class="fas fa-comments"></i> ${displaySubChat.messages.length}</span>
                ${finalOriginalSize !== null && finalOriginalSize > 0 && finalSummarizedSize !== null && status === 'success' ? `<span class="tool-metric response-size"><i class="fas fa-box"></i> ${finalOriginalSize < 1024 ? `${finalOriginalSize} bytes` : finalOriginalSize < 1024 * 1024 ? `${(finalOriginalSize / 1024).toFixed(1)} KB` : `${(finalOriginalSize / (1024 * 1024)).toFixed(1)} MB`} → ${finalSummarizedSize < 1024 ? `${finalSummarizedSize} bytes` : finalSummarizedSize < 1024 * 1024 ? `${(finalSummarizedSize / 1024).toFixed(1)} KB` : `${(finalSummarizedSize / (1024 * 1024)).toFixed(1)} MB`}</span>` : ''}
                ${toolMetadata.tool_purpose ? `<span class="tool-metric" data-tooltip="${this.escapeHtml(toolMetadata.tool_purpose)}"><i class="fas fa-info-circle"></i> Purpose</span>` : ''}
            </span>
        `;
    ...
    async handleGenerateTitleClick(chatId) {
        ...
        try {
            ...
            const provider = titleProvider || (() => {
                ...
                const p = createLLMProvider(providerApiType, proxyProvider.proxyUrl, modelName, proxyProvider.availableProviders?.[providerType]);
                p.onLog = (logEntry) => {
                    const prefix = logEntry.direction === 'sent' ? 'llm-request' : 'llm-response';
                    const providerName = providerType.charAt(0).toUpperCase() + providerType.slice(1);
                    this.addLogEntry(`${prefix}: ${providerName}`, logEntry);
    ...
    async summarizeConversation(chatId) {
        ...
        try {
            ...
            const provider = window.createLLMProvider(
                ...
                modelName,
                llmProviderConfig.availableProviders?.[providerType]
            );
            provider.onLog = (logEntry) => {
                const prefix = logEntry.direction === 'sent' ? 'llm-request' : 'llm-response';
                const providerName = providerType.charAt(0).toUpperCase() + providerType.slice(1);
