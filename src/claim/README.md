...
## Connecting to Netdata Cloud On-Prem (self-hosted)

Netdata Cloud is also available as a self-hosted **Enterprise On-Prem** deployment that runs entirely within your own infrastructure, with no connection to the Netdata SaaS Cloud Service. For the distinction between the two versions, see [Netdata Cloud Versions](/docs/netdata-cloud/versions.md).

Connecting an Agent or Parent node to an On-Prem Cloud uses the **exact same claiming mechanism** as the SaaS flow above — the Agent-Cloud Link ([ACLK](/src/aclk/README.md)) uses the same challenge/response and MQTT-over-WebSocket protocol. The only change is the **target URL**: point the claim at your own On-Prem Cloud installation instead of the SaaS default.

### Set the claim URL to your On-Prem Cloud

Set the `url` option in `claim.conf` (or the `NETDATA_CLAIM_URL` environment variable) to the base URL of your On-Prem Cloud installation — the address you use to open the Cloud UI in a browser, as configured during your [On-Prem deployment](https://github.com/netdata/netdata-cloud-onprem/blob/master/docs/learn.netdata.cloud/README.md):

```bash
# claim.conf [global] section — the only setting that changes for On-Prem
url = https://onprem.cloud.example.com   # replace with your On-Prem Cloud base URL
```

The remaining options (`token`, `rooms`, `proxy`, `insecure`) are identical to the SaaS claiming methods described above. Use the claiming token and Rooms generated by **your On-Prem Cloud**, not the SaaS Cloud.

### Where the ACLK connects after claiming

Once claimed, the ACLK opens its persistent connection **outbound** to your On-Prem Cloud installation — secure WebSocket (WSS) / MQTT-over-WebSocket on port `443` — and does **not** contact `app.netdata.cloud`, `api.netdata.cloud`, or `mqtt.netdata.cloud`. The Agent discovers its connection endpoints from your On-Prem Cloud, so all ACLK traffic stays within your infrastructure and never reaches Netdata's SaaS services.

The specific domains and ports the Agent must reach are those of **your own On-Prem Cloud installation** (its ingress and MQTT-over-WebSocket service), not the SaaS domains listed elsewhere in this document. See [Allowlisting for Netdata Cloud On-Prem](/src/aclk/README.md#allowlisting-for-netdata-cloud-on-prem) and the [On-Prem deployment documentation](https://github.com/netdata/netdata-cloud-onprem/blob/master/docs/learn.netdata.cloud/README.md) for the exact endpoints your installation exposes.
