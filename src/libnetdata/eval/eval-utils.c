...
void expression_hardcode_variable(EVAL_EXPRESSION *expression, STRING *variable, NETDATA_DOUBLE value) {
    ...
    if (matches) {
        ...
        size_t find1_size = string_strlen(variable) + 2;
        char *find1 = mallocz(find1_size);
        snprintfz(find1, find1_size, "$%s", string2str(variable));
        size_t find1_len = strlen(find1);

        size_t find2_size = string_strlen(variable) + 4;
        char *find2 = mallocz(find2_size);
        ...
        char *buf1 = mallocz(max_buf_size);
        char *buf2 = mallocz(max_buf_size);
