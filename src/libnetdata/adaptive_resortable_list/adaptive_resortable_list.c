...
void arl_begin(ARL_BASE *base)
{
    ...
    if (likely(base->iteration > 10)) {
        ...
        if (unlikely(base->relinkings > (base->expected + base->allocated)))
            netdata_log_info(
                "ARL '%s' has %zu relinkings with %zu expected and %zu allocated entries. Is the source changing so fast?",
                base->name,
                base->relinkings,
                base->expected,
                base->allocated);
        ...
        if (unlikely(base->slow > base->fast))
            netdata_log_info(
                "ARL '%s' has %zu fast searches and %zu slow searches. Is the source really changing so fast?",
                base->name,
                base->fast,
                base->slow);
