...
static char url_percent_escape_decode_legacy(const char *s) {
...
static char url_decode_multibyte_utf8_legacy(const char *s, char *d, const char *d_end) {
    char first_byte = url_percent_escape_decode_legacy(s);
    ...
    while(to_read > 0) {
        char c = url_percent_escape_decode_legacy(s);
...
char *url_decode_r(char *to, const char *url, size_t size) {
    ...
    while(*s && d < e) {
        if(unlikely(*s == '%')) {
            char t = url_percent_escape_decode_legacy(s);
            if(IS_UTF8_BYTE(t)) {
                char bytes_written = url_decode_multibyte_utf8_legacy(s, d, e);
...
int url_unittest(void) {
    ...
    // Non-request callers retain the historical permissive decoder contract.
    if(!url_decode_r(decoded, "%4G", sizeof(decoded)) || strcmp(decoded, "P") != 0)
        errors++;
