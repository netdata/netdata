...
static void send_rejection_ack(int fd, uint16_t status)
{
    ...
    encode_control_header(&ack_hdr, NIPC_CODE_HELLO_ACK, status,
                          sizeof(ack_buf));
                                           ...
                                           nipc_uds_session_t *session)
{
    ...
    nipc_uds_error_t err = nipc_uds_raw_send(
        fd, buf, NIPC_HEADER_LEN + sizeof(hello_buf));
    if (err != NIPC_UDS_OK)
        return err;
                                           ...
                                           nipc_uds_session_t *session)
{
    ...
    uint32_t s_resp_pay = apply_default(cfg->max_response_payload_bytes,
    ...
    nipc_uds_error_t send_ack_err = nipc_uds_raw_send(
        fd, pkt, NIPC_HEADER_LEN + sizeof(ack_buf));
