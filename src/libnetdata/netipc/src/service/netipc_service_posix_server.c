...
void nipc_server_run(nipc_managed_server_t *server)
{
    ...
    while (__atomic_load_n(&server->running, __ATOMIC_RELAXED)) {
        ...
        int pr = nipc_service_posix_poll_with_shutdown(server->listener.fd, &server->running);
        if (pr <= 0)
...
// codeql[cpp/stack-address-escape]
        ...
        int rc = nipc_service_posix_pthread_create(&sctx->thread, NULL,
                                        nipc_service_posix_session_handler_thread, sctx);
        if (rc != 0) {
