...
pub struct JournalRequest {
    ...
    /// Defaulted so capability/`info` probes without a range still deserialize;
    /// the handler substitutes a valid window when `after >= before`.
    #[serde(default)]
    pub after: u32,

    /// Unix timestamp for the end of the time range (seconds). Defaulted for the
