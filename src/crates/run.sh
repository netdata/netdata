...
# Stage 1 (convert once): flatten the protobuf WAL into a flattened WAL. Re-run
# only when the source WAL changes; comment out to iterate on the build alone.
mkdir -p "$FLAT_DIR"
$NG_INDEX --convert --in "$PROTOBUF_WAL" --flat "$FLAT_DIR"

# Stage 2 (the measured step): build a standard SFST index from the flattened WAL.
