...
impl Writer {
    ...
    ///
    /// `machine_id` and `invocation_id` are the producer identities stamped
    /// into every file's `FileId`. They MUST come from the caller (the otel
    /// path resolves them from `NETDATA_REGISTRY_UNIQUE_ID` /
    /// `NETDATA_INVOCATION_ID`); a nil UUID is rejected as a defense against a
    /// supervisor that failed to inject them before sending the config to a
    /// worker. The caller provides a shared sequence counter (e.g., shared
    /// across per-tenant writers). The directory is created if it doesn't exist.
    pub fn new(
        path: &Path,
        config: Config,
        seq: Arc<SeqAllocator>,
        ...
        if machine_id.is_nil() || invocation_id.is_nil() {
            return Err(crate::Error::InvalidHeader(
                "machine_id/invocation_id must not be the nil UUID; the supervisor \
                 must inject NETDATA_REGISTRY_UNIQUE_ID / NETDATA_INVOCATION_ID"
                    .to_string(),
            ));
        }
...
mod tests {
    ...
    fn filename_embeds_passed_identity() {
        // The caller-supplied machine_id/invocation_id MUST land in the FileId
        // stem so the file is attributable to this node and invocation. The
        // 32-hex simple form is the filename's only copy of these values.
        let tmp = tempfile::tempdir().unwrap();
        let (machine_id, invocation_id) = crate::test_identity();
        let seq = Arc::new(SeqAllocator::ephemeral(0));
        ...
        // The WAL filename is `{machine:32hex}-{invocation:32hex}-{...}.wal`.
        ...
        let invocation_hex = invocation_id.as_simple().to_string();
        assert!(
            wal_name.starts_with(&format!("{machine_hex}-{invocation_hex}-")),
