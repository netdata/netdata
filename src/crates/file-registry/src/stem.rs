//! Shared filename-stem codec for the `{machine}-{boot}-...` shape.
//!
...
/// Format the `{machine}-{boot}` prefix (two simple-form UUIDs).
pub fn format_uuid_pair(machine_id: Uuid, boot_id: Uuid) -> String {
    format!("{}-{}", machine_id.as_simple(), boot_id.as_simple())
}
