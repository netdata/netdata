...
fn sfst_prefix_has_trailing_slash() {
    assert_eq!(
        ...
        "v1/logs/tenants/tenant1/sfst/2026-04-17/",
    );
}
...
fn catalog_key_is_versioned_signal_catalog_date_tenant() {
    ...
    assert!(key.starts_with("v1/logs/catalog/2026-04-17/tenant1/"));
    assert!(key.ends_with(".catalog"));
}

#[test]
fn parse_sfst_date_happy_path() {
    let key = "v1/logs/tenants/tenant1/sfst/2026-04-17/abc123.sfst";
    assert_eq!(parse_sfst_date(key), Some(sample_date()));
}
...
fn parse_sfst_date_rejects_unknown_shapes() {
    ...
    // Truncated.
    assert!(parse_sfst_date("v1/logs/tenants/tenant1/sfst").is_none());
    // Date doesn't parse.
    assert!(parse_sfst_date("v1/logs/tenants/tenant1/sfst/not-a-date/x").is_none());
}
