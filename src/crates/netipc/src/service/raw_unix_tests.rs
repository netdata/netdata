...
#[test]
fn test_lookup_zero_item_calls() {
    let apps_svc = unique_service("rs_svc_apps_lookup_zero");
    let mut apps_server = TestServer::start(
        &apps_svc,
        METHOD_APPS_LOOKUP,
        Some(test_apps_lookup_dispatch()),
    );
    ...
    assert_eq!(apps_view.generation, 0);
    assert_eq!(apps_client.status().call_count, 1);
    apps_client.close();
    apps_server.stop();
    cleanup_all(&apps_svc);

    let cgroups_svc = unique_service("rs_svc_cgroups_lookup_zero");
...
fn test_cgroups_lookup_transparent_payload_exceeded_retry() {
    ...
    let mut server = TestServer::start_with(&svc, cfg, METHOD_CGROUPS_LOOKUP, Some(handler), 8);
    let mut client = cgroups_lookup_client(&svc, client_config());
    connect_ready(&mut client);
...
#[test]
fn test_apps_lookup_transparent_payload_exceeded_retry() {
    let svc = unique_service("rs_svc_apps_lookup_scale");
    let mut cfg = server_config();
    ...
    let mut server = TestServer::start_with(&svc, cfg, METHOD_APPS_LOOKUP, Some(handler), 8);
    let mut client = apps_lookup_client(&svc, client_config());
    connect_ready(&mut client);
...
#[test]
fn test_lookup_timeout_during_followup_subcall() {
    let svc = unique_service("rs_svc_apps_lookup_followup_timeout");
    let mut cfg = server_config();
    cfg.max_response_payload_bytes = 320;
    let calls = Arc::new(std::sync::atomic::AtomicU32::new(0));
    let handler_calls = calls.clone();
    ...
    let svc = unique_service("rs_svc_cgroups_lookup_followup_timeout");
    let mut cfg = server_config();
...
#[test]
fn test_lookup_abort_during_followup_subcall() {
    let svc = unique_service("rs_svc_apps_lookup_followup_abort");
    let mut cfg = server_config();
    cfg.max_response_payload_bytes = 320;
    let calls = Arc::new(std::sync::atomic::AtomicU32::new(0));
    let handler_calls = calls.clone();
    ...
    let svc = unique_service("rs_svc_cgroups_lookup_followup_abort");
    let mut cfg = server_config();
    cfg.max_response_payload_bytes = 160;
    let calls = Arc::new(std::sync::atomic::AtomicU32::new(0));
    let handler_calls = calls.clone();
...
fn run_apps_lookup_request_boundary_case(
    request_cap: u32,
    expected_max_items: u32,
    min_calls: u32,
    label: &str,
) {
    let svc = unique_service(&format!("rs_svc_apps_lookup_request_split_{label}"));
    let mut cfg = server_config();
...
fn run_cgroups_lookup_request_boundary_case(
    request_cap: u32,
    expected_max_items: u32,
    min_calls: u32,
    label: &str,
) {
    let svc = unique_service(&format!("rs_svc_cgroups_lookup_request_split_{label}"));
    let mut cfg = server_config();
    cfg.max_request_payload_bytes = request_cap;
    cfg.max_response_payload_bytes = 4096;
    let calls = Arc::new(std::sync::atomic::AtomicU32::new(0));
...
#[test]
fn test_lookup_proactive_request_split() {
    run_apps_lookup_request_boundary_case(63, 2, 4, "cap_minus_1");
    run_apps_lookup_request_boundary_case(64, 3, 3, "cap_exact");
    run_apps_lookup_request_boundary_case(65, 3, 3, "cap_plus_1");

    run_cgroups_lookup_request_boundary_case(47, 1, 5, "cap_minus_1");
    run_cgroups_lookup_request_boundary_case(48, 2, 3, "cap_exact");
    run_cgroups_lookup_request_boundary_case(49, 2, 3, "cap_plus_1");
}

#[test]
fn test_cgroups_lookup_oversized_request_key() {
    let svc = unique_service("rs_svc_cgroups_lookup_oversized_request_key");
    let mut cfg = server_config();
...
fn assert_apps_lookup_bad_response_unix(
    name: &str,
    pids: &[u32],
    payload: Vec<u8>,
    want: NipcError,
) {
    let svc = unique_service(&format!("rs_unix_apps_lookup_bad_{name}"));
...
fn assert_cgroups_lookup_bad_response_unix(
    name: &str,
    paths: &[&[u8]],
    payload: Vec<u8>,
    want: NipcError,
) {
    let svc = unique_service(&format!("rs_unix_cgroups_lookup_bad_{name}"));
    let mut server = start_raw_session_server(&svc, server_config(), move |session, req_hdr, _| {
        let mut resp_hdr = Header {
            kind: KIND_RESPONSE,
...
#[test]
fn test_lookup_request_capacity_reconnects() {
    let svc = unique_service("rs_svc_apps_lookup_request_capacity");
    let mut cfg = server_config();
    ...
    let svc = unique_service("rs_svc_cgroups_lookup_request_capacity");
    let mut cfg = server_config();
    cfg.max_request_payload_bytes = 256;
    let mut server = TestServer::start_with(
        &svc,
...
#[test]
fn test_raw_call_overflow_no_growth_stops_bounded() {
    let svc = unique_service("rs_svc_overflow_no_growth");
    let mut scfg = server_config();
...
fn run_large_apps_lookup(prefix: &str, item_count: usize) {
...
fn run_large_cgroups_lookup(prefix: &str, item_count: usize) {
    let svc = unique_service(prefix);
...
#[test]
fn test_lookup_large_logical_calls() {
    run_large_apps_lookup("rs_svc_apps_lookup_large_8192", LOOKUP_TOPOLOGY_SCALE_ITEMS);
    run_large_apps_lookup("rs_svc_apps_lookup_large_32768", LOOKUP_HPC_SCALE_ITEMS);
    run_large_cgroups_lookup(
        "rs_svc_cgroups_lookup_large_8192",
        LOOKUP_TOPOLOGY_SCALE_ITEMS,
    );
    run_large_cgroups_lookup("rs_svc_cgroups_lookup_large_32768", LOOKUP_HPC_SCALE_ITEMS);
}

fn run_large_apps_response_split(prefix: &str) {
    let svc = unique_service(prefix);
...
fn run_large_cgroups_response_split(prefix: &str) {
    let svc = unique_service(prefix);
...
#[test]
fn test_lookup_large_response_split_calls() {
    run_large_apps_response_split("rs_svc_apps_lookup_large_response_split");
    run_large_cgroups_response_split("rs_svc_cgroups_lookup_large_response_split");
}

#[test]
fn test_lookup_logical_limits() {
    ...
    assert_eq!(
        client.call_apps_lookup(&[1234]).unwrap_err(),
        crate::protocol::NipcError::Overflow
    );
    client.close();
    server.stop();
    cleanup_all(&svc);

    let svc = unique_service("rs_svc_cgroups_lookup_response_limit");
    let mut server = TestServer::start(
        &svc,
        METHOD_CGROUPS_LOOKUP,
        Some(test_cgroups_lookup_dispatch()),
    ...
    let svc = unique_service("rs_svc_apps_lookup_logical_subcall_limit");
    let mut cfg = server_config();
    cfg.max_request_payload_bytes = 48;
    let mut server = TestServer::start_with(
        &svc,
    ...
    assert_eq!(
        client
            ...
            .unwrap_err(),
        crate::protocol::NipcError::BadLayout
    );
}

#[test]
fn test_cgroups_lookup_rejects_mixed_generation_retry() {
    let svc = unique_service("rs_svc_cgroups_lookup_generation");
    let mut cfg = server_config();
    cfg.max_response_payload_bytes = 160;
    let calls = Arc::new(std::sync::atomic::AtomicU32::new(0));
    let handler_calls = calls.clone();
...
#[test]
fn test_apps_lookup_rejects_mixed_generation_retry() {
    let svc = unique_service("rs_svc_apps_lookup_generation");
    let mut cfg = server_config();
    cfg.max_response_payload_bytes = 320;
    let calls = Arc::new(std::sync::atomic::AtomicU32::new(0));
    let handler_calls = calls.clone();
...
#[test]
fn test_lookup_rejects_malformed_followup_response_unix() {
    let svc = unique_service("rs_unix_apps_lookup_bad_followup");
    let calls = Arc::new(std::sync::atomic::AtomicU32::new(0));
    let handler_calls = calls.clone();
    let handler = apps_lookup_dispatch(Arc::new(move |req, builder| {
    ...
    let mut server = TestServer::start(&svc, METHOD_APPS_LOOKUP, Some(handler));
    let mut client = apps_lookup_client(&svc, client_config());
    connect_ready(&mut client);
    ...
    let svc = unique_service("rs_unix_cgroups_lookup_bad_followup");
    let calls = Arc::new(std::sync::atomic::AtomicU32::new(0));
    let handler_calls = calls.clone();
    let handler = cgroups_lookup_dispatch(Arc::new(move |req, builder| {
        let call = handler_calls.fetch_add(1, Ordering::SeqCst) + 1;
    ...
    let mut server = TestServer::start(&svc, METHOD_CGROUPS_LOOKUP, Some(handler));
    let mut client = cgroups_lookup_client(&svc, client_config());
    connect_ready(&mut client);
...
#[test]
fn test_lookup_endpoint_gone_after_partial_progress_unix() {
    let svc = unique_service("rs_unix_apps_lookup_partial_disconnect");
    ...
    let svc = unique_service("rs_unix_cgroups_lookup_partial_disconnect");
    let mut server = start_raw_session_server(&svc, server_config(), |session, hdr, payload| {
...
#[test]
fn test_lookup_endpoint_gone_before_first_subcall_unix() {
    let svc = unique_service("rs_unix_apps_lookup_gone_before_call");
    let mut server = TestServer::start(&svc, METHOD_APPS_LOOKUP, Some(test_apps_lookup_dispatch()));
    let mut client = apps_lookup_client(&svc, client_config());
    connect_ready(&mut client);
    server.stop();
    ...
    let svc = unique_service("rs_unix_cgroups_lookup_gone_before_call");
    let mut server = TestServer::start(
        &svc,
        METHOD_CGROUPS_LOOKUP,
        Some(test_cgroups_lookup_dispatch()),
...
#[test]
fn test_lookup_endpoint_absent_before_call_unix() {
    let svc = unique_service("rs_unix_apps_lookup_absent");
    cleanup_all(&svc);
    let mut client = apps_lookup_client(&svc, client_config());
    ...
    let svc = unique_service("rs_unix_cgroups_lookup_absent");
    cleanup_all(&svc);
    let mut client = cgroups_lookup_client(&svc, client_config());
    assert!(
        client.refresh(),
...
#[test]
fn test_lookup_rejects_malformed_typed_responses_unix() {
    assert_apps_lookup_bad_response_unix("truncated", &[1234], vec![1, 2, 3], NipcError::Truncated);
    assert_apps_lookup_bad_response_unix(
        "count",
        &[1234],
    ...
    assert_apps_lookup_bad_response_unix("invalid_status", &[1234], payload, NipcError::BadLayout);
    let mut payload = apps_lookup_response_payload_known_with_label(1234);
    patch_lookup_item_status(
        &mut payload,
        APPS_LOOKUP_RESP_HDR_SIZE,
    ...
    assert_cgroups_lookup_bad_response_unix(
        "truncated",
        &[b"/a"],
        vec![1, 2, 3],
        NipcError::Truncated,
...
#[test]
fn test_batch_dispatch_builder_overflow_respects_configured_ceiling() {
    ...
    let err = client
        .call_increment_batch(&[10, 20])
        .expect_err("batch builder overflow should respect the configured ceiling");
    assert_eq!(err, NipcError::Overflow);
    assert!(
        !client.ready(),
        "client should leave READY after capped overflow"
    );
    assert!(
        client.status().reconnect_count >= 1,
        "capped overflow should reconnect at least once"
    );
