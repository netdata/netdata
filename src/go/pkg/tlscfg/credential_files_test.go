...
func TestTLSReaderRequired(t *testing.T) {
	_, err := NewTLSConfigWithReader(context.Background(), TLSConfig{TLSCA: "synthetic-ca"}, nil)
...
func TestTLSParserDoesNotDisclosePEMLabels(t *testing.T) {
	...
	_, err := NewTLSConfigWithReader(context.Background(), TLSConfig{TLSCert: path, TLSKey: path}, credentialfiletest.New(t))
	require.ErrorIs(t, err, ErrTLSFile)
