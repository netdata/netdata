...
// AnalysisStats reports deterministic work performed by an analysis.
type AnalysisStats struct {
	States      int
	Transitions int
}

// GlobIntersectionOptions controls GlobIntersectionWitness.
type GlobIntersectionOptions struct {
	Exclude         []string
...
// matched by both ordered simple-pattern expressions. Earlier negative terms
// retain the same first-match precedence as NewSimplePatternsMatcher.
func SimplePatternIntersectionWitness(
	ctx context.Context,
	leftExpr string,
	rightExpr string,
	requireNonEmpty bool,
...
func (m *analysisMeter) addState() error {
	if m.stats.States >= m.budget.MaxStates {
...
func (m *analysisMeter) addTransition() error {
	if m.stats.Transitions >= m.budget.MaxTransitions {
