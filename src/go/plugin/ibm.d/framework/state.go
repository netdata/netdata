...
// SetGlobal stores a global metric (no labels)
func (s *CollectorState) SetGlobal(ctx interface{}, values map[string]int64) {
	// Pass nil labels for global metrics
	s.Set(ctx, nil, values)
}
...
func structToMap(labels interface{}) map[string]string {
	...
	// This will handle QueueLabels{Queue: "DEV", Type: "local"}
