...
func (c *Client) SendPCFCommand(command C.MQLONG, parameters []pcfParameter) ([]byte, error) {
	...
	c.protocol.Debugf("Executing MQPUT for command %s to queue manager '%s' (message size: %d bytes)", 
	...
	c.protocol.Debugf("MQPUT successful for command %s to queue manager '%s'", mqcmdToString(command), c.config.QueueManager)
...
func (c *Client) getPCFResponse(requestMd *C.MQMD, hReplyObj C.MQHOBJ) ([]byte, error) {
	...
	c.protocol.Debugf("Waiting for response from queue manager '%s'", c.config.QueueManager)
	...
	for {
		...
		c.protocol.Debugf("Attempting MQGET (message #%d) from queue manager '%s' (wait interval: %dms)", 
		...
		if reason == C.MQRC_NO_MSG_AVAILABLE {
			...
			c.protocol.Debugf("No more messages available from queue manager '%s' (collected %d messages)", 
		...
		if reason != C.MQRC_TRUNCATED_MSG_FAILED {
			...
			if len(allResponses) == 0 {
				c.protocol.Errorf("MQGET length check failed for queue manager '%s' - completion code %d, reason %d (%s)", 
			...
			c.protocol.Warningf("MQGET failed for subsequent message from queue manager '%s' - completion code %d, reason %d (%s)", 
				c.config.QueueManager, compCode, reason, mqReasonString(int32(reason)))
			break
		}
		...
		c.protocol.Debugf("Response message size is %d bytes, allocating buffer", bufferLength)
		...
		if compCode != C.MQCC_OK {
			...
			if len(allResponses) == 0 {
				c.protocol.Errorf("MQGET data failed for queue manager '%s' - completion code %d, reason %d (%s)", 
					c.config.QueueManager, compCode, reason, mqReasonString(int32(reason)))
				return nil, fmt.Errorf("MQGET failed: completion code %d, reason %d (%s)", compCode, reason, mqReasonString(int32(reason)))
			}
			c.protocol.Warningf("MQGET data failed for subsequent message from queue manager '%s' - completion code %d, reason %d (%s)", 
				c.config.QueueManager, compCode, reason, mqReasonString(int32(reason)))
			break
		}
		
		c.protocol.Debugf("Successfully received message #%d (%d bytes) from queue manager '%s'", 
	...
	if len(allResponses) == 0 {
		c.protocol.Errorf("No response received from queue manager '%s' after waiting", c.config.QueueManager)
		return nil, fmt.Errorf("no PCF response received")
	}
	
	c.protocol.Debugf("Successfully collected %d total bytes of response data from queue manager '%s'", 
