...
// MQ Queue File Size constants (IBM MQ 9.1.5+)
const (
	MQIACF_CUR_Q_FILE_SIZE     = int32(C.MQIACF_CUR_Q_FILE_SIZE)
	MQIACF_CUR_MAX_FILE_SIZE   = int32(C.MQIACF_CUR_MAX_FILE_SIZE)
)

// MQ Channel Extended Metrics constants
const (
	MQIACH_BUFFERS_SENT         = int32(C.MQIACH_BUFFERS_SENT)
	MQIACH_BUFFERS_RCVD         = int32(C.MQIACH_BUFFERS_RCVD)
	MQIACH_CURRENT_MSGS         = int32(C.MQIACH_CURRENT_MSGS)
	MQIACH_XMITQ_TIME_INDICATOR = int32(C.MQIACH_XMITQ_TIME_INDICATOR)
	MQIACH_MCA_STATUS           = int32(C.MQIACH_MCA_STATUS)
	MQIACH_INDOUBT_STATUS       = int32(C.MQIACH_INDOUBT_STATUS)
	MQIACH_SSL_KEY_RESETS       = int32(C.MQIACH_SSL_KEY_RESETS)
	MQIACH_NPM_SPEED            = int32(C.MQIACH_NPM_SPEED)
	MQIACH_CURRENT_SHARING_CONVS = int32(C.MQIACH_CURRENT_SHARING_CONVS)
)

// MQ Subscription PCF commands
const (
	MQCMD_INQUIRE_SUBSCRIPTION = int32(C.MQCMD_INQUIRE_SUBSCRIPTION)
	MQCMD_INQUIRE_SUB_STATUS   = int32(C.MQCMD_INQUIRE_SUB_STATUS)
)

// MQ Subscription attributes
const (
	MQCACF_SUB_NAME        = int32(C.MQCACF_SUB_NAME)
	MQCA_TOPIC_STRING      = int32(C.MQCA_TOPIC_STRING)
	MQIACF_SUB_TYPE        = int32(C.MQIACF_SUB_TYPE)
	MQIACF_MESSAGE_COUNT   = int32(C.MQIACF_MESSAGE_COUNT)
	MQCACF_LAST_MSG_DATE   = int32(C.MQCACF_LAST_MSG_DATE)
	MQCACF_LAST_MSG_TIME   = int32(C.MQCACF_LAST_MSG_TIME)
)
