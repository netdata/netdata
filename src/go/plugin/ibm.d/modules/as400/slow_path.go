...
func (c *Collector) fetchMessageQueues(ctx context.Context, beat time.Time, do queryFunc) (messageQueueSnapshot, error) {
	...
	for _, target := range c.messageQueueTargets {
		...
		err := do(ctx, queryName, buildMessageQueueQuery(target, c.supportsMessageQueueTableFunction()), func(column, value string, lineEnd bool) {
			switch column {
			case "MESSAGE_COUNT":
...
func (c *Collector) fetchJobQueues(ctx context.Context, beat time.Time, do queryFunc) (jobQueueSnapshot, error) {
	...
	for _, target := range c.jobQueueTargets {
		...
		err := do(ctx, queryName, buildJobQueueQuery(target), func(column, value string, lineEnd bool) {
			switch column {
...
func (c *Collector) fetchOutputQueues(ctx context.Context, beat time.Time, do queryFunc) (outputQueueSnapshot, error) {
	...
	for _, target := range c.outputQueueTargets {
		...
		err := do(ctx, queryName, buildOutputQueueEntriesQuery(target), func(column, value string, lineEnd bool) {
			if lineEnd {
				entriesCount++
			}
		})
		...
		viewErr := do(ctx, queryName+"_view", buildOutputQueueInfoQuery(target), func(column, value string, lineEnd bool) {
			switch column {
