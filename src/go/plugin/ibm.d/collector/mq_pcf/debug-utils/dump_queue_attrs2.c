// dump_queue_attrs2.c - Simplified version using permanent reply queue
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <cmqc.h>
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        printf("Usage: %s <queue_manager> <queue_name>\n", argv[0]);
        return 1;
    }
    ...
    if (compCode != MQCC_OK) {
        printf("MQCONN failed: CompCode=%d, Reason=%d\n", compCode, reason);
        return 1;
    }
    ...
    // Create and open a temporary queue
    memset(&od, 0, sizeof(od));
    od.Version = MQOD_VERSION_1;
    strcpy(od.ObjectName, "SYSTEM.DEFAULT.MODEL.QUEUE");
    strcpy(od.DynamicQName, "PCF.*");
    od.ObjectType = MQOT_Q;
    MQOPEN(hConn, &od, MQOO_INPUT_AS_Q_DEF | MQOO_INQUIRE, &hReplyQ, &compCode, &reason);
    if (compCode != MQCC_OK) {
        printf("MQOPEN reply queue failed: CompCode=%d, Reason=%d\n", compCode, reason);
        // Try without dynamic queue
        memset(&od, 0, sizeof(od));
        od.Version = MQOD_VERSION_1;
        strcpy(od.ObjectName, "SYSTEM.ADMIN.EVENT");
        MQOPEN(hConn, &od, MQOO_INPUT_AS_Q_DEF, &hReplyQ, &compCode, &reason);
        if (compCode != MQCC_OK) {
            printf("MQOPEN fallback failed: CompCode=%d, Reason=%d\n", compCode, reason);
            MQCLOSE(hConn, &hObj, MQCO_NONE, &compCode, &reason);
            MQDISC(&hConn, &compCode, &reason);
            return 1;
        }
        printf("Using fallback reply queue\n");
    } else {
        // Get the actual name of the dynamic queue
        printf("Created dynamic reply queue: %.48s\n", od.ObjectName);
