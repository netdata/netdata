// Test MQCMD_INQUIRE_TOPIC with Queue Manager CCSID (original failing code)
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <cmqc.h>
...
int main() {
    ...
    // Send command - TEST Queue Manager CCSID (original failing code)
    strcpy(md.Format, MQFMT_ADMIN);
    md.MsgType = MQMT_REQUEST;
    md.CodedCharSetId = MQCCSI_Q_MGR;  // Queue Manager CCSID - ORIGINAL FAILING CODE
    md.Encoding = MQENC_NATIVE;
    
    printf("Testing MQCMD_INQUIRE_TOPIC with Queue Manager CCSID (MQCCSI_Q_MGR)...\n");
    ...
    if (compCode != MQCC_OK) {
        ...
        if (reason == 2115) {
            printf("MQRC_2115 (TARGET_CCSID_ERROR) - This confirms the original issue!\n");
        } else {
            printf("Different error than expected MQRC_2115\n");
        }
    } else {
        printf("MQPUT successful! Queue Manager CCSID worked (unexpected).\n");
    }
