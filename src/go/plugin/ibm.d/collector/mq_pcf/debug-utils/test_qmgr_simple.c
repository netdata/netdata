// Test simple MQCMD_INQUIRE_Q_MGR to verify PCF setup works
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <cmqc.h>
...
int main() {
    ...
    strcpy(replyOd.DynamicQName, "QMGR.REPLY.*");
    ...
    // Test simple MQCMD_INQUIRE_Q_MGR command
    printf("\n=== Test: MQCMD_INQUIRE_Q_MGR ===\n");
    
    // Create PCF command message
    char buffer[4096];
    MQCFH *cfh = (MQCFH *)buffer;
    
    // PCF header - no parameters needed for INQUIRE_Q_MGR
    cfh->Type = MQCFT_COMMAND;
    cfh->StrucLength = MQCFH_STRUC_LENGTH;
    cfh->Version = MQCFH_VERSION_1;
    cfh->Command = MQCMD_INQUIRE_Q_MGR;
    cfh->MsgSeqNumber = 1;
    cfh->Control = MQCFC_LAST;
    cfh->ParameterCount = 0;  // No parameters for basic Q_MGR inquiry
    ...
    MQLONG msgSize = MQCFH_STRUC_LENGTH;
    ...
    if (respCfh->CompCode == MQCC_OK) {
        printf("✅ SUCCESS: MQCMD_INQUIRE_Q_MGR worked! PCF setup is correct.\n");
    } else {
        printf("❌ FAILED: MQCMD_INQUIRE_Q_MGR returned CompCode=%d, Reason=%d\n", 
