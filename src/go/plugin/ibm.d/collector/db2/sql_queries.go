...
const (
	...
	queryMonGetDatabase = `
		SELECT 
			SUM(TOTAL_APP_COMMITS) as COMMITS,
			SUM(TOTAL_APP_ROLLBACKS) as ROLLBACKS,
	...
	queryMonGetTransactionLog = `
		SELECT 
			LOG_USED_KB * 1024 as TOTAL_LOG_USED,
			LOG_AVAILABLE_KB * 1024 as TOTAL_LOG_AVAILABLE,
			CASE 
				WHEN (LOG_USED_KB + LOG_AVAILABLE_KB) > 0 
				THEN (LOG_USED_KB * 100) / (LOG_USED_KB + LOG_AVAILABLE_KB)
				ELSE 0 
			END as LOG_UTILIZATION,
			NUM_LOG_READ_IO as LOG_READS,
			NUM_LOG_WRITE_IO as LOG_WRITES
