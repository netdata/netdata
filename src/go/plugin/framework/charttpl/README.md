...
> `autogen.exclude` at the profile root to suppress fallback charts while retaining samples; use the job `selector` or
> a relabeling `drop` rule when the samples themselves must be discarded.
...
| `autogen.exclude`                     | array[string] | _(empty)_   | Suppress fallback charts for unmatched source families matching any positive glob.     |
...
`autogen.exclude` is evaluated only after every authored dimension selector has had a chance to route the series.
Patterns are positive globs combined with OR semantics; negative (`!`) terms are rejected. Histogram and summary
components match by their source family (`foo`, not `foo_bucket`/`foo_sum`/`foo_count`), and MeasureSet fields match
their source name (`foo`, not `foo_<field>`). Therefore, an authored heatmap for `foo_bucket` still materializes when
`foo` is excluded, while unmatched `foo_sum` and `foo_count` fallback charts are suppressed. Exclusion changes chart
generation only; the series remains in the metric store.
