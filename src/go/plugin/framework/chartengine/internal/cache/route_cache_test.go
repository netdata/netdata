...
func TestRouteCache(t *testing.T) {
	tests := map[string]struct {
		...
		"stores positive and negative entries": {
			run: func(t *testing.T) {
		...
		"revision mismatch does not retain stale entry in same build": {
			run: func(t *testing.T) {
				rc := NewRouteCache[string]()
		...
		"retain prunes by seen build sequence": {
			run: func(t *testing.T) {
				rc := NewRouteCache[string]()
		...
		"retain full-drops when no entries seen in build": {
			run: func(t *testing.T) {
				rc := NewRouteCache[string]()
