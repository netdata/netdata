...
1. **Prepare outside publication.** Provider construction, configuration, and `Init` always run as a process-owned
   ...
   - Vault performs one real authenticated `GET /v1/auth/token/lookup-self` through the same token, namespace, client,
     TLS/proxy, timeout, and cancellation path used for secret resolution. HTTP 200 is operational success. An exact
     permission-only HTTP 403 returns `dyncfg.ErrTestUnsupported` and becomes explicit validation-only success because the
     response cannot reliably prove token validity across Vault versions and authentication restrictions. Invalid-token,
     ambiguous, malformed, oversized, and unexpected responses fail. The Test reads no secret and does not prove
     secret-path permission or that an intermediary preserved the namespace header. It buffers at most 1 MiB of a
     classified 403 body. The request creates Vault audit/activity evidence and can consume the final use of a limited-use
     service token. Vault token files use the shared regular-file-only 1 MiB configured-file reader.
