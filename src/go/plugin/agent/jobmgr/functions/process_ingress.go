...
func (pi *ProcessIngress) observeReadReturn() error {
	pi.mu.Lock()
	observer := pi.readObserver
	census := pi.censusLocked()
	pi.mu.Unlock()
	if observer == nil {
		return nil
	}
