...
func (ai adoptionInjection) afterIncumbentStops() bool {
	return ai == injectRuntimeBusy || ai == injectRuntimeQuarantined || ai == injectStartupFailure
}

// TestDynCfgJobRepliesStateAdoption drives collector-job DynCfg commands
...
// are unchanged. restart is not saved: it answers 2xx only when a job runs.
func TestDynCfgJobRepliesStateAdoption(t *testing.T) {
	tests := map[string]struct {
		...
		"update running succeeds":                      {command: dyncfg.CommandUpdate, status: dyncfg.StatusRunning, wantCode: 200},
		...
		"update running with a retried check failure":  {command: dyncfg.CommandUpdate, status: dyncfg.StatusRunning, inject: injectCheckUnclassified, retry: 1, wantCode: 202},
		...
		"update running with a retried missing vnode":  {command: dyncfg.CommandUpdate, status: dyncfg.StatusRunning, inject: injectMissingVnode, retry: 1, wantCode: 202},
		"update running with a busy runtime":           {command: dyncfg.CommandUpdate, status: dyncfg.StatusRunning, inject: injectRuntimeBusy, wantCode: 202},
		"update running with a quarantined runtime":    {command: dyncfg.CommandUpdate, status: dyncfg.StatusRunning, inject: injectRuntimeQuarantined, wantCode: 202},
		...
		"enable disabled succeeds":                     {command: dyncfg.CommandEnable, status: dyncfg.StatusDisabled, wantCode: 200},
		"enable disabled with a check failure":         {command: dyncfg.CommandEnable, status: dyncfg.StatusDisabled, inject: injectCheckUnclassified, wantCode: 422},
		...
		"restart running succeeds":                     {command: dyncfg.CommandRestart, status: dyncfg.StatusRunning, wantCode: 200},
		...
		"restart running with a busy runtime":          {command: dyncfg.CommandRestart, status: dyncfg.StatusRunning, inject: injectRuntimeBusy, wantCode: 503},
		...
		"add with a missing vnode":                     {command: dyncfg.CommandAdd, inject: injectMissingVnode, wantCode: 503},
...
func requireAdoptionHoldsRequest(
	...
	case dyncfg.CommandAdd, dyncfg.CommandUpdate:
		...
		want := map[bool][]string{
			...
			false: {dyncfg.StatusRunning.String(), dyncfg.StatusFailed.String()},
	...
	if inject == injectStartupFailure {
		configureRuntimeTestCollector(controller, func() *runtimeTestCollector {
			return &runtimeTestCollector{
				run: func(context.Context, func()) error { return errors.New("listen: address in use") },
			}
		})
		return
	}
...
func injectAdoptionAttemptFailure(controller *DynCfgJobController, inject adoptionInjection) {
	...
	case injectRuntimeQuarantined:
		namespace, err = jobmgr.ProcessAttemptJobRuntime, jobmgr.ErrProcessAttemptQuarantined
	default:
		return
	}
