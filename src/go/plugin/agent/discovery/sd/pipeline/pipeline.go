...
func New(cfg Config, makeDiscoverers DiscovererFactory) (*Pipeline, error) {
	if err := ValidateConfig(cfg); err != nil {
		return nil, err
	}
	if makeDiscoverers == nil {
		return nil, errors.New("discoverer factory is not set")
	}
	...
	if err := p.registerDiscoverers(cfg, makeDiscoverers); err != nil {
		return nil, err
	}
...
func (p *Pipeline) registerDiscoverers(conf Config, makeDiscoverers DiscovererFactory) error {
	discs, err := makeDiscoverers(conf.Discoverer, conf.Source)
	if err != nil {
		return fmt.Errorf("failed to create %q discoverer: %w", conf.Discoverer.Type(), err)
	}
	p.discoverers = append(p.discoverers, discs...)
