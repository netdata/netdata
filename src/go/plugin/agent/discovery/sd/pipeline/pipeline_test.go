...
func Test_defaultConfigs(t *testing.T) {
	dir := "../../../../go.d/config/go.d/sd/"
	entries, err := os.ReadDir(dir)
	require.NoError(t, err)

	require.NotEmpty(t, entries)
	factory := func(_ DiscovererPayload, _ string) ([]model.Discoverer, error) {
		return []model.Discoverer{newMockDiscoverer("", newMockTargetGroup("test"))}, nil
	}
...
func TestNew(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			_, err = New(cfg, func(_ DiscovererPayload, _ string) ([]model.Discoverer, error) {
				return []model.Discoverer{newMockDiscoverer("", newMockTargetGroup("test"))}, nil
			})
