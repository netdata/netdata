...
- **Direct journal storage**: Enabled by default for explicit jobs. Stores traps as local direct journal files and exposes the embedded `snmp_traps:logs` Function. Direct-journal jobs appear as `__logs_sources` options.
- **OTLP/gRPC export**: Optional backend that exports traps as OTLP LogRecords. It can run together with direct journal storage, or as the only output when `journal.enabled` is `false`.
...
- `enabled`: Write traps to local direct journal files and expose the job as a `__logs_sources` option in the embedded `snmp_traps:logs` Function.
...
- `enabled`: Enable OTLP/gRPC Logs export. This can run together with direct journal storage or as the only output when `journal.enabled` is `false`.
