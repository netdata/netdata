...
func (c *Collector) collectNVMeDevice(mx map[string]int64, devicePath string) error {
	...
	mx["device_"+dev+"_critical_warning_available_spare"] = oldmetrix.Bool(parseValue(stats.CriticalWarningValue)&1 != 0)
	mx["device_"+dev+"_critical_warning_temp_threshold"] = oldmetrix.Bool(parseValue(stats.CriticalWarningValue)&(1<<1) != 0)
	mx["device_"+dev+"_critical_warning_nvm_subsystem_reliability"] = oldmetrix.Bool(parseValue(stats.CriticalWarningValue)&(1<<2) != 0)
	mx["device_"+dev+"_critical_warning_read_only"] = oldmetrix.Bool(parseValue(stats.CriticalWarningValue)&(1<<3) != 0)
	mx["device_"+dev+"_critical_warning_volatile_mem_backup_failed"] = oldmetrix.Bool(parseValue(stats.CriticalWarningValue)&(1<<4) != 0)
	mx["device_"+dev+"_critical_warning_persistent_memory_read_only"] = oldmetrix.Bool(parseValue(stats.CriticalWarningValue)&(1<<5) != 0)
