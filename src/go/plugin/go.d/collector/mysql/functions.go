...
func (c *Collector) buildMySQLDynamicSQL(cols []mysqlColumnMeta, sortColumn string, limit int) string {
	...
	for _, col := range cols {
		if col.isPicoseconds {
			...
			selectParts = append(selectParts, fmt.Sprintf("%s/1000000000 AS %s", col.dbColumn, col.dbColumn))
		} else if col.dbColumn == "SCHEMA_NAME" {
			selectParts = append(selectParts, fmt.Sprintf("IFNULL(%s, '') AS %s", col.dbColumn, col.dbColumn))
		} else {
			selectParts = append(selectParts, col.dbColumn)
		}
	}
