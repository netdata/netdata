...
Three collector-activity charts expose the inputs behind that cost model per resolved account and region. **CloudWatch API Calls** counts collector-issued `ListMetrics` and `GetMetricData` calls, including every pagination page. **CloudWatch Metric Requests** counts the calculated `GetMetricData` billing units submitted on those pages, using AWS's up-to-five-statistics grouping. **CloudWatch Raw Queries** counts submitted `MetricDataQuery` items by profile for tuning. Calls and metric-request units deliberately have no profile attribution because one shared scan or request can serve multiple profiles; targets resolving to the same account aggregate. The counters survive failed collection cycles and appear after the next successful collection, but reset with the collector job or process. They exclude SDK-internal retry attempts and are billing inputs, not an exact AWS invoice; AWS owns the billing and pagination rules.
...
Every job also emits three **Collector Activity** charts per resolved account and region: `cloudwatch.collector_api_calls` by API operation, `cloudwatch.collector_metric_requests` for calculated AWS metric-request units, and `cloudwatch.collector_queries` by source profile. These are incremental rates derived from job-lifetime counters; they measure collector-issued work and are not an AWS invoice.
