...
func (c *Collector) fetchSummary(ctx context.Context) (*summary, error) {
	if c.exec == nil {
		return nil, fmt.Errorf("ras-mc-ctl client not initialized")
	}

	// Bail out before spawning anything if the runtime is already shutting down.
	if err := ctx.Err(); err != nil {
		return nil, err
	}

	out, err := c.exec.summary(ctx)
	if err != nil {
