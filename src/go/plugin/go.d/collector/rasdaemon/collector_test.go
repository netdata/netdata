...
func (m *mockRasMcCtl) summary(ctx context.Context) ([]byte, error) {
	m.calls++
	m.lastCtx = ctx
	if m.err != nil {
		return nil, m.err
...
func TestCollector_CollectContextCancelled(t *testing.T) {
	...
	cli := &mockRasMcCtl{out: out}
	collr := newTestCollector(cli)
	...
	// An already-cancelled context must short-circuit before ras-mc-ctl is spawned at all,
	// rather than running it and discarding the result.
	assert.Zero(t, cli.calls, "ras-mc-ctl should not be executed when the context is already cancelled")
}

// TestCollector_CollectPassesContextToExec pins that the collection context reaches the command,
// so that stopping the job interrupts an in-flight ras-mc-ctl instead of waiting out the timeout.
func TestCollector_CollectPassesContextToExec(t *testing.T) {
	out, err := os.ReadFile("testdata/summary-no-errors.txt")
	require.NoError(t, err)
	...
	require.NotNil(t, cli.lastCtx)
	assert.Equal(t, "collect", cli.lastCtx.Value(ctxKey{}), "the collection context must reach the command")
}
