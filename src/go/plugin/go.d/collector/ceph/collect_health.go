...
func (c *Collector) collectHealth(ctx context.Context, mx map[string]int64) error {
	...
	if c.Metrics.ClientIO || c.Metrics.Recovery {
		if resp.ClientPerf == nil {
			if c.Metrics.ClientIO {
				missing("client_io", "client_perf")
			}
			if c.Metrics.Recovery {
...
func collectObjectHealth(stats struct {
	...
	if stats.NumObjects < 0 || stats.NumObjectCopies < 0 || stats.NumObjectsDegraded < 0 ||
		...
		stats.NumObjectsMisplaced > stats.NumObjectCopies-stats.NumObjectsDegraded ||
		stats.NumObjectsUnfound > stats.NumObjectCopies-stats.NumObjectsDegraded-stats.NumObjectsMisplaced {
	...
	if stats.NumObjectCopies == 0 {
		mx["objects_healthy_num"] = 0
		mx["objects_misplaced_num"] = 0
		mx["objects_degraded_num"] = 0
		mx["objects_unfound_num"] = 0
	} else {
		mx["objects_healthy_num"] = percent(stats.NumObjectCopies-stats.NumObjectsDegraded-stats.NumObjectsMisplaced-stats.NumObjectsUnfound, stats.NumObjectCopies)
		mx["objects_misplaced_num"] = percent(stats.NumObjectsMisplaced, stats.NumObjectCopies)
		mx["objects_degraded_num"] = percent(stats.NumObjectsDegraded, stats.NumObjectCopies)
		mx["objects_unfound_num"] = percent(stats.NumObjectsUnfound, stats.NumObjectCopies)
	}
}
