...
func TestLegacyPowerReadingRequiresActualConsumption(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			node := &graphNode{Key: "power", Kind: "sensor", SourceModel: "deprecated_power", SourcePath: "PowerControl", Data: test.data}
			var got []float64
...
func TestExcerptSourceFormsProduceOneReading(t *testing.T) {
	for name, source := range map[string]any{
		...
		t.Run(name, func(t *testing.T) {
			node := &graphNode{Key: "fan", Kind: "fan", Data: map[string]any{"SpeedPercent": source}}
...
func TestReadingSourceHealthIsIndependentOfNumericValue(t *testing.T) {
	...
	for name, test := range map[string]struct {
		...
		"source OK beyond threshold":     {value: json.Number("90"), present: true, health: "OK", want: readingResult{Valid: true, Value: 90, Alarm: "clear"}},
		"warning without value":          {health: "Warning", want: readingResult{Alarm: "warning"}},
		"critical with null value":       {present: true, health: "Critical", want: readingResult{Alarm: "critical"}},
		"critical with invalid value":    {value: "invalid", present: true, health: "Critical", want: readingResult{Alarm: "critical"}},
		"zero is a reading":              {value: json.Number("0"), present: true, want: readingResult{Valid: true}},
		"false is not a numeric reading": {value: false, present: true, want: readingResult{}},
		"missing health is not inferred": {value: json.Number("90"), present: true, want: readingResult{Valid: true, Value: 90}},
	} {
		for sourceName, makeNode := range map[string]func(map[string]any) *graphNode{
			"standalone sensor": func(data map[string]any) *graphNode {
				...
				return &graphNode{Key: "sensor", Kind: "sensor", Data: data}
			},
			"object excerpt": func(data map[string]any) *graphNode {
				return &graphNode{Key: "fan", Kind: "fan", Data: map[string]any{"SpeedPercent": data}}
			},
			"array excerpt": func(data map[string]any) *graphNode {
				return &graphNode{Key: "sensor", Kind: "sensor", SourceModel: "embedded_sensor_excerpt", SensorExcerpts: []sensorExcerptSource{{Path: "FanSpeedPercent[0]", Type: "Percent", Units: "%", Data: data}}}
			},
		} {
			t.Run(sourceName+"/"+name, func(t *testing.T) {
				data := map[string]any{"Thresholds": map[string]any{"UpperCritical": map[string]any{"Reading": json.Number("70")}}}
				if test.present {
				...
				reading := readings[0]
				assert.Equal(t, test.want, readingResult{Valid: reading.Valid, Value: reading.Value, Alarm: reading.SourceAlarm})
				var states []string
...
func TestStoredEnergyExcerptKeepsGaugeSemantics(t *testing.T) {
	for name, source := range map[string]any{
		...
		t.Run(name, func(t *testing.T) {
			node := &graphNode{Key: "battery", Kind: "battery", Enrichment: map[string]map[string]any{"battery_metrics": {"StoredEnergyWattHours": source}}}
			client := &protocolClient{}
