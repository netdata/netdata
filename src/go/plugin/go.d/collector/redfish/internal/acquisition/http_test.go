...
func TestProtocolClientCheckAcceptsJSONWithIncompatibleContentType(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		testutil.WriteJSONBody(
			w,
			testutil.Resource("/redfish/v1/", "ServiceRoot", "Service", map[string]any{"RedfishVersion": "1.20.0"}),
		)
	}))
		...
		t.Run(fmt.Sprintf("serial=%v/redirect=%v", tc.serial, tc.redirect), func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				case sensors:
					testutil.WriteJSON(
						w,
						testutil.Collection(sensors, "Sensor", sensors+"/1", sensors+"/2", sensors+"/3", sensors+"/4"),
					)
				default:
					n := active.Add(1)
					defer active.Add(-1)
