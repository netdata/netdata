...
func buildSNMPTopologyV1OSPFNeighborsTable(
	...
	for i, row := range rows {
		...
		neighborIPs[i] = nullableStringRef(stringsDict, normalizeIPAddress(topologyV1ScalarLabelValue(row.values["neighbor_ip"])))
		states[i] = nullableStringRef(stringsDict, normalizeOSPFNeighborState(topologyV1ScalarLabelValue(row.values["state"])))
		localIPs[i] = nullableStringRef(stringsDict, normalizeIPAddress(topologyV1ScalarLabelValue(row.values["local_ip"])))
