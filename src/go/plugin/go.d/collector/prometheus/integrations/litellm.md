...
failures, Redis/PostgreSQL operations, and spend-update queues. Optional charts appear when their callbacks and
features export metrics.
...
its `/metrics` endpoint. Use LiteLLM's `prometheus_metrics_config` when a lower-cardinality label set is required.
