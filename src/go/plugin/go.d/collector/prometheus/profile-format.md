...
- **Only collected series can be charted.** Gauge families named `*_info` are skipped. Untyped scalar families are
  collected only when the selected profile or job `fallback_type` maps them to a gauge or counter, or when the name
  ends in `_total` (the last-resort implicit counter rule). Declared type wins; job policy wins over profile policy; and an explicit
