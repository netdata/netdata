...
func (f *funcDeadlockInfo) queryLatestDeadlock(ctx context.Context) (time.Time, string, string, error) {
	...
	started := time.Now()
	defer func() {
		c.Debugf("deadlock-info: source=%s duration=%s", source, time.Since(started).Round(time.Millisecond))
	}()
	if c.Functions.DeadlockInfo.UseRingBuffer {
		source = deadlockSourceRingBuffer
		err = readRingBuffer()
	} else {
		target, available, targetErr := c.resolveMSSQLErrorReadTarget(ctx, "system_health", false)
		if targetErr != nil {
			err = targetErr
		} else if !available {
			err = errDeadlockEventFileUnavailable
		} else {
			err = c.db.QueryRowContext(ctx, querySystemHealthLatestDeadlockEventFile, sql.Named("filePath", target.filePath)).Scan(&deadlockTime, &deadlockXML)
		}
