...
func (c *Collector) buildMSSQLDynamicSQL(cols []mssqlColumnMeta, sortColumn string, timeWindowDays int, limit int) string {
	...
	for _, col := range cols {
		...
		case col.isIdentity:
			switch col.uiKey {
			case "queryHash":
				expr = "CONVERT(VARCHAR(64), q.query_hash, 1) AS query_hash"
			case "query":
				expr = "qt.query_sql_text"
			case "database":
				expr = "DB_NAME() AS database_name"
			}
		case col.uiKey == "calls":
			expr = "SUM(rs.count_executions) AS calls"
		case col.uiKey == "totalTime":
			// Total time = sum of (avg_duration * executions) converted to milliseconds
			expr = "SUM(rs.avg_duration * rs.count_executions) / 1000.0 AS total_time"
	...
	// Map UI sort key to SQL expression
	orderByExpr := "total_time" // default
	switch sortColumn {
	case "calls":
		orderByExpr = "calls"
	case "totalTime":
		orderByExpr = "total_time"
	case "avgTime":
		orderByExpr = "avgTime"
	case "avgCpu":
		orderByExpr = "avgCpu"
	case "avgReads":
		orderByExpr = "avgReads"
	case "avgWrites":
		orderByExpr = "avgWrites"
	case "avgPhysReads":
		orderByExpr = "avgPhysReads"
	case "avgDop":
		orderByExpr = "avgDop"
	case "avgMemory":
		orderByExpr = "avgMemory"
	case "avgRows":
		orderByExpr = "avgRows"
	case "avgLogBytes":
		orderByExpr = "avgLogBytes"
	case "avgTempdb":
		orderByExpr = "avgTempdb"
	}
