...
WITH ranked_stats AS (
    ...
    WHERE qs.query_hash IS NOT NULL
      ...
      AND (DB_NAME(CONVERT(INT, pa.value)) IS NULL OR DB_NAME(CONVERT(INT, pa.value)) NOT IN ('master', 'tempdb', 'model', 'msdb'))
    %s
), aggregated AS (
    SELECT
      ...
      MIN(rs.database_id) AS database_id,
...
CROSS APPLY sys.dm_exec_sql_text(CONVERT(VARBINARY(64), a.sql_handle_hex, 1)) AS qt
ORDER BY [%s] DESC;
`, recencyFilter, groupExpr, limit, strings.Join(outerExpr, ",\n  "), orderByExpr)
}
