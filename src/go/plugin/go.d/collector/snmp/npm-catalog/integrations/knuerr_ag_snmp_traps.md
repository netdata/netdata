<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/knuerr_ag_snmp_traps.md"
sidebar_label: "Knuerr AG SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [knuerr-ag, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Knuerr AG devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/knuerr-ag-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/knuerr-ag-snmp-traps"
-->
...
# Knuerr AG SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Knuerr AG SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Knuerr AG devices with Netdata. The bundled Knuerr AG trap profile decodes 2 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Knuerr AG enterprise OID space, and decodes the varbinds using the bundled Knuerr AG trap profile. No per-trap configuration.
...
Traps from Knuerr AG devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **2 trap definitions** from Knuerr AG across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `notice` 2

**By category:** `state_change` 1 · `auth` 1
