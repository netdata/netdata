<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/xerox_snmp_traps.md"
sidebar_label: "Xerox SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [xerox, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Xerox devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/xerox-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/xerox-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Xerox SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Xerox devices with Netdata. The bundled Xerox trap profile decodes 26 trap definitions across 8 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Xerox enterprise OID space, and decodes the varbinds using the bundled Xerox trap profile. No per-trap configuration.
...
Traps from Xerox devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **26 trap definitions** from Xerox across **8 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `warning` 6 · `notice` 15 · `info` 5

**By category:** `state_change` 11 · `config_change` 5 · `security` 3 · `diagnostic` 7

### Coverage by MIB

| MIB | Trap definitions |
|:----|----:|
| `XEROX-JOB-MONITORING-EXT-MIB` | 7 |
| `XEROX-COMMON-MIB` | 4 |
| `XEROX-HOST-RESOURCES-EXT-MIB` | 4 |
| `XEROX-SECURITY-MIB` | 4 |
| `XEROX-SERVICE-MONITORING-MIB` | 3 |
| `XEROX-SYS-ADMIN-MIB` | 2 |
| `XEROX-PRINTER-EXT-MIB` | 1 |
| `XEROX-RESOURCES-MIB` | 1 |

### Sample decoded traps

- `XEROX-HOST-RESOURCES-EXT-MIB::xcmHrDevInfoV2Event`
- `XEROX-HOST-RESOURCES-EXT-MIB::xcmHrDevMgmtV2Event`
