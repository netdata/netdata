<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/jacarta_ltd_snmp_traps.md"
sidebar_label: "Jacarta Ltd SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [jacarta-ltd, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Jacarta Ltd devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/jacarta-ltd-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/jacarta-ltd-snmp-traps"
-->
...
# Jacarta Ltd SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Jacarta Ltd SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Jacarta Ltd devices with Netdata. The bundled Jacarta Ltd trap profile decodes 22 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Jacarta Ltd enterprise OID space, and decodes the varbinds using the bundled Jacarta Ltd trap profile. No per-trap configuration.
...
Traps from Jacarta Ltd devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **22 trap definitions** from Jacarta Ltd across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `crit` 5 · `warning` 10 · `notice` 7

**By category:** `state_change` 2 · `diagnostic` 20
...
- `InterSeptor-MIB::inSeptCommunicationRestored`
- `InterSeptor-MIB::inSeptCommunicationLost`
