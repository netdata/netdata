<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/kentix_gmbh_snmp_traps.md"
sidebar_label: "Kentix GmbH SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [kentix-gmbh, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Kentix GmbH devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/kentix-gmbh-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/kentix-gmbh-snmp-traps"
-->
...
# Kentix GmbH SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Kentix GmbH SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Kentix GmbH devices with Netdata. The bundled Kentix GmbH trap profile decodes 4 trap definitions across 3 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Kentix GmbH enterprise OID space, and decodes the varbinds using the bundled Kentix GmbH trap profile. No per-trap configuration.
...
Traps from Kentix GmbH devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **4 trap definitions** from Kentix GmbH across **3 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `warning` 3 · `notice` 1

**By category:** `auth` 2 · `diagnostic` 2
