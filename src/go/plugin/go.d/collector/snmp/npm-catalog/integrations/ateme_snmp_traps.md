<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/ateme_snmp_traps.md"
sidebar_label: "Ateme SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [ateme, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Ateme devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/ateme-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/ateme-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Ateme SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Ateme devices with Netdata. The bundled Ateme trap profile decodes 181 trap definitions across 2 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Ateme enterprise OID space, and decodes the varbinds using the bundled Ateme trap profile. No per-trap configuration.
...
Traps from Ateme devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **181 trap definitions** from Ateme across **2 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `emerg` 1 · `alert` 6 · `crit` 4 · `err` 6 · `warning` 139 · `notice` 23 · `info` 2

**By category:** `state_change` 12 · `config_change` 11 · `security` 1 · `license` 11 · `diagnostic` 146

### Coverage by MIB

| MIB | Trap definitions |
|:----|----:|
| `ATEME-TITAN-EDGE-MIB` | 180 |
| `TITAN-MIB` | 1 |

### Sample decoded traps

- `TITAN-MIB::alarm`
- `ATEME-TITAN-EDGE-MIB::titanEdgeApplianceMonitorLogSystemStartedNotification`
