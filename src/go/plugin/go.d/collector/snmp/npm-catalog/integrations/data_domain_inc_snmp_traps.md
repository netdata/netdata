<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/data_domain_inc_snmp_traps.md"
sidebar_label: "Data Domain Inc SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [data-domain-inc, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Data Domain Inc devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/data-domain-inc-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/data-domain-inc-snmp-traps"
-->
...
# Data Domain Inc SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Data Domain Inc SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Data Domain Inc devices with Netdata. The bundled Data Domain Inc trap profile decodes 247 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Data Domain Inc enterprise OID space, and decodes the varbinds using the bundled Data Domain Inc trap profile. No per-trap configuration.
...
Traps from Data Domain Inc devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **247 trap definitions** from Data Domain Inc across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `emerg` 4 · `alert` 30 · `crit` 88 · `err` 6 · `warning` 106 · `notice` 13

**By category:** `state_change` 26 · `config_change` 4 · `security` 4 · `license` 3 · `diagnostic` 210
