<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/reltec_corporation_snmp_traps.md"
sidebar_label: "Reltec Corporation SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [reltec-corporation, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Reltec Corporation devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/reltec-corporation-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/reltec-corporation-snmp-traps"
-->
...
# Reltec Corporation SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Reltec Corporation SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Reltec Corporation devices with Netdata. The bundled Reltec Corporation trap profile decodes 90 trap definitions across 2 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Reltec Corporation enterprise OID space, and decodes the varbinds using the bundled Reltec Corporation trap profile. No per-trap configuration.
...
Traps from Reltec Corporation devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **90 trap definitions** from Reltec Corporation across **2 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 18 · `crit` 21 · `err` 1 · `warning` 48 · `notice` 2

**By category:** `diagnostic` 90
...
- `VEC-MIBv5-9::psEmergencyStop`
- `VEC-MIBv5-9::psAlarmVeryLowVoltage`
