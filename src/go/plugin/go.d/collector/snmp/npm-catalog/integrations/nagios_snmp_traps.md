<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/nagios_snmp_traps.md"
sidebar_label: "Nagios SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [nagios, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Nagios devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/nagios-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/nagios-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Nagios SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Nagios devices with Netdata. The bundled Nagios trap profile decodes 4 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Nagios enterprise OID space, and decodes the varbinds using the bundled Nagios trap profile. No per-trap configuration.
...
Traps from Nagios devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **4 trap definitions** from Nagios across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.
