<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/ingrasys_snmp_traps.md"
sidebar_label: "Ingrasys SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [ingrasys, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Ingrasys devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/ingrasys-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/ingrasys-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Ingrasys SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Ingrasys devices with Netdata. The bundled Ingrasys trap profile decodes 246 trap definitions across 2 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Ingrasys enterprise OID space, and decodes the varbinds using the bundled Ingrasys trap profile. No per-trap configuration.
...
Traps from Ingrasys devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **246 trap definitions** from Ingrasys across **2 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 14 · `crit` 63 · `warning` 110 · `notice` 50 · `info` 9

**By category:** `state_change` 18 · `diagnostic` 228
...
- `USHA-MIB::ushaPowerRestored`
- `USHA-MIB::ushaPowerFail`
