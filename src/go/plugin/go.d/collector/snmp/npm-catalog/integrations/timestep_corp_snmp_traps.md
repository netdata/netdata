<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/timestep_corp_snmp_traps.md"
sidebar_label: "Timestep Corp SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [timestep-corp, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Timestep Corp devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/timestep-corp-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/timestep-corp-snmp-traps"
-->
...
# Timestep Corp SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Timestep Corp SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Timestep Corp devices with Netdata. The bundled Timestep Corp trap profile decodes 8 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Timestep Corp enterprise OID space, and decodes the varbinds using the bundled Timestep Corp trap profile. No per-trap configuration.
...
Traps from Timestep Corp devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **8 trap definitions** from Timestep Corp across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `warning` 8

**By category:** `state_change` 1 · `security` 7

### Coverage by MIB

| MIB | Trap definitions |
|:----|----:|
| `TIMESTEP-IPSEC-MIB` | 8 |

### Sample decoded traps

- `TIMESTEP-IPSEC-MIB::ipsecTrapPermIkeNegFailure`
- `TIMESTEP-IPSEC-MIB::ipsecTrapTransIkeNegFailure`
