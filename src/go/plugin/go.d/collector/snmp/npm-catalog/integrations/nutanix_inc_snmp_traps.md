<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/nutanix_inc_snmp_traps.md"
sidebar_label: "Nutanix Inc SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [nutanix-inc, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Nutanix Inc devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/nutanix-inc-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/nutanix-inc-snmp-traps"
-->
...
# Nutanix Inc SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Nutanix Inc SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Nutanix Inc devices with Netdata. The bundled Nutanix Inc trap profile decodes 860 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Nutanix Inc enterprise OID space, and decodes the varbinds using the bundled Nutanix Inc trap profile. No per-trap configuration.
...
Traps from Nutanix Inc devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **860 trap definitions** from Nutanix Inc across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 11 · `crit` 152 · `err` 31 · `warning` 623 · `notice` 42 · `info` 1

**By category:** `state_change` 97 · `config_change` 21 · `security` 19 · `auth` 2 · `license` 17 · `diagnostic` 704
