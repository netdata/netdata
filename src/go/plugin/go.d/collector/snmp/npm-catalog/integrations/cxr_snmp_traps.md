<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/cxr_snmp_traps.md"
sidebar_label: "Cxr SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [cxr, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Cxr devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/cxr-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/cxr-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Cxr SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Cxr devices with Netdata. The bundled Cxr trap profile decodes 73 trap definitions across 2 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Cxr enterprise OID space, and decodes the varbinds using the bundled Cxr trap profile. No per-trap configuration.
...
Traps from Cxr devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **73 trap definitions** from Cxr across **2 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 5 · `crit` 9 · `err` 3 · `warning` 23 · `notice` 32 · `info` 1

**By category:** `state_change` 20 · `config_change` 1 · `security` 4 · `mobility` 1 · `diagnostic` 47
...
- `CXR-CFIP-MIB::alarmUndefined`
- `CXR-CFIP-MIB::newlyPowered`
