<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/p_cube_ltd_snmp_traps.md"
sidebar_label: "P Cube Ltd SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [p-cube-ltd, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from P Cube Ltd devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/p-cube-ltd-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/p-cube-ltd-snmp-traps"
-->
...
# P Cube Ltd SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="P Cube Ltd SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from P Cube Ltd devices with Netdata. The bundled P Cube Ltd trap profile decodes 45 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the P Cube Ltd enterprise OID space, and decodes the varbinds using the bundled P Cube Ltd trap profile. No per-trap configuration.
...
Traps from P Cube Ltd devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **45 trap definitions** from P Cube Ltd across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `emerg` 1 · `alert` 2 · `crit` 6 · `warning` 22 · `notice` 14

**By category:** `state_change` 19 · `config_change` 1 · `security` 5 · `auth` 4 · `diagnostic` 16
