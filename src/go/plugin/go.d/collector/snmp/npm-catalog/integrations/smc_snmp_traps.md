<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/smc_snmp_traps.md"
sidebar_label: "Smc SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [smc, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Smc devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/smc-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/smc-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Smc SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Smc devices with Netdata. The bundled Smc trap profile decodes 2 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Smc enterprise OID space, and decodes the varbinds using the bundled Smc trap profile. No per-trap configuration.
...
Traps from Smc devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **2 trap definitions** from Smc across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 1 · `warning` 1

**By category:** `security` 1 · `diagnostic` 1
