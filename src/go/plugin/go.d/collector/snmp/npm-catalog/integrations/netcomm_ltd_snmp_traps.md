<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/netcomm_ltd_snmp_traps.md"
sidebar_label: "Netcomm Ltd SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [netcomm-ltd, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Netcomm Ltd devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/netcomm-ltd-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/netcomm-ltd-snmp-traps"
-->
...
# Netcomm Ltd SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Netcomm Ltd SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Netcomm Ltd devices with Netdata. The bundled Netcomm Ltd trap profile decodes 99 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Netcomm Ltd enterprise OID space, and decodes the varbinds using the bundled Netcomm Ltd trap profile. No per-trap configuration.
...
Traps from Netcomm Ltd devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **99 trap definitions** from Netcomm Ltd across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 13 · `crit` 18 · `err` 2 · `warning` 44 · `notice` 21 · `info` 1

**By category:** `state_change` 22 · `config_change` 1 · `security` 1 · `auth` 1 · `diagnostic` 74
