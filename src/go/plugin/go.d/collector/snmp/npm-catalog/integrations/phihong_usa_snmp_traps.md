<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/phihong_usa_snmp_traps.md"
sidebar_label: "Phihong USA SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [phihong-usa, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Phihong USA devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/phihong-usa-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/phihong-usa-snmp-traps"
-->
...
# Phihong USA SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Phihong USA SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Phihong USA devices with Netdata. The bundled Phihong USA trap profile decodes 8 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Phihong USA enterprise OID space, and decodes the varbinds using the bundled Phihong USA trap profile. No per-trap configuration.
...
Traps from Phihong USA devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **8 trap definitions** from Phihong USA across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `crit` 4 · `err` 1 · `warning` 3
...
- `PHIHONG-PoE-MIB::poePortHWFailTrap`
- `PHIHONG-PoE-MIB::poePortPeakOverCurrentTrap`
