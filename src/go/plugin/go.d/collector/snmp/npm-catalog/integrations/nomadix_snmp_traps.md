<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/nomadix_snmp_traps.md"
sidebar_label: "Nomadix SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [nomadix, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Nomadix devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/nomadix-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/nomadix-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Nomadix SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Nomadix devices with Netdata. The bundled Nomadix trap profile decodes 4 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Nomadix enterprise OID space, and decodes the varbinds using the bundled Nomadix trap profile. No per-trap configuration.
...
Traps from Nomadix devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **4 trap definitions** from Nomadix across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `warning` 3 · `notice` 1

**By category:** `state_change` 1 · `auth` 1 · `license` 1 · `diagnostic` 1
