<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/cohesity_inc_snmp_traps.md"
sidebar_label: "Cohesity Inc SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [cohesity-inc, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Cohesity Inc devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/cohesity-inc-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/cohesity-inc-snmp-traps"
-->
...
# Cohesity Inc SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Cohesity Inc SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Cohesity Inc devices with Netdata. The bundled Cohesity Inc trap profile decodes 418 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Cohesity Inc enterprise OID space, and decodes the varbinds using the bundled Cohesity Inc trap profile. No per-trap configuration.
...
Traps from Cohesity Inc devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **418 trap definitions** from Cohesity Inc across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `emerg` 1 · `alert` 42 · `crit` 42 · `err` 22 · `warning` 227 · `notice` 73 · `info` 11

**By category:** `state_change` 51 · `config_change` 36 · `security` 21 · `auth` 5 · `license` 20 · `diagnostic` 285

### Coverage by MIB

| MIB | Trap definitions |
|:----|----:|
| `COHESITY-APPLIANCE-MIB` | 418 |

### Sample decoded traps

- `COHESITY-APPLIANCE-MIB::diskNotHealthyNotif`
- `COHESITY-APPLIANCE-MIB::nodePingFailedNotif`
