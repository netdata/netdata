<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/vmware_esx.md"
sidebar_label: "Vmware ESX"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/Device Metrics/Integrations"
keywords: [vmware, esx, snmp, server, npm]
description: "Monitor Vmware ESX (server) with Netdata over SNMP."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/device-metrics/integrations/vmware-esx"
slug: "/network-performance-monitoring/device-metrics/integrations/vmware-esx"
-->
...
<img src="https://netdata.cloud/img/vmware.svg" width="150" alt="Vmware ESX"/>
...
Monitor Vmware ESX (server) with Netdata over SNMP. Netdata recognizes the device automatically by its `sysObjectID` (recognized across 1 device identifiers) and collects the metrics this profile declares — on top of the generic SNMP baseline — with no manual OID configuration.

Netdata's SNMP collector matches the device to the **vmware-esx.yaml** profile via `sysObjectID`/`sysDescr`, then polls the OIDs it declares.
...
Auto-detected as Vmware ESX via sysObjectID/sysDescr.
...
|---|---|---|---|
| `snmp.device_prof_vmwHardwareStatus` | `{status}` | per vmw_subsystem_index, vmw_subsystem_type | Last reported status of this component |

### Hardware / HBA

| Metric (chart context) | Unit | Scope | Description |
|---|---|---|---|
| `snmp.device_prof_vmwHbaStatus` | `{status}` | per vmw_hba_device_name | Current HBA status |
