<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/peribit_networks_snmp_traps.md"
sidebar_label: "Peribit Networks SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [peribit-networks, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Peribit Networks devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/peribit-networks-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/peribit-networks-snmp-traps"
-->
...
# Peribit Networks SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Peribit Networks SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Peribit Networks devices with Netdata. The bundled Peribit Networks trap profile decodes 40 trap definitions across 2 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Peribit Networks enterprise OID space, and decodes the varbinds using the bundled Peribit Networks trap profile. No per-trap configuration.
...
Traps from Peribit Networks devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **40 trap definitions** from Peribit Networks across **2 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 7 · `crit` 4 · `warning` 15 · `notice` 14

**By category:** `state_change` 23 · `security` 1 · `auth` 1 · `license` 3 · `diagnostic` 12
...
|:----|----:|
| `JUNIPER-WX-MIB` | 23 |
| `JUNIPER-WX-COMMON-MIB` | 17 |

### Sample decoded traps

- `JUNIPER-WX-COMMON-MIB::jnxWxCommonEventInFailSafeMode`
- `JUNIPER-WX-COMMON-MIB::jnxWxCommonEventPowerSupplyFailure`
