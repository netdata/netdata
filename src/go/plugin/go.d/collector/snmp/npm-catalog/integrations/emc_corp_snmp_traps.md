<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/emc_corp_snmp_traps.md"
sidebar_label: "Emc Corp SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [emc-corp, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Emc Corp devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/emc-corp-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/emc-corp-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Emc Corp SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Emc Corp devices with Netdata. The bundled Emc Corp trap profile decodes 33 trap definitions across 6 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Emc Corp enterprise OID space, and decodes the varbinds using the bundled Emc Corp trap profile. No per-trap configuration.
...
Traps from Emc Corp devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **33 trap definitions** from Emc Corp across **6 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `emerg` 2 · `alert` 4 · `crit` 8 · `err` 2 · `warning` 8 · `notice` 4 · `info` 3 · `debug` 2

**By category:** `state_change` 4 · `diagnostic` 29
...
- `EMC-MIB::emcDeviceStatusTrap`
- `EMC-MIB::emcSymmetrixStatusTrap`
