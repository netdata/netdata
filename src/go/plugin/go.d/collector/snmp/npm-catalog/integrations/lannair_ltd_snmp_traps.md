<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/lannair_ltd_snmp_traps.md"
sidebar_label: "Lannair Ltd SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [lannair-ltd, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Lannair Ltd devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/lannair-ltd-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/lannair-ltd-snmp-traps"
-->
...
# Lannair Ltd SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Lannair Ltd SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Lannair Ltd devices with Netdata. The bundled Lannair Ltd trap profile decodes 17 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Lannair Ltd enterprise OID space, and decodes the varbinds using the bundled Lannair Ltd trap profile. No per-trap configuration.
...
Traps from Lannair Ltd devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **17 trap definitions** from Lannair Ltd across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `warning` 5 · `notice` 10 · `info` 2

**By category:** `state_change` 3 · `mobility` 8 · `diagnostic` 3 · `unknown` 3
...
- `BREEZECOM-MIB::brzAProamingIn`
- `BREEZECOM-MIB::brzAPassociated`
