<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/fireeye_inc_snmp_traps.md"
sidebar_label: "Fireeye Inc SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [fireeye-inc, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Fireeye Inc devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/fireeye-inc-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/fireeye-inc-snmp-traps"
-->
...
# Fireeye Inc SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Fireeye Inc SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Fireeye Inc devices with Netdata. The bundled Fireeye Inc trap profile decodes 32 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Fireeye Inc enterprise OID space, and decodes the varbinds using the bundled Fireeye Inc trap profile. No per-trap configuration.
...
Traps from Fireeye Inc devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **32 trap definitions** from Fireeye Inc across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 12 · `crit` 3 · `warning` 7 · `notice` 10

**By category:** `state_change` 4 · `security` 11 · `license` 1 · `diagnostic` 16
