<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/isilon_ststems_snmp_traps.md"
sidebar_label: "Isilon Ststems SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [isilon-ststems, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Isilon Ststems devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/isilon-ststems-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/isilon-ststems-snmp-traps"
-->
...
# Isilon Ststems SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Isilon Ststems SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Isilon Ststems devices with Netdata. The bundled Isilon Ststems trap profile decodes 203 trap definitions across 2 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Isilon Ststems enterprise OID space, and decodes the varbinds using the bundled Isilon Ststems trap profile. No per-trap configuration.
...
Traps from Isilon Ststems devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **203 trap definitions** from Isilon Ststems across **2 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `emerg` 14 · `alert` 6 · `crit` 84 · `err` 1 · `warning` 88 · `notice` 9 · `info` 1

**By category:** `state_change` 10 · `config_change` 4 · `security` 8 · `license` 3 · `diagnostic` 178
