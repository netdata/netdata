<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/unisys_corp_snmp_traps.md"
sidebar_label: "Unisys Corp SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [unisys-corp, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Unisys Corp devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/unisys-corp-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/unisys-corp-snmp-traps"
-->
...
# Unisys Corp SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Unisys Corp SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Unisys Corp devices with Netdata. The bundled Unisys Corp trap profile decodes 53 trap definitions across 2 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Unisys Corp enterprise OID space, and decodes the varbinds using the bundled Unisys Corp trap profile. No per-trap configuration.
...
Traps from Unisys Corp devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **53 trap definitions** from Unisys Corp across **2 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 2 · `crit` 7 · `warning` 22 · `notice` 19 · `info` 3

**By category:** `state_change` 5 · `config_change` 1 · `security` 1 · `diagnostic` 46
...
- `VISINET2-MIB::uemTrapConfigurationChange`
- `VISINET2-MIB::uemTrapSensorCritical`
