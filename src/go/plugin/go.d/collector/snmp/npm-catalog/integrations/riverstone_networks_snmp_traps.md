<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/riverstone_networks_snmp_traps.md"
sidebar_label: "Riverstone Networks SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [riverstone-networks, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Riverstone Networks devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/riverstone-networks-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/riverstone-networks-snmp-traps"
-->
...
# Riverstone Networks SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Riverstone Networks SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Riverstone Networks devices with Netdata. The bundled Riverstone Networks trap profile decodes 23 trap definitions across 4 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Riverstone Networks enterprise OID space, and decodes the varbinds using the bundled Riverstone Networks trap profile. No per-trap configuration.
...
Traps from Riverstone Networks devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **23 trap definitions** from Riverstone Networks across **4 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 4 · `crit` 3 · `warning` 6 · `notice` 10

**By category:** `state_change` 10 · `config_change` 2 · `diagnostic` 11

### Coverage by MIB

| MIB | Trap definitions |
|:----|----:|
| `RIVERSTONE-NOTIFICATIONS-MIB` | 13 |
| `RIVERSTONE-MPLS-MIB` | 5 |
| `RIVERSTONE-LFAP-MIB` | 3 |
| `RIVERSTONE-CONFIG-MIB` | 2 |
