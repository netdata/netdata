<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/beronet_gmbh_snmp_traps.md"
sidebar_label: "Beronet GmbH SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [beronet-gmbh, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Beronet GmbH devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/beronet-gmbh-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/beronet-gmbh-snmp-traps"
-->
...
# Beronet GmbH SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Beronet GmbH SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Beronet GmbH devices with Netdata. The bundled Beronet GmbH trap profile decodes 3 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Beronet GmbH enterprise OID space, and decodes the varbinds using the bundled Beronet GmbH trap profile. No per-trap configuration.
...
Traps from Beronet GmbH devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **3 trap definitions** from Beronet GmbH across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `warning` 2 · `notice` 1

**By category:** `state_change` 3
