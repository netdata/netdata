<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/nokia_snmp_traps.md"
sidebar_label: "Nokia SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [nokia, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Nokia devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/nokia-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/nokia-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Nokia SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Nokia devices with Netdata. The bundled Nokia trap profile decodes 42 trap definitions across 6 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Nokia enterprise OID space, and decodes the varbinds using the bundled Nokia trap profile. No per-trap configuration.
...
Traps from Nokia devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **42 trap definitions** from Nokia across **6 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 7 · `crit` 5 · `err` 1 · `warning` 10 · `notice` 17 · `info` 2

**By category:** `state_change` 14 · `config_change` 5 · `license` 1 · `diagnostic` 22

### Coverage by MIB

| MIB | Trap definitions |
|:----|----:|
| `NOKIA-IPSO-SYSTEM-MIB` | 17 |
| `NOKIA-ALCHEMYOS-CLUSTER-MIB` | 10 |
| `NOKIA-ENHANCED-SNMP-SOLUTION-SUITE-ALARM-IRP` | 6 |
| `NOKIA-IPSO-LBCLUSTER-MIB` | 5 |
| `NOKIA-ENHANCED-SNMP-SOLUTION-SUITE-PM-IRP` | 2 |
| `NOKIA-IPSO-LINKAGGREGATION-MIB` | 2 |
