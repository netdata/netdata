<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/verilink_corp_snmp_traps.md"
sidebar_label: "Verilink Corp SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [verilink-corp, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Verilink Corp devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/verilink-corp-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/verilink-corp-snmp-traps"
-->
...
# Verilink Corp SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Verilink Corp SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Verilink Corp devices with Netdata. The bundled Verilink Corp trap profile decodes 599 trap definitions across 4 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Verilink Corp enterprise OID space, and decodes the varbinds using the bundled Verilink Corp trap profile. No per-trap configuration.
...
Traps from Verilink Corp devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **599 trap definitions** from Verilink Corp across **4 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 7 · `crit` 46 · `warning` 376 · `notice` 170

**By category:** `state_change` 164 · `config_change` 1 · `security` 4 · `auth` 1 · `diagnostic` 429

### Coverage by MIB

| MIB | Trap definitions |
|:----|----:|
| `VERILINK-ENTERPRISE-NCMALARM-MIB` | 546 |
| `IPADv2-MIB` | 32 |
| `IPAD-MIB` | 20 |
| `IPAD-IPSTART-MIB` | 1 |
...
- `VERILINK-ENTERPRISE-NCMALARM-MIB::ncmTrap`
- `VERILINK-ENTERPRISE-NCMALARM-MIB::ncmnearCSUPowerUP`
