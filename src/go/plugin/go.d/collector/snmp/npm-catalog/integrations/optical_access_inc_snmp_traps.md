<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/optical_access_inc_snmp_traps.md"
sidebar_label: "Optical Access Inc SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [optical-access-inc, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Optical Access Inc devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/optical-access-inc-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/optical-access-inc-snmp-traps"
-->
...
# Optical Access Inc SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Optical Access Inc SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Optical Access Inc devices with Netdata. The bundled Optical Access Inc trap profile decodes 193 trap definitions across 17 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Optical Access Inc enterprise OID space, and decodes the varbinds using the bundled Optical Access Inc trap profile. No per-trap configuration.
...
Traps from Optical Access Inc devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **193 trap definitions** from Optical Access Inc across **17 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 5 · `crit` 10 · `err` 1 · `warning` 108 · `notice` 65 · `info` 4

**By category:** `state_change` 83 · `config_change` 1 · `security` 2 · `diagnostic` 107
...
| `OS-RFC2544-MIB` | 6 |
| `OS-PWE3-ICCP-MIB` | 5 |
| `OS-PROV-SOAM-PM-MIB` | 4 |
| `OS-SYNCE-MIB` | 4 |
| `OS-VIF-MIB` | 3 |
| `OS-ERP-MIB` | 2 |
| `OS-ETH-SERV-MIB` | 1 |
| `OS-ETH-SERV-PROTECTION-MIB` | 1 |
| `OS-PROV-Y1564-MIB` | 1 |
| `OS-PTP-MIB` | 1 |
| `OS-SYSLOG-MIB` | 1 |
