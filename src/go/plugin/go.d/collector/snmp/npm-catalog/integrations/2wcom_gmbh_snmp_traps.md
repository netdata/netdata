<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/2wcom_gmbh_snmp_traps.md"
sidebar_label: "2Wcom GmbH SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [2wcom-gmbh, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from 2Wcom GmbH devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/2wcom-gmbh-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/2wcom-gmbh-snmp-traps"
-->
...
# 2Wcom GmbH SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="2Wcom GmbH SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from 2Wcom GmbH devices with Netdata. The bundled 2Wcom GmbH trap profile decodes 19 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the 2Wcom GmbH enterprise OID space, and decodes the varbinds using the bundled 2Wcom GmbH trap profile. No per-trap configuration.
...
Traps from 2Wcom GmbH devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **19 trap definitions** from 2Wcom GmbH across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `warning` 16 · `notice` 3

**By category:** `state_change` 5 · `config_change` 1 · `diagnostic` 13
...
- `FLEXDSR04::tunerRfPowerEvent`
- `FLEXDSR04::tunerCnEvent`
