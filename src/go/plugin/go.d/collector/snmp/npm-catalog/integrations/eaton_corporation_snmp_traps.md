<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/eaton_corporation_snmp_traps.md"
sidebar_label: "Eaton Corporation SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [eaton-corporation, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Eaton Corporation devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/eaton-corporation-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/eaton-corporation-snmp-traps"
-->
...
# Eaton Corporation SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Eaton Corporation SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Eaton Corporation devices with Netdata. The bundled Eaton Corporation trap profile decodes 175 trap definitions across 8 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Eaton Corporation enterprise OID space, and decodes the varbinds using the bundled Eaton Corporation trap profile. No per-trap configuration.
...
Traps from Eaton Corporation devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **175 trap definitions** from Eaton Corporation across **8 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 12 · `crit` 24 · `err` 1 · `warning` 63 · `notice` 70 · `info` 5

**By category:** `state_change` 59 · `config_change` 9 · `security` 2 · `auth` 8 · `diagnostic` 97
