<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/starent_networks_snmp_traps.md"
sidebar_label: "Starent Networks SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [starent-networks, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Starent Networks devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/starent-networks-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/starent-networks-snmp-traps"
-->
...
# Starent Networks SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Starent Networks SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Starent Networks devices with Netdata. The bundled Starent Networks trap profile decodes 821 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Starent Networks enterprise OID space, and decodes the varbinds using the bundled Starent Networks trap profile. No per-trap configuration.
...
Traps from Starent Networks devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **821 trap definitions** from Starent Networks across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 35 · `crit` 117 · `err` 8 · `warning` 262 · `notice` 391 · `info` 7 · `debug` 1

**By category:** `state_change` 560 · `config_change` 11 · `security` 11 · `auth` 3 · `license` 8 · `mobility` 1 · `diagnostic` 227
