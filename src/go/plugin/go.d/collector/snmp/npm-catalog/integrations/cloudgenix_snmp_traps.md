<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/cloudgenix_snmp_traps.md"
sidebar_label: "Cloudgenix SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [cloudgenix, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Cloudgenix devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/cloudgenix-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/cloudgenix-snmp-traps"
-->
...
# Cloudgenix SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Cloudgenix SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Cloudgenix devices with Netdata. The bundled Cloudgenix trap profile decodes 19 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Cloudgenix enterprise OID space, and decodes the varbinds using the bundled Cloudgenix trap profile. No per-trap configuration.
...
Traps from Cloudgenix devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **19 trap definitions** from Cloudgenix across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `crit` 3 · `warning` 6 · `notice` 10

**By category:** `state_change` 16 · `diagnostic` 3
