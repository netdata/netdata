<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/spectra_logic_snmp_traps.md"
sidebar_label: "Spectra Logic SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [spectra-logic, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Spectra Logic devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/spectra-logic-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/spectra-logic-snmp-traps"
-->
...
# Spectra Logic SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Spectra Logic SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Spectra Logic devices with Netdata. The bundled Spectra Logic trap profile decodes 11 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Spectra Logic enterprise OID space, and decodes the varbinds using the bundled Spectra Logic trap profile. No per-trap configuration.
...
Traps from Spectra Logic devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **11 trap definitions** from Spectra Logic across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `warning` 9 · `notice` 1 · `info` 1

**By category:** `state_change` 3 · `diagnostic` 8

### Coverage by MIB

| MIB | Trap definitions |
|:----|----:|
| `SPECTRA-LOGIC-STRATA-MIB` | 11 |

### Sample decoded traps

- `SPECTRA-LOGIC-STRATA-MIB::nCPUStatus`
- `SPECTRA-LOGIC-STRATA-MIB::nBootDriveStatus`
