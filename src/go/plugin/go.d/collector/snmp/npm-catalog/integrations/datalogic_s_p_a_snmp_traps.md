<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/datalogic_s_p_a_snmp_traps.md"
sidebar_label: "Datalogic S P A SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [datalogic-s-p-a, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Datalogic S P A devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/datalogic-s-p-a-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/datalogic-s-p-a-snmp-traps"
-->
...
# Datalogic S P A SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Datalogic S P A SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Datalogic S P A devices with Netdata. The bundled Datalogic S P A trap profile decodes 214 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Datalogic S P A enterprise OID space, and decodes the varbinds using the bundled Datalogic S P A trap profile. No per-trap configuration.
...
Traps from Datalogic S P A devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **214 trap definitions** from Datalogic S P A across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 31 · `crit` 60 · `err` 2 · `warning` 104 · `notice` 16 · `info` 1

**By category:** `state_change` 13 · `config_change` 2 · `diagnostic` 199
...
- `NextUPSSystems::cmpACFailure`
- `NextUPSSystems::cmpFanFailure`
