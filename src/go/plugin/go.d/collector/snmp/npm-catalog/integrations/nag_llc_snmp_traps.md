<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/nag_llc_snmp_traps.md"
sidebar_label: "Nag LLC SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [nag-llc, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Nag LLC devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/nag-llc-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/nag-llc-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Nag LLC SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Nag LLC devices with Netdata. The bundled Nag LLC trap profile decodes 348 trap definitions across 9 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Nag LLC enterprise OID space, and decodes the varbinds using the bundled Nag LLC trap profile. No per-trap configuration.
...
Traps from Nag LLC devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **348 trap definitions** from Nag LLC across **9 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 47 · `crit` 21 · `warning` 109 · `notice` 142 · `info` 29

**By category:** `state_change` 96 · `config_change` 4 · `security` 15 · `auth` 4 · `diagnostic` 227 · `unknown` 2
