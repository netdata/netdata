<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/garderos_gmbh_snmp_traps.md"
sidebar_label: "Garderos GmbH SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [garderos-gmbh, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Garderos GmbH devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/garderos-gmbh-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/garderos-gmbh-snmp-traps"
-->
...
# Garderos GmbH SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Garderos GmbH SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Garderos GmbH devices with Netdata. The bundled Garderos GmbH trap profile decodes 1 trap definition across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Garderos GmbH enterprise OID space, and decodes the varbinds using the bundled Garderos GmbH trap profile. No per-trap configuration.
...
Traps from Garderos GmbH devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **1 trap definition** from Garderos GmbH across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.
...
|:----|----:|
| `GARDEROS-ROUTER-MIB` | 1 |

### Sample decoded traps

- `GARDEROS-ROUTER-MIB::routerNotification`
