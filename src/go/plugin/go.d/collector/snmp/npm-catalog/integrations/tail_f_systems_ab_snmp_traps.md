<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/tail_f_systems_ab_snmp_traps.md"
sidebar_label: "Tail F Systems AB SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [tail-f-systems-ab, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Tail F Systems AB devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/tail-f-systems-ab-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/tail-f-systems-ab-snmp-traps"
-->
...
# Tail F Systems AB SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Tail F Systems AB SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Tail F Systems AB devices with Netdata. The bundled Tail F Systems AB trap profile decodes 6 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Tail F Systems AB enterprise OID space, and decodes the varbinds using the bundled Tail F Systems AB trap profile. No per-trap configuration.
...
Traps from Tail F Systems AB devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **6 trap definitions** from Tail F Systems AB across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `crit` 2 · `warning` 3 · `notice` 1

**By category:** `state_change` 1 · `diagnostic` 5
