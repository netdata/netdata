<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/gandalf_snmp_traps.md"
sidebar_label: "Gandalf SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [gandalf, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Gandalf devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/gandalf-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/gandalf-snmp-traps"
-->
...
<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Gandalf SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Gandalf devices with Netdata. The bundled Gandalf trap profile decodes 59 trap definitions across 3 MIBs into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Gandalf enterprise OID space, and decodes the varbinds using the bundled Gandalf trap profile. No per-trap configuration.
...
Traps from Gandalf devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **59 trap definitions** from Gandalf across **3 MIBs** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 4 · `crit` 9 · `err` 2 · `warning` 27 · `notice` 15 · `info` 2

**By category:** `state_change` 23 · `config_change` 5 · `security` 3 · `diagnostic` 27 · `unknown` 1
...
|:----|----:|
| `XPRESS-MIB` | 40 |
| `GANDALF-ENTERPRISE-MIB` | 14 |
| `GANDALF-ROUTER-MIB` | 5 |

### Sample decoded traps

- `GANDALF-ENTERPRISE-MIB::gCardInstalled`
- `GANDALF-ENTERPRISE-MIB::gCardRemoved`
