<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/pure_storage_snmp_traps.md"
sidebar_label: "Pure Storage SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [pure-storage, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Pure Storage devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/pure-storage-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/pure-storage-snmp-traps"
-->
...
# Pure Storage SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Pure Storage SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Pure Storage devices with Netdata. The bundled Pure Storage trap profile decodes 3 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Pure Storage enterprise OID space, and decodes the varbinds using the bundled Pure Storage trap profile. No per-trap configuration.
...
Traps from Pure Storage devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **3 trap definitions** from Pure Storage across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `crit` 1 · `warning` 1 · `notice` 1
