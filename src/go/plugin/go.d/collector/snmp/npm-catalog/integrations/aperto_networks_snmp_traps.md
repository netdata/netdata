<!--
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/snmp/npm-catalog/integrations/aperto_networks_snmp_traps.md"
sidebar_label: "Aperto Networks SNMP Traps"
learn_status: "Published"
learn_rel_path: "Network Performance Monitoring/SNMP Traps/Integrations"
keywords: [aperto-networks, snmp, trap, traps, inform, notification, npm]
description: "Receive, decode, and store SNMP traps and INFORMs from Aperto Networks devices with Netdata."
...
learn_link: "https://learn.netdata.cloud/docs/network-performance-monitoring/snmp-traps/integrations/aperto-networks-snmp-traps"
slug: "/network-performance-monitoring/snmp-traps/integrations/aperto-networks-snmp-traps"
-->
...
# Aperto Networks SNMP Traps


<img src="https://netdata.cloud/img/SNMP.png" width="150" alt="Aperto Networks SNMP Traps"/>
...
Receive, decode, and store SNMP traps and INFORMs from Aperto Networks devices with Netdata. The bundled Aperto Networks trap profile decodes 70 trap definitions across 1 MIB into structured journal events with named, typed varbinds — searchable and filterable in the Logs tab.

Netdata's SNMP trap listener receives traps on UDP/162, matches them to the Aperto Networks enterprise OID space, and decodes the varbinds using the bundled Aperto Networks trap profile. No per-trap configuration.
...
Traps from Aperto Networks devices are decoded automatically once the device is pointed at the Agent's trap listener.
...
Netdata decodes **70 trap definitions** from Aperto Networks across **1 MIB** into structured journal events. A trap is decoded **only if the device actually sends it**; inclusion here means the profile can decode it.

**By severity:** `alert` 3 · `crit` 11 · `err` 6 · `warning` 21 · `notice` 29

**By category:** `state_change` 17 · `config_change` 9 · `diagnostic` 44
...
- `DEVTRAPS-MIB::aniDevTrapSuSyncAcquired`
- `DEVTRAPS-MIB::aniDevTrapBsuSuUp`
