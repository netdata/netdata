...
unmatched `sysObjectID` does not fall back to that list. `snmputils.GetSysInfo` accepts a numeric OID sent as
`OctetString` text and treats any other non-OID value as empty, so such devices use `manual_profiles`; the probe
records only the value's ASN.1 type. After catalog matching, consumer projection keeps metrics,
