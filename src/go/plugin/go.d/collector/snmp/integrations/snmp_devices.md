...
|  | topology.refresh_every | How often to refresh topology snapshots independently from normal SNMP metric collection. Static jobs default to 30m. Jobs generated by the stock SNMP discovery template inherit the discovery rescan interval when it is periodic. | 30m | no |
|  | topology.stale_after | How long to keep the last successful topology snapshot after refresh failures before excluding it from topology responses. Defaults to 2x topology.refresh_every. | 1h | no |
