...
type legacyBGPFixtureCase struct {
	sysObjectID  string
	profileFile  string
	fixture      string
	tables       map[string][]string
	virtuals     []string
	walkPrefixes map[string]string
...
func TestCollector_Collect_AristaAndDellBGP_FromLibreNMSFixtures(t *testing.T) {
	tests := map[string]legacyBGPFixtureCase{
		"Arista EOS emits peer and prefix metrics": {
		...
		"Dell OS10 emits peer and prefix metrics": {
			...
			virtuals: []string{"bgpPeerAvailability", "bgpPeerUpdates"},
			walkPrefixes: map[string]string{
				"dell.os10bgp4V2PeerTable":           "1.3.6.1.4.1.674.11000.5000.200.1.1.2.",
				"dell.os10bgp4V2PeerErrorsTable":     "1.3.6.1.4.1.674.11000.5000.200.1.1.3.",
				"dell.os10bgp4V2PeerEventTimesTable": "1.3.6.1.4.1.674.11000.5000.200.1.1.4.",
				"dell.os10bgp4V2PeerCountersTable":   "1.3.6.1.4.1.674.11000.5000.200.1.1.7.",
				"dell.os10bgp4V2PrefixGaugesTable":   "1.3.6.1.4.1.674.11000.5000.200.1.1.8.",
			},
			validate: func(t *testing.T, metrics []ddsnmp.Metric) {
				availability := requireMetricWithTags(t, metrics, "bgpPeerAvailability", map[string]string{
					"routing_instance": "1",
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			tc.validate(t, collectLegacyBGPMetricsFromFixture(t, tc))
		})
	}
}

func collectLegacyBGPMetricsFromFixture(t *testing.T, tc legacyBGPFixtureCase) []ddsnmp.Metric {
	t.Helper()
	...
	walks := make(map[string][]gosnmp.SnmpPDU, len(tc.walkPrefixes))
	for table, prefix := range tc.walkPrefixes {
		walks[table] = snmprecPDUsWithPrefix(pdus, prefix)
	}

	ctrl, mockHandler := setupMockHandler(t)
	defer ctrl.Finish()

	setProfileWalkExpectations(t, mockHandler, profile, walks)
