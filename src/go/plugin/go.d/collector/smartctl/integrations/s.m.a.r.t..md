<!--startmeta
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/plugin/go.d/collector/smartctl/integrations/s.m.a.r.t..md"
...
keywords: ['smart', 'S.M.A.R.T.', 'SCSI devices', 'ATA devices', 'Windows']
...
It relies on the [`smartctl`](https://linux.die.net/man/8/smartctl) CLI tool.
On Linux and BSD, it executes `smartctl` through `ndsudo`, a Netdata helper specifically designed to run privileged commands securely within the Netdata environment.
On Windows, it executes the installed `smartctl.exe` directly from `PATH` or the default smartmontools installation directory.

Executed commands:
-  `smartctl --json --scan` or `smartctl --json --scan-open`
...
On Windows, install smartmontools and make sure `smartctl.exe` is available in `PATH` or under the default `Program Files\smartmontools\bin\smartctl.exe` location.
