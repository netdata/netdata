...
const (
	...
	prioNodeClusterCommunicationBandwidth
...
var nodeChartsTmpl = module.Charts{
	...
	nodeBandwidthChartTmpl.Copy(),

	nodeRetainMessagesChartsTmpl.Copy(),
	nodeRetainMemoryUsageChartTmpl.Copy(),

	nodeClusterCommunicationBandwidthChartTmpl.Copy(),
...
var nodeMqtt5ChartsTmpl = module.Charts{
	...
	nodeMqttDISCONNECTPacketsChartTmpl.Copy(),
...
var (
	...
	nodeClientKeepaliveExpiredChartTmpl = module.Chart{
		ID:       "node_%s_client_keepalive_expired",
		...
		Dims: module.Dims{
			{ID: dimNode(metricClientKeepaliveExpired), Name: "closed", Algo: module.Incremental},
		},
	}
...
var (
	...
	nodeRouterMemoryChartTmpl = module.Chart{
		...
		Dims: module.Dims{
			{ID: dimNode(metricRouterMemory), Name: "used", Div: 1024},
		},
	}
)
...
var (
	...
	nodeSystemIOChartTmpl = module.Chart{
		...
		Dims: module.Dims{
			{ID: dimNode(metricSystemIOIn), Name: "received", Algo: module.Incremental, Mul: 8, Div: 1024},
			{ID: dimNode(metricSystemIOOut), Name: "sent", Algo: module.Incremental, Mul: 8, Div: -1024},
		},
	}
	...
	nodeSystemMemoryAllocatedChartTmpl = module.Chart{
		...
		Dims: module.Dims{
			{ID: dimNode(metricVMMemoryProcesses), Name: "processes", Div: 1024},
			{ID: dimNode(metricVMMemorySystem), Name: "system", Div: 1024},
		},
	}
)
...
// Bandwidth
var (
	nodeBandwidthChartTmpl = module.Chart{
		ID:       "node_%s_bandwidth",
		Title:    "Bandwidth",
		Units:    "bytes/s",
		Fam:      "bandwidth",
		Ctx:      "vernemq.node_bandwidth",
		Type:     module.Area,
		Priority: prioNodeBandwidth,
		Dims: module.Dims{
...
var (
	...
	nodeRetainMemoryUsageChartTmpl = module.Chart{
		...
		Dims: module.Dims{
			{ID: dimNode(metricRetainMemory), Name: "used", Div: 1024},
		},
	}
)
...
// Cluster
var (
	nodeClusterCommunicationBandwidthChartTmpl = module.Chart{
		ID:       "node_%s_cluster_bandwidth",
		Title:    "Communication with Other Cluster Nodes",
		Units:    "bytes/s",
		Fam:      "cluster",
		Ctx:      "vernemq.node_cluster_bandwidth",
		Type:     module.Area,
		Priority: prioNodeClusterCommunicationBandwidth,
		Dims: module.Dims{
			{ID: dimNode(metricClusterBytesReceived), Name: "received", Algo: module.Incremental, Mul: 8},
...
var (
	...
	nodeMqttAUTHReceivedByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_auth_received_by_reason_code",
	...
	nodeMqttAUTHSentByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_auth_sent_by_reason_code",
...
var (
	...
	nodeMqttCONNACKSentByReturnCodeChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_connack_sent_by_return_code",
	...
	nodeMqttCONNACKSentByReasonCodeChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_connack_sent_by_reason_code",
...
// DISCONNECT
var (
	nodeMqttDISCONNECTPacketsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_disconnect",
	...
	nodeMqttDISCONNECTReceivedByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_disconnect_received_by_reason_code",
	...
	nodeMqttDISCONNECTSentByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_disconnect_sent_by_reason_code",
...
var (
	nodeMqttSUBSCRIBEPacketsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_subscribe",
		Title:    "MQTT SUBSCRIBE and SUBACK",
		Units:    "packets/s",
		Fam:      "mqtt subscribe",
	...
	modeMqttSUBSCRIBEErrorsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_subscribe_error",
		...
		Dims: module.Dims{
			{ID: dimMqttVer(metricSUBSCRIBEError), Name: "failed", Algo: module.Incremental},
		},
	}
	nodeMqttSUBSCRIBEAuthErrorsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_subscribe_auth_error",
		...
		Dims: module.Dims{
			{ID: dimMqttVer(metricSUBSCRIBEAuthError), Name: "unauth", Algo: module.Incremental},
		},
	}
)
...
var (
	nodeMqttUNSUBSCRIBEPacketsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_unsubscribe",
		Title:    "MQTT UNSUBSCRIBE and UNSUBACK",
		Units:    "packets/s",
	...
	nodeMqttUNSUBSCRIBEErrorsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_unsubscribe_error",
		...
		Dims: module.Dims{
			{ID: dimMqttVer(metricUNSUBSCRIBEError), Name: "failed", Algo: module.Incremental},
		},
	}
)
...
var (
	...
	nodeMqttPUBLISHErrorsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_publish_errors",
		...
		Dims: module.Dims{
			{ID: dimMqttVer(metricPUBLISHError), Name: "failed", Algo: module.Incremental},
		},
	}
	nodeMqttPUBLISHAuthErrorsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_publish_auth_errors",
		...
		Dims: module.Dims{
			{ID: dimMqttVer(metricPUBLISHAuthError), Name: "unauth", Algo: module.Incremental},
		},
	}
	nodeMqttPUBACKPacketsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_puback",
		Title:    "MQTT QoS 1 PUBACK",
		Units:    "packets/s",
		Fam:      "mqtt publish",
	...
	nodeMqttPUBACKReceivedByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_puback_received_by_reason_code",
	...
	nodeMqttPUBACKSentByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_puback_sent_by_reason_code",
	...
	nodeMqttPUBACKUnexpectedMessagesChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_puback_unexpected",
	...
	nodeMqttPUBRECPacketsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_pubrec",
		Title:    "MQTT PUBREC QoS 2",
		Units:    "packets/s",
		Fam:      "mqtt publish",
	...
	nodeMqttPUBRECReceivedByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_pubrec_received_by_reason_code",
	...
	nodeMqttPUBRECSentByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_pubrec_sent_by_reason_code",
	...
	nodeMqttPUBRECUnexpectedMessagesChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_pubrec_unexpected",
	...
	nodeMqttPUBRELPacketsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_pubrel",
		Title:    "MQTT PUBREL QoS 2",
		Units:    "packets/s",
		Fam:      "mqtt publish",
	...
	nodeMqttPUBRELReceivedByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_pubrel_received_by_reason_code",
	...
	nodeMqttPUBRELSentByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_pubrel_sent_by_reason_code",
	...
	nodeMqttPUBCOMPPacketsChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_pubcomp",
		Title:    "MQTT PUBCOMP QoS 2",
		Units:    "packets/s",
		Fam:      "mqtt publish",
	...
	nodeMqttPUBCOMPReceivedByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_pubcomp_received_by_reason_code",
	...
	nodeMqttPUBCOMPSentByReasonChartTmpl = func() module.Chart {
		chart := module.Chart{
			ID:       "node_%s_mqtt_v%s_pubcomp_sent_by_reason_code",
	...
	nodeMqttPUBCOMPUnexpectedMessagesChartTmpl = module.Chart{
		ID:       "node_%s_mqtt_v%s_pubcomp_unexpected",
...
func (v *VerneMQ) addNodeCharts(node string) {
	...
	for _, chart := range *charts {
		...
		chart.ID = fmt.Sprintf(chart.ID, node)
		for _, dim := range chart.Dims {
			dim.ID = fmt.Sprintf(dim.ID, node)
		}
	}

	for _, ver := range []string{"4", "5"} {
		var mqttCharts *module.Charts

		switch ver {
		case "4":
			mqttCharts = nodeMqtt4ChartsTmpl.Copy()
		case "5":
			mqttCharts = nodeMqtt5ChartsTmpl.Copy()
		default:
			continue
		}
		for _, chart := range *mqttCharts {
			chart.ID = fmt.Sprintf(chart.ID, node, ver)
			chart.Labels = []module.Label{
				{Key: "node", Value: node},
				{Key: "mqtt_version", Value: ver},
			}
			for _, dim := range chart.Dims {
				dim.ID = fmt.Sprintf(dim.ID, node, ver)
			}
		}
		*charts = append(*charts, *mqttCharts...)
	}
...
func dimNode(name string) string               { return name + "_node_%s" }
func dimMqttVer(name string) string            { return dimNode(name) + "_mqtt_ver_%s" }
func dimMqttReason(name, reason string) string { return dimMqttVer(name) + "_reason_code_" + reason }
func dimMqttRCode(name, rcode string) string   { return dimMqttVer(name) + "_return_code_" + rcode }
