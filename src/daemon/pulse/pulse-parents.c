...
void pulse_receiver_waiting(int16_t hops __maybe_unused) {
    __atomic_add_fetch(&nodes_receivers_waiting, 1, __ATOMIC_RELAXED);
}

void pulse_receiver_not_waiting(int16_t hops __maybe_unused) {
    __atomic_sub_fetch(&nodes_receivers_waiting, 1, __ATOMIC_RELAXED);
}

void pulse_receiver_running(int16_t hops __maybe_unused) {
    __atomic_add_fetch(&nodes_receivers_running, 1, __ATOMIC_RELAXED);
}

void pulse_receiver_not_running(int16_t hops __maybe_unused, STREAM_HANDSHAKE reason __maybe_unused) {
...
void pulse_sender_not_running(int16_t hops __maybe_unused, STREAM_HANDSHAKE reason __maybe_unused, bool from_receiver __maybe_unused) {
    __atomic_sub_fetch(&nodes_senders_running, 1, __ATOMIC_RELAXED);
}

void pulse_sender_connecting(void) {
    __atomic_add_fetch(&nodes_senders_connecting, 1, __ATOMIC_RELAXED);
}

void pulse_sender_not_connecting(void) {
    __atomic_sub_fetch(&nodes_senders_connecting, 1, __ATOMIC_RELAXED);
}
