...
void dbengine_retention_statistics(bool extended __maybe_unused) {
    ...
    for (size_t tier = 0; tier < nd_profile.storage_tiers; tier++) {
        ...
        //                           datafile
        uint64_t disk_space = dbengine_get_used_disk_space(dbengine_multidb_ctx[tier]);
        ...
        if (!config_disk_space) {
            config_disk_space = dbengine_get_directory_free_bytes_space(dbengine_multidb_ctx[tier]);
        ...
        time_t max_retention_s = dbengine_max_retention_s(dbengine_multidb_ctx[tier]);
