...
|       script to execute on alarm       | `/usr/libexec/netdata/plugins.d/alarm-notify.sh` (non-Windows) | The program that sends Alert notifications. Windows does not execute the legacy `alarm-notify.sh` shell script; configure an explicit native executable instead. Note that in versions before 1.16, the plugins.d directory may be installed in a different location in certain OSs (e.g. under `/usr/lib/netdata`). |
