...
void sql_health_alarm_log_load(RRDHOST *host)
{
    ...
    while (service_running(SERVICE_HEALTH) && sqlite3_step_monitored(res) == SQLITE_ROW) {
        ...
        // EXEC_IN_PROGRESS is a transient runtime flag that tracks a live
        // popen handle. It must never survive a restart — the popen it
        // referred to is gone, and leaving it set would (a) make deletion
        // gates keep the entry forever, and (b) confuse any code that walks
        // the alarm log looking for in-flight notifications.
