...
size_t dbengine_decompress(void *dst, void *src, size_t dst_size, size_t src_size, uint8_t algorithm) {
    switch(algorithm) {
        ...
        case DBENGINE_COMPRESSION_LZ4: {
            ...
            if(rc < 0) {
                dbengine_log(NULL, NDLP_ERR, "DBENGINE: LZ4 decompression error %d", rc);
                rc = 0;
            }
        ...
        case DBENGINE_COMPRESSION_ZSTD: {
            ...
            if (ZSTD_isError(decompressed_size)) {
                dbengine_log(NULL, NDLP_ERR, "DBENGINE: ZSTD decompression error %s",
