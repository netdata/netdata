...
// throttling state is one object whichever arm runs. Two deliberate differences from the original, both of them
// forced: it cannot see nd_log's single-threaded-child elision, so it always takes the spinlock, and it applies no
// priority pre-filter, because filtering is the sink's job
