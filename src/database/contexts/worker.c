...
static void rrdinstance_load_dimension_callback(SQL_DIMENSION_DATA *sd, void *data) {
    ...
    if (unlikely(!acquired)) {
        rca = (RRDCONTEXT_ACQUIRED *)dictionary_get_and_acquire_item(host->rrdctx.contexts, sd->context);
        if(!rca) {
        ...
        rc = rrdcontext_acquired_value(rca);

        ria = (RRDINSTANCE_ACQUIRED *)dictionary_get_and_acquire_item(rc->rrdinstances, sd->chart_id);
        if(!ria) {
        ...
        ri = rrdinstance_acquired_value(ria);
    } else
        ri = sd->ri;
...
void rrdhost_load_rrdcontext_data(RRDHOST *host) {
    ...
    ctx_get_dimension_list(host, rrdinstance_load_dimension_callback, host);
