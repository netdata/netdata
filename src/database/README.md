...
| `alloc`    | Uses dynamically allocated memory (heap) for a fixed-size circular buffer. Not persistent across restarts. Automatically used as a fallback when the `ram` mode cannot use memory-mapped files (mmap), or when `dbengine` is not available. |
