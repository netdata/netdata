...
static inline bool ml_nonnegative_time_t_fits_int64(int64_t value)
{
    return value >= 0;
}

template <>
inline bool ml_nonnegative_time_t_fits_int64<true>(int64_t value)
{
...
bool ml_kmeans_deserialize(ml_kmeans_inlined_t *inlined_km, struct json_object *root)
{
    ...
    if (!ml_nonnegative_time_t_fits_int64<(sizeof(time_t) < sizeof(int64_t))>(raw_after)) {
    ...
    if (!ml_nonnegative_time_t_fits_int64<(sizeof(time_t) < sizeof(int64_t))>(raw_before)) {
