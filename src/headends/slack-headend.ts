...
export class SlackHeadend implements Headend {
  ...
  private resolveRoute?: (args: { kind: 'mentions' | 'channel-posts' | 'dms'; channelId: string; channelName?: string }) => Promise<RoutingResolution | undefined>;
  ...
  public async start(context: HeadendContext): Promise<void> {
    ...
    const { slackConfig, apiConfig, systemPrompt, defaultSessions, resolveRoute } = this.initializeAgents(primaryPath);

    const slackBotTokenRaw = slackConfig.botToken;
    const slackAppTokenRaw = slackConfig.appToken;
    if (typeof slackBotTokenRaw !== 'string' || slackBotTokenRaw.length === 0 || typeof slackAppTokenRaw !== 'string' || slackAppTokenRaw.length === 0) {
    ...
    this.slackSigningSecret = typeof slackConfig.signingSecret === 'string' && slackConfig.signingSecret.length > 0
      ? slackConfig.signingSecret
      : undefined;

    this.fallbackPort = typeof apiConfig.port === 'number' && Number.isFinite(apiConfig.port) ? apiConfig.port : 8080;
    ...
    const historyLimit = typeof slackConfig.historyLimit === 'number' ? slackConfig.historyLimit : 100;
    const historyCharsCap = typeof slackConfig.historyCharsCap === 'number' ? slackConfig.historyCharsCap : 100000;
    const updateIntervalMs = typeof slackConfig.updateIntervalMs === 'number' ? slackConfig.updateIntervalMs : 2000;
    ...
    initSlackHeadend({
      ...
      registerRunSlot: (session, runId, release) => { this.registerRunRelease(session, runId, release); },
    });
  ...
  public async stop(): Promise<void> {
    ...
    if (this.fallbackServer !== undefined) {
      await new Promise<void>((resolve) => { this.fallbackServer?.close(() => resolve()); });
      this.fallbackServer = undefined;
    }
    this.closeDeferred.resolve({ reason: 'stopped', graceful: true });
  }

  private async startFallbackServer(port: number, route: RestExtraRoute): Promise<void> {
    ...
    const server = http.createServer((req, res) => {
      void (async () => {
        ...
        if (method === route.method && pathNormalized === route.path) {
          await route.handler({ req, res, url, requestId: crypto.randomUUID() });
          return;
        }
        res.statusCode = 404;
        res.end('not_found');
  ...
  private async handleSlashCommandRequest(req: http.IncomingMessage, res: http.ServerResponse, url: URL, requestId: string): Promise<void> {
    try {
  ...
  private parseOpenerTone(value: unknown): SlackOpenerTone | undefined {
    if (value === 'random' || value === 'cheerful' || value === 'formal' || value === 'busy') {
      return value;
    }
    return undefined;
  }

  private async processSlashCommand(args: { command: string; text: string; userId: string; channelId: string; teamId: string; responseUrl: string; requestId: string }): Promise<void> {
    try {
      const clientUnknown = this.slackClient;
      const routeResolver = this.resolveRoute;
      if (clientUnknown === undefined || routeResolver === undefined || this.defaultSession === undefined) return;
      const client = clientUnknown as any;

      const channelInfo = await (client as any).conversations.info({ channel: args.channelId });
      const channelName = typeof channelInfo?.channel?.name === 'string' ? channelInfo.channel.name : undefined;
      const resolvedRoute = await routeResolver({ kind: 'channel-posts', channelId: args.channelId, channelName });
      const activeSessions = resolvedRoute?.sessions ?? this.defaultSession;
      const activeSystem = resolvedRoute?.systemPrompt ?? this.defaultSystemPrompt ?? '';
      if (activeSessions === undefined || activeSystem.length === 0) return;

      const when = new Date().toLocaleString();
      const whoLabel = await this.lookupUserLabel(args.userId);
      const UNKNOWN = 'unknown';
      const template = resolvedRoute?.promptTemplates?.channelPost ?? [
        ...
        if (args.responseUrl !== '') {
          try {
            await fetch(args.responseUrl, {
              ...
              body: JSON.stringify({ response_type: 'ephemeral', text: 'Not a channel member; continuing in DM…' }),
            });
          } catch { /* ignore */ }
        }
      ...
      const opener = whoLabel.length > 0 ? `Starting… (${whoLabel})` : 'Starting…';
      const initial = await client.chat.postMessage({ channel: targetChannel, text: opener });
      const liveTs = String(initial.ts ?? Date.now());
      const release = await this.slackLimiter.acquire();
      let runId: string;
      try {
        runId = activeSessions.startRun(
          { source: 'slack', teamId: args.teamId, channelId: targetChannel, threadTsOrSessionId: liveTs },
          activeSystem,
          userPrompt,
          [],
  ...
  private async pollAndFinalize(sessionManager: SessionManager, runId: string, channel: string, ts: string): Promise<void> {
    ...
    const out = (() => {
      ...
      if (result !== undefined && typeof result.finalReport?.content === 'string' && result.finalReport.content.length > 0) {
        return result.finalReport.content;
      }
      if (meta !== undefined && typeof meta.error === 'string' && meta.error.length > 0) {
        return `❌ ${meta.error}`;
      }
      return '✅ Done';
    })();
    await client.chat.postMessage({ channel, thread_ts: ts, text: out.substring(0, 3500) });
  }
  ...
  private async lookupUserLabel(userId: string): Promise<string> {
    ...
    try {
      ...
      const primary = real.length > 0 ? real : (display.length > 0 ? display : userId);
      const extras: string[] = [];
  ...
  private verifySlackSignature(req: http.IncomingMessage, rawBody: string): boolean {
    ...
    const hmac = crypto.createHmac('sha256', this.slackSigningSecret).update(base).digest('hex');
    const expected = `v0=${hmac}`;
    return crypto.timingSafeEqual(Buffer.from(expected), Buffer.from(sig));
  }

  private initializeAgents(primaryAgentPath: string): {
    slackConfig: { botToken?: string; appToken?: string; signingSecret?: string; mentions?: boolean; dms?: boolean; updateIntervalMs?: number; historyLimit?: number; historyCharsCap?: number; openerTone?: 'random' | 'cheerful' | 'formal' | 'busy'; routing?: unknown };
    apiConfig: { port?: number };
    systemPrompt: string;
    defaultSessions: SessionManager;
    resolveRoute: (args: { kind: 'mentions' | 'channel-posts' | 'dms'; channelId: string; channelName?: string }) => Promise<RoutingResolution | undefined>;
  } {
    ...
    const expandStrict = (obj: unknown, vars: Record<string, string>, section: string): unknown => {
      if (typeof obj === 'string') {
        return obj.replace(/\$\{([^}]+)\}/g, (_m, n: string) => {
          const v = vars[n];
          if (v === undefined) throw new Error(`Unresolved variable \${${n}} in section '${section}'`);
          return v;
        });
      }
    ...
    const compiled = this.compileRouting(slackResolved.routing, primaryAgentPath);

    const resolveRoute = (args: { kind: 'mentions' | 'channel-posts' | 'dms'; channelId: string; channelName?: string }): Promise<RoutingResolution | undefined> => {
      // deny first
      const channelNameValue = typeof args.channelName === 'string' && args.channelName.length > 0 ? args.channelName : undefined;
      // eslint-disable-next-line functional/no-loop-statements
      for (const deny of compiled.denies) {
        if (deny.engage.size > 0 && !deny.engage.has(args.kind)) continue;
        if (deny.channelIdPatterns.some((re) => re.test(args.channelId))) return Promise.resolve(undefined);
        if (channelNameValue !== undefined && deny.channelNamePatterns.some((re) => re.test(channelNameValue))) return Promise.resolve(undefined);
      }
      // eslint-disable-next-line functional/no-loop-statements
      for (const rule of compiled.rules) {
        if (rule.engage.size > 0 && !rule.engage.has(args.kind)) continue;
        const idMatch = rule.channelIdPatterns.some((re) => re.test(args.channelId));
        const nameMatch = channelNameValue !== undefined ? rule.channelNamePatterns.some((re) => re.test(channelNameValue)) : false;
        if (idMatch || nameMatch) {
          const rec = this.agentCache.get(rule.agentPath);
          if (!rec) continue;
          return Promise.resolve({ sessions: rec.sessions, systemPrompt: rec.loaded.systemTemplate, promptTemplates: rule.promptTemplates, contextPolicy: rule.contextPolicy });
        }
      }
      const def = compiled.default;
      if (!def) return Promise.resolve(undefined);
      const engageSet = def.engage;
      const engageConfigured = engageSet.size > 0 ? engageSet.has(args.kind) : args.kind === 'mentions' ? slackResolved.mentions !== false : args.kind === 'dms' ? slackResolved.dms !== false : false;
      ...
      return Promise.resolve({ sessions: rec.sessions, systemPrompt: rec.loaded.systemTemplate, promptTemplates: def.promptTemplates, contextPolicy: def.contextPolicy });
    };
  ...
  private compileRouting(rawRouting: unknown, agentPath: string): CompiledRouting {
    const routing = (rawRouting ?? {}) as Record<string, unknown>;
    const compiled: CompiledRouting = { default: undefined, denies: [], rules: [] };
    ...
    const resolveAgentPath = (p: string): string => (path.isAbsolute(p) ? p : path.resolve(path.dirname(agentPath), p));
    ...
    if (defaultRouting && typeof defaultRouting === 'object') {
      ...
      const agent = typeof def.agent === 'string' && def.agent.length > 0 ? def.agent : agentPath;
      preloadAgent(agent);
      compiled.default = {
        agentPath: resolveAgentPath(agent),
        engage,
        promptTemplates: def.promptTemplates as { mention?: string; dm?: string; channelPost?: string } | undefined,
        contextPolicy: def.contextPolicy as { channelPost?: 'selfOnly' | 'previousOnly' | 'selfAndPrevious' } | undefined,
      };
    } else {
      preloadAgent(agentPath);
    }

    const denyList = Array.isArray(routing.deny) ? routing.deny as Record<string, unknown>[] : [];
    denyList.forEach((deny) => {
      const { namePats, idPats } = parseChannels(deny.channels);
      compiled.denies.push({ channelNamePatterns: namePats, channelIdPatterns: idPats, engage: toEngage(deny.engage) });
    });

    const rules = Array.isArray(routing.rules) ? routing.rules as Record<string, unknown>[] : [];
    rules.forEach((rule) => {
      if (rule.enabled === false) return;
      const agent = typeof rule.agent === 'string' && rule.agent.length > 0 ? rule.agent : agentPath;
      preloadAgent(agent);
      ...
      compiled.rules.push({
        ...
        promptTemplates: rule.promptTemplates as { mention?: string; dm?: string; channelPost?: string } | undefined,
        contextPolicy: rule.contextPolicy as { channelPost?: 'selfOnly' | 'previousOnly' | 'selfAndPrevious' } | undefined,
      });
    });
  ...
  private registerRunRelease(session: SessionManager, runId: string, release: () => void): void {
    if (!this.registeredSessions.has(session)) {
      session.onTreeUpdate((id) => {
        ...
        if (meta === undefined || (meta.status !== 'running' && meta.status !== 'stopping')) {
