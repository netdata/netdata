...
# dirty hack for email notifcations to work with bash 3
# only default destination is supported 
...
do
    ...
    [[ -z "${a}" || ${a} == '0' ]] && a="${DEFAULT_RECIPIENT_EMAIL}"
    for r in ${a//,/ }
    do
...
[[ "${to_email}" == "0" ]] && to_email=${DEFAULT_RECIPIENT_EMAIL}
