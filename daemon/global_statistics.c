...
static inline void sqlite3_statistics_copy(struct sqlite3_statistics *gs) {
    ...
    usec_t timeout = rrdb.default_rrd_update_every * USEC_PER_SEC + rrdb.default_rrd_update_every * USEC_PER_SEC / 3;
...
void *global_statistics_main(void *ptr)
{
    ...
    int update_every =
        (int)config_get_number(CONFIG_SECTION_GLOBAL_STATISTICS, "update every", rrdb.localhost->rrd_update_every);
