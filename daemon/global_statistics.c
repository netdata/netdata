...
void global_statistics_charts(void) {
    ...
    if (hosts_with_dbengine) {
...

        {
            ...
            static RRDDIM *rd_flushing_warnings = NULL;
            ...
            if (unlikely(!st_errors)) {
                ...
                rd_flushing_warnings = rrddim_add(st_errors, "flushing_warnings", NULL, 1, 1, RRD_ALGORITHM_INCREMENTAL);
                rd_flushing_errors = rrddim_add(st_errors, "flushing_errors", NULL, 1, 1, RRD_ALGORITHM_INCREMENTAL);
            }
            else
            ...
            rrddim_set_by_pointer(st_errors, rd_flushing_warnings, (collected_number)stats_array[34]);
            rrddim_set_by_pointer(st_errors, rd_flushing_errors, (collected_number)stats_array[36]);
