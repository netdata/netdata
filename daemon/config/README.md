...
|           severity-level           |           `info`                | The severity level allows user to select the messages type stored inside logs. Two levels are allowed: `info` - is the lowest level that stores only general information; `error` - it stores previous level and also all errors detected by netdata and plugins during runtime.   |
