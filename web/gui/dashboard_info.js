...
netdataDashboard.context = {
   ...
   // Apps eBPF stuff
    ...
    'postgres.checkpoints': {
        info: '<p>Number of checkpoints that have been performed. Checkpoints are periodic maintenance operations the database performs to make sure that everything it’s been caching in memory has been synchronized with the disk. This is essential in case of failure scenarios as the standby server will know where to start replaying transactions from.</p><p>A high number of checkpoints being requested may indicate your databases are under heavy load. Increasing <a href="https://www.postgresql.org/docs/current/runtime-config-wal.html#GUC-MAX-WAL-SIZE" target="_blank"><i>max_wal_size</i></a> may help checkpoints become time-driven (scheduled) as opposed to load-driven (requested), up to a point.</p><p><b>Scheduled</b> - checkpoints triggered as per schedule when time elapsed from the previous checkpoint is greater than <a href="https://www.postgresql.org/docs/current/runtime-config-wal.html#GUC-CHECKPOINT-TIMEOUT" target="_blank"><i>checkpoint_timeout</i></a>. <b>Requested</b> - checkpoints triggered due to WAL updates reaching the <i>max_wal_size</i> before the <i>checkpoint_timeout</i> is reached.</p>'
    },
