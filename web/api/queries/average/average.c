...
void grouping_reset_average(RRDR *r, unsigned int index)
{
    struct grouping_average *g = (struct grouping_average *)r->internal.gf[index].grouping_data;
    g->sum = 0;
    g->count = 0;
}

void grouping_free_average(RRDR *r, unsigned int index)
{
    freez(r->internal.gf[index].grouping_data);
    r->internal.gf[index].grouping_data = NULL;
}

void grouping_add_average(RRDR *r, calculated_number value, unsigned int index)
{
    if (!isnan(value)) {
        struct grouping_average *g = (struct grouping_average *)r->internal.gf[index].grouping_data;
        g->sum += value;
...
calculated_number grouping_flush_average(RRDR *r, RRDR_VALUE_FLAGS *rrdr_value_options_ptr, unsigned int index)
{
    struct grouping_average *g = (struct grouping_average *)r->internal.gf[index].grouping_data;
