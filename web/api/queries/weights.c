...
int web_api_v12_weights(BUFFER *wb, QUERY_WEIGHTS_REQUEST *qwr) {
    ...
    if(!rrdr_relative_window_to_absolute(&qwr->after, &qwr->before, NULL, rrdb.unittest_running))
    ...
    if(qwr->method == WEIGHTS_METHOD_MC_KS2 || qwr->method == WEIGHTS_METHOD_MC_VOLUME) {
        ...
        rrdr_relative_window_to_absolute(&qwr->baseline_after, &qwr->baseline_before, NULL, rrdb.unittest_running);
