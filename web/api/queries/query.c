...
void rrdr_fill_tier_gap_from_smaller_tiers(RRDDIM *rd, size_t tier, time_t now) {
    ...
    for(int read_tier = (int)tier - 1; read_tier >= 0 ; read_tier--){
        time_t smaller_tier_first_time = se_metric_oldest_time(rd->tiers[read_tier]->mode, rd->tiers[read_tier]->db_metric_handle);
        time_t smaller_tier_last_time = se_metric_latest_time(rd->tiers[read_tier]->mode, rd->tiers[read_tier]->db_metric_handle);
