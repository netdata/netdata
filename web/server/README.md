...
-   All IPv6 IPs at port 19996. Only metric streaming requests from other Netdata agents will be accepted on this port. Only encrypted streams will be allowed (i.e. transmitters also need to be [configured for TLS](/streaming/README.md).
...
Since v1.16.0, Netdata supports encrypted HTTP connections to the web server, plus encryption of streaming data between a transmitter and its receiver, via the TLS protocol.
...
-   Allow incoming transmitter connections to use both unencrypted and encrypted communications for streaming.
...
For information how to configure the transmitter to use TLS, check [securing the communication](/streaming/README.md#securing-streaming-communications) in the streaming documentation. There you will find additional details on the expected behavior for client and server nodes, when their respective TLS options are enabled.
...
-   Refuse unencrypted streaming connections from transmitters on the default port.
...
-   `allow streaming from` checks if the transmitter willing to stream metrics to this Netdata is allowed.
...
|accept a streaming request every seconds|`0`|Can be used to set a limit on how often a receiver Netdata server will accept streaming requests from transmitters in a [streaming and replication setup](/streaming/README.md)|
