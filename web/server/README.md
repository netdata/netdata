...
|respect do not track policy|`no`|If set to `yes`, Netdata will respect the user's browser preferences for Do Not Track (DNT) and storing cookies. If DNT is _enabled_ in the browser, and this option is set to `yes`, users will not be able to connect their nodes to Netdata Cloud. For certain browsers, users must disable DNT and change this option to `yes` for full functionality.|
