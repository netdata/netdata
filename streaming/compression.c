...
struct compressor_state *create_compressor()
{
    ...
    state->reset(state);
    info("STREAM_COMPRESSION: Initialize streaming compression!");
    return state;
}
...
static void lz4_decompressor_reset(struct decompressor_state *state)
{
    if (state->data) {
        ...
        info("STREAM_COMPRESSION: Resetting decompressor.");
    }
}
...
static void lz4_decompressor_destroy(struct decompressor_state **state)
{
    if (state && *state) {
        ...
        if (s->data) {
            info("STREAM_COMPRESSION: Destroying decompressor.");
            if (s->data->stream)
...
static size_t decode_compress_header(const char *data, size_t data_size)
{
    ...
    info("STREAM_COMPRESSION: Decoding decompress header.");
    return length;
}
...
static size_t lz4_decompressor_start(struct decompressor_state *state, const char *header, size_t header_size)
{
    ...
    info("STREAM_COMPRESSION: Starting Decompressor!");
...
static size_t lz4_decompressor_put(struct decompressor_state *state, const char *data, size_t size)
{
    ...
    if (state->buffer_pos + size > state->buffer_len) {
        info("STREAM: Decompressor buffer overflow %lu + %lu > %lu",
    ...
    info("STREAM_COMPRESSION: Decompressor put data in the buffer.");
...
static size_t lz4_decompressor_decompress(struct decompressor_state *state)
{
    ...
    info("STREAM_COMPRESSION: Decompressing....");
    ...
    if (old_avg_saving != avg_saving || old_avg_size != avg_size){
        ...
        info("STREAM_COMPRESSION_STATS: Saving: %lu%% (avg. %lu%%), avg.size: %lu", saving, avg_saving, avg_size);
    }
...
static size_t lz4_decompressor_get(struct decompressor_state *state, char *data, size_t size)
{
    ...
    info("STREAM_COMPRESSION: Get the decompressed data.");
...
struct decompressor_state *create_decompressor()
{
    ...
    state->reset(state);
    info("STREAM_COMPRESSION: Initialize streaming decompression!");
    return state;
}
#endif
