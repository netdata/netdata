...
REGISTRY_URL *registry_url_get(const char *url, size_t urllen) {
    ...
    netdata_log_debug(D_REGISTRY, "Registry: registry_url_get('%s', %zu)", url, urllen);
    ...
    if(!u) {
        netdata_log_debug(D_REGISTRY, "Registry: registry_url_get('%s', %zu): allocating %zu bytes", url, urllen, sizeof(REGISTRY_URL) + urllen);
        ...
        netdata_log_debug(D_REGISTRY, "Registry: registry_url_get('%s'): indexing it", url);
...
void registry_url_link(REGISTRY_URL *u) {
    u->links++;
    netdata_log_debug(D_REGISTRY, "Registry: registry_url_link('%s'): URL has now %u links", u->url, u->links);
}

void registry_url_unlink(REGISTRY_URL *u) {
    u->links--;
    if(!u->links) {
        netdata_log_debug(D_REGISTRY, "Registry: registry_url_unlink('%s'): No more links for this URL", u->url);
        ...
        netdata_log_debug(D_REGISTRY, "Registry: registry_url_unlink('%s'): URL has %u links left", u->url, u->links);
}
