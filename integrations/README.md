...
Those packages cover generation and `python3 -m unittest integrations.tests.test_collector_metadata` (the collector
metadata content checks). Running `python3 -m unittest integrations.tests.test_descriptions` also requires
`markdown-it-py`; `./integrations/pip.sh` installs the complete generation-and-test dependency set. The legacy
