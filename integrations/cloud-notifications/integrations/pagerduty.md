...
3. Once the service is created you will be redirected to its configuration page, where you can copy the **Integration Key** and **Integration URL (Alert Events)** fields, that you will need need to add to your notification configuration on Netdata UI.
...
5. A modal will be presented to you to enter the required details to enable the configuration:
  ...
  * **Integration configuration** are the specific notification integration required settings, which vary by notification method. For PagerDuty:
    ...
    - Integration URL (Alert Events) - This is the URL provided by PagerDuty where we will send the notifications.
