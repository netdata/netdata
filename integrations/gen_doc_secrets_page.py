...
SECRETS_PAGE = {
    ...
    "intro": [
        "Secret references resolve in stock collector configurations, user configuration files, and jobs saved through Dynamic Configuration. "
        "By default, jobs generated by service discovery treat all reference syntax as literal text, including references written in discovery rules or credentials.",
        "To allow references in all jobs from a trusted pipeline, set its top-level `trust_discovered_targets: yes`. "
        "Target-controlled values copied by service rules can then make Netdata read local files, run commands, or fetch stored secrets with the secret providers' permissions. "
        "A collector may send the resolved values back to the target. Enable only if you control and trust every discoverable target; enabling accepts that risk.",
