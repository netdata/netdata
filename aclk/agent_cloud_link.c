...
int aclk_submit_request(struct aclk_request *request)
{
    //    char *topic;    // Topic to respond to
    //    char *data;     // Internal data (NULL if request from the cloud)
    //    char *msg_id;   // msg_id generated by the cloud (NULL if internal)
...
int aclk_process_query()
{
    ...
    query_count++;
    info("Query #%d (%s) (%s) in queue %d seconds", query_count, this_query->topic, this_query->query,
...
int aclk_send_metadata_info()
{
    ...
    aclk_create_metadata_message(aclk_buffer, "info", NULL, info_json);
...
int aclk_send_single_chart(char *hostname, char *chart)
{
    ...
    aclk_create_metadata_message(aclk_buffer, ACLK_MSG_TYPE_CHART, NULL, info_json);
...
void aclk_rrdset2json(RRDSET *st, BUFFER *wb, char *hostname, int is_slave)
{
    ...
    buffer_sprintf(
        ...
        "\t\t\t\"update_every\": %d,\n"
        "\t\t\t\"dimensions\": {\n",
        hostname, is_slave, st->id, st->name, st->type, st->family, st->context, st->title, st->name, st->priority,
