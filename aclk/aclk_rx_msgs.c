...
void aclk_handle_new_cloud_msg(const char *message_type, const char *msg, size_t msg_len)
{
    if (!strcmp(message_type, "CreateNodeInstanceResult")) {
        ...
        if (host != localhost) {
            error_report("Currently we do not support children! Will not send node update.");
            freez(res.machine_guid);
            freez(res.node_id);
            return;
        }

        // This will eventually be used host != localhost check above will be removed soon
        if (host) {
