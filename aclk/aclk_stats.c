...
static void aclk_stats_collect(struct aclk_metrics_per_sample *per_sample, struct aclk_metrics *permanent)
{
    ...
    if (unlikely(!st_aclkstats)) {
        st_aclkstats = rrdset_create_localhost(
            ...
            "connected", "netdata", "stats", 200000, rrdb.localhost->rrd_update_every, RRDSET_TYPE_LINE);
...
static void aclk_stats_query_queue(struct aclk_metrics_per_sample *per_sample)
{
    ...
    if (unlikely(!st_query_thread)) {
        st_query_thread = rrdset_create_localhost(
            ...
            "netdata", "stats", 200001, rrdb.localhost->rrd_update_every, RRDSET_TYPE_AREA);

        rd_queued = rrddim_add(st_query_thread, "added", NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
        rd_dispatched = rrddim_add(st_query_thread, "dispatched", NULL, -1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
    }
...
static void aclk_stats_latency(struct aclk_metrics_per_sample *per_sample)
{
    ...
    if (unlikely(!st)) {
        st = rrdset_create_localhost(
            ...
            "netdata", "stats", 200002, rrdb.localhost->rrd_update_every, RRDSET_TYPE_LINE);
...
static void aclk_stats_cloud_req(struct aclk_metrics_per_sample *per_sample)
{
    ...
    if (unlikely(!st)) {
        st = rrdset_create_localhost(
            ...
            "netdata", "stats", 200005, rrdb.localhost->rrd_update_every, RRDSET_TYPE_STACKED);

        rd_rq_rcvd = rrddim_add(st, "received", NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
        rd_rq_err = rrddim_add(st, "malformed", NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
    }
...
static void aclk_stats_cloud_req_type(struct aclk_metrics_per_sample *per_sample)
{
    ...
    if (unlikely(!st)) {
        st = rrdset_create_localhost(
            ...
            "netdata", "stats", 200006, rrdb.localhost->rrd_update_every, RRDSET_TYPE_STACKED);

        for (int i = 0; i < ACLK_QUERY_TYPE_COUNT; i++)
            dims[i] = rrddim_add(st, aclk_query_get_name(i, 1), NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
...
static void aclk_stats_cloud_req_http_type(struct aclk_metrics_per_sample *per_sample)
{
    ...
    if (unlikely(!st)) {
        st = rrdset_create_localhost(
            ...
            "netdata", "stats", 200007, rrdb.localhost->rrd_update_every, RRDSET_TYPE_STACKED);

        for (int i = 0; i < ACLK_STATS_CLOUD_HTTP_REQ_TYPE_CNT; i++)
            rd_rq_types[i] = rrddim_add(st, cloud_req_http_type_names[i], NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
    }
...
static void aclk_stats_query_threads(uint32_t *queries_per_thread)
{
    ...
    if (unlikely(!st)) {
        st = rrdset_create_localhost(
            ...
            "netdata", "stats", 200009, rrdb.localhost->rrd_update_every, RRDSET_TYPE_STACKED);
        ...
        for (int i = 0; i < aclk_stats_cfg.query_thread_count; i++) {
            ...
            aclk_qt_data[i].dim = rrddim_add(st, dim_name, NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
        }
    }
...
static void aclk_stats_query_time(struct aclk_metrics_per_sample *per_sample)
{
    ...
    if (unlikely(!st)) {
        st = rrdset_create_localhost(
            ...
            "netdata", "stats", 200008, rrdb.localhost->rrd_update_every, RRDSET_TYPE_LINE);

        rd_rq_avg = rrddim_add(st, "avg", NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
        rd_rq_max = rrddim_add(st, "max", NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
        rd_rq_total = rrddim_add(st, "total", NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
    }
...
static void aclk_stats_newproto_rx(uint32_t *rx_msgs_sample)
{
    ...
    if (unlikely(!st)) {
        st = rrdset_create_localhost(
            ...
            "netdata", "stats", 200010, rrdb.localhost->rrd_update_every, RRDSET_TYPE_STACKED);

        for (unsigned int i = 0; i < aclk_stats_cfg.proto_hdl_cnt; i++) {
            aclk_stats_cfg.rx_msg_dims[i] = rrddim_add(st, rx_handler_get_name(i), NULL, 1, rrdb.localhost->rrd_update_every, RRD_ALGORITHM_ABSOLUTE);
        }
    }
...
static void aclk_stats_mqtt_wss(struct mqtt_wss_stats *stats)
{
    ...
    if (unlikely(!st)) {
        st = rrdset_create_localhost(
            ...
            "netdata", "stats", 200011, rrdb.localhost->rrd_update_every, RRDSET_TYPE_STACKED);
    ...
    if (unlikely(!st_txbuf_perc)) {
        st_txbuf_perc = rrdset_create_localhost(
            ...
            "netdata", "stats", 200012, rrdb.localhost->rrd_update_every, RRDSET_TYPE_LINE);
    ...
    if (unlikely(!st_txbuf)) {
        st_txbuf = rrdset_create_localhost(
            ...
            "netdata", "stats", 200013, rrdb.localhost->rrd_update_every, RRDSET_TYPE_LINE);
    ...
    if (unlikely(!st_timing)) {
        st_timing = rrdset_create_localhost(
            ...
            "netdata", "stats", 200014, rrdb.localhost->rrd_update_every, RRDSET_TYPE_STACKED);
...
void *aclk_stats_main_thread(void *ptr)
{
    ...
    heartbeat_t hb;
    heartbeat_init(&hb);
    usec_t step_ut = rrdb.localhost->rrd_update_every * USEC_PER_SEC;
