...
#define prometheus_config_get_boolean(name, value)                                                                     \
    ...
    while (tmp_ci_list) {
        ...
        if (EXPORTING_OPTIONS_DATA_SOURCE(tmp_instance->config.options) != EXPORTING_SOURCE_DATA_AS_COLLECTED &&
            tmp_instance->config.update_every % rrdb.localhost->rrd_update_every)
            netdata_log_info(
                ...
                tmp_instance->config.update_every, tmp_instance->config.name, rrdb.localhost->rrd_update_every);
