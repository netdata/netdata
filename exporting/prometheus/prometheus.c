...
inline int can_send_rrdset(struct instance *instance, RRDSET *st, SIMPLE_PATTERN *filter)
{
    ...
    if (rrdset_flag_check(st, RRDSET_FLAG_EXPORTING_IGNORE))
        return 0;
    ...
    } else if (!rrdset_flag_check(st, RRDSET_FLAG_EXPORTING_SEND)) {
...
inline char *prometheus_units_copy(char *d, const char *s, size_t usable, int showoldunits)
{
    ...
    if (showoldunits) {
        ...
        for (i = 0; units[i].newunit; i++) {
            if (hash == units[i].hash && !strcmp(s, units[i].newunit)) {
    ...
    PROMETHEUS_OUTPUT_OPTIONS output_options)
{
    ...
    rrdset_foreach_read(st, host) {
        ...
        if (can_send_rrdset(instance, st, filter)) {
                ...
                if (rd->collector.counter && !rrddim_flag_check(rd, RRDDIM_FLAG_OBSOLETE)) {
                    ...
                    if (as_collected) {
                        ...
                        if (homogeneous) {
                            ...
                            if (output_options & PROMETHEUS_OUTPUT_TYPES)
...
void rrd_stats_api_v1_charts_allmetrics_prometheus_single_host(
    ...
    PROMETHEUS_OUTPUT_OPTIONS output_options)
{
    if (!prometheus_exporter_instance || !prometheus_exporter_instance->config.initialized)
        return;
