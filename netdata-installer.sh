...
USAGE: ${PROGRAM} [options]
  ...
  --enable-plugin-otel-metrics Enable the Netdata OpenTelemetry Metrics plugin. Default: disabled
  --disable-plugin-otel-metrics Explicitly disable the Netdata OpenTelemetry Metrics plugin.
...
while [ -n "${1}" ]; do
  case "${1}" in
    ...
    "--enable-plugin-otel-metrics") ENABLE_NOM=1 ;;
    "--disable-plugin-otel-metrics") ENABLE_NOM=0 ;;
