                                    ...
                                    void *buf, unsigned max_size)
{
    ...
    for (i = 0, valid_pages = 0 ; i < count ; ++i) {
        ...
        if (unlikely(start_time > end_time)) {
            error("Invalid page encountered, start time %lu > end time %lu", (long unsigned int) start_time , (long unsigned int) end_time );
            continue;
        }
        ...
        if (unlikely(start_time == end_time)) {
            ...
            if (unlikely(entries > 1)) {
                error("Invalid page encountered, start time %lu = end time but %zu entries were found", (long unsigned int) start_time, entries);
                continue;
            }
        }
