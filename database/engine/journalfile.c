...
void journalfile_v2_populate_retention_to_mrg(struct rrdengine_instance *ctx, struct rrdengine_journalfile *journalfile) {
    ...
    netdata_log_debug("DBENGINE: journal v2 of tier %d, datafile %u populated, size: %0.2f MiB, metrics: %0.2f k, %0.2f ms"
...
int journalfile_v2_load(struct rrdengine_instance *ctx, struct rrdengine_journalfile *journalfile, struct rrdengine_datafile *datafile)
{
    ...
    netdata_log_debug("DBENGINE: checking integrity of '%s'", path_v2);
    ...
    netdata_log_debug("DBENGINE: journal v2 '%s' loaded, size: %0.2f MiB, metrics: %0.2f k, "
                     ...
                     struct rrdengine_datafile *datafile)
{
    ...
    netdata_log_debug("DBENGINE: loading journal file '%s'", path);
    ...
    netdata_log_debug("DBENGINE: journal file '%s' loaded (size:%"PRIu64").", path, file_size);
