    ...
    while(UV_EOF != uv_fs_scandir_next(&req, &dent)) {
        if (dent.type == UV_DIRENT_DIR) {
            if (!uuid_parse(dent.name, uuid)) {
                uv_fs_t stat_req;
                snprintfz(dbengine_file, FILENAME_MAX, "%s/%s/dbengine", dbfiles_path, dent.name);
...
void compute_multidb_diskspace(char *varlib_dir, char *cache_dir)
{
    char multidb_disk_space_file[FILENAME_MAX + 1];
    FILE *fp;
    computed_multidb_disk_quota_mb = -1;

    snprintfz(multidb_disk_space_file, FILENAME_MAX, "%s/dbengine_multihost_size", varlib_dir);
    ...
    if (likely(fp)) {
        ...
        if (rc == 1 && computed_multidb_disk_quota_mb >= RRDENG_MIN_DISK_SPACE_MB)
            info("Setting computed default multidb storage size to %dMB", computed_multidb_disk_quota_mb);
        else {
            errno = 0;
    ...
    if (computed_multidb_disk_quota_mb == -1) {
        int rc = count_legacy_children(cache_dir);
        if (likely(rc >= 0)) {
...
//                error("Failed to store the default multidb disk quota size on '%s'", multidb_disk_space_file);
            ...
            info("No legacy dbengine folders detected");
    }
