...
int rrdcontext_to_json_v2(BUFFER *wb, struct api_v2_contexts_request *req, CONTEXTS_V2_MODE mode) {
    ...
    if(mode & CONTEXTS_V2_ALERTS) {
        ...
        if(req->options & CONTEXT_V2_OPTION_ALERTS_INSTANCES) {
            buffer_json_member_add_array(wb, "alert_instances");
            {
                ...
                dfe_start_read(ctl.alerts.alert_instances, t){
                    buffer_json_add_array_item_object(wb);
                    {
                        ...
                        if(req->options & CONTEXT_V2_OPTION_ALERTS_CONFIG) {
                        ...
                        buffer_json_member_add_double(wb, "last_status_change_value", t->last_status_change_value);
                        buffer_json_member_add_time_t(wb, "last_status_change", t->last_status_change);
                        buffer_json_member_add_uint64(wb, "global_id", t->global_id);
                        buffer_json_member_add_uuid(wb, "last_transition_id", &t->last_transition_id);
        ...
        if(req->options & CONTEXT_V2_OPTION_ALERTS_CONFIG) {
            buffer_json_member_add_array(wb, "alert_configurations");
            {
                ...
                dfe_start_read(ctl.alerts.alert_configs, t){
                    ...
                    {
                        buffer_json_member_add_uint64(wb, "aci", t->aci);
                        buffer_json_member_add_string(wb, "config_hash_id", t_dfe.name);
                        buffer_json_member_add_string(wb, "context", string2str(rc->rrdset->context));
                        ...
                        buffer_json_member_add_string(wb, "recipient", rc->recipient ? string2str(rc->recipient) : string2str(rc->rrdset->rrdhost->health.health_default_recipient));
                        ...
                        if (RRDCALC_HAS_DB_LOOKUP(rc)) {
                            ...
                            web_client_api_request_v1_data_options_to_buffer_json_array(wb, "lookup_options", (RRDR_OPTIONS)rc->options);
                        }
                        ...
                        buffer_json_member_add_string(wb, "exec", rc->exec ? rrdcalc_exec(rc) : string2str(localhost->health.health_default_exec));
                        buffer_json_member_add_string(wb, "info", string2str(rc->original_info));
                        ...
                        buffer_json_member_add_string(wb, "source", string2str(rc->source));
                    }
