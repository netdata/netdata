...
void aclk_push_alert_event(struct aclk_sync_host_config *wc)
{
    ...
    buffer_sprintf(sql, "select aa.sequence_id, hl.unique_id, hl.alarm_id, hl.config_hash_id, hl.updated_by_id, hl.when_key, " \
        ...
        " hl.old_status, hl.delay, hl.new_value, hl.old_value, hl.last_repeat, hl.chart_context, hl.event_hash_id, hl.alarm_event_id  " \
    ...
    while (sqlite3_step_monitored(res) == SQLITE_ROW) {
        ...
        alarm_log.event_hash = sqlite3_column_type(res, 27) == SQLITE_NULL ?
                                      strdupz((char *)"") :
...
void health_alarm_entry2proto_nolock(struct alarm_log_entry *alarm_log, ALARM_ENTRY *ae, RRDHOST *host)
{
    ...
    char event_hash_id[UUID_STR_LEN];
    uuid_unparse_lower(ae->event_hash_id, event_hash_id);
    ...
    alarm_log->event_hash = strdupz((char *)event_hash_id);
