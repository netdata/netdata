...
void aclk_send_chart_event(struct aclk_database_worker_config *wc, struct aclk_database_cmd cmd)
{
    ...
    if (unlikely(!wc->chart_updates)) {
        log_access("-- [%s (%s)]: Ignoring chart push event, updates have been turned off for this node.", wc->node_id, wc->host->hostname);
        return;
    }
    ...
    while (loop > 0) {
        ...
        if (likely(first_sequence)) {
            ...
            log_access("<- [%s (%s)]: Sending charts and dimensions update, batch_id %ld, first sequence %ld, last sequence %ld", wc->node_id, wc->host->hostname, wc->batch_id, first_sequence, last_sequence);
    ...
    if (start_sequence_id != wc->chart_sequence_id) {
        ...
        if (!wc->chart_payload_count)
            log_access("-- [%s (%s)]: Sync of charts and dimensions done in %ld seconds.", wc->node_id, wc->host->hostname, now_realtime_sec() - wc->startup_time);
    }
...
int aclk_send_chart_config(struct aclk_database_worker_config *wc, struct aclk_database_cmd cmd)
{
    ...
    if (likely(chart_config.config_hash)) {
        log_access("<- [%s (%s)]: Sending chart config for %s.", wc->node_id, wc->host->hostname, hash_id);
        aclk_chart_config_updated(&chart_config, 1);
        destroy_chart_config_updated(&chart_config);
    }
    else
        log_access("-- [%s (%s)]: Chart config for %s not found.", wc->node_id, wc->host->hostname, hash_id);
...
void aclk_receive_chart_ack(struct aclk_database_worker_config *wc, struct aclk_database_cmd cmd)
{
    ...
    log_access("-> [%s (%s)]: Received ack chart sequence id %ld.", wc->node_id, wc->host->hostname, cmd.param1);
...
void aclk_receive_chart_reset(struct aclk_database_worker_config *wc, struct aclk_database_cmd cmd)
{
    ...
    if (cmd.param1 == 1) {
        db_lock();
        buffer_flush(sql);
        log_access("-> [%s (%s)]: Received chart full resync.", wc->node_id, wc->host->hostname);
        ...
        log_access("-- [%s (%s)]: Restarting chart sync from sequence %"PRIu64, wc->node_id, wc->host->hostname, cmd.param1);
        wc->chart_payload_count = sql_get_pending_count(wc);
...
void aclk_get_chart_config(char **hash_id)
{
    ...
    for (int i = 0; hash_id[i]; ++i) {
        ...
        log_access("-> [%s (%s)]: Request %d for chart config with hash %s received.", wc->node_id, wc->host->hostname, i, hash_id[i]);
...
static void aclk_submit_param_command(char *node_id, enum aclk_database_opcode aclk_command, uint64_t param)
{
    ...
    if (wc)
        ...
        if (aclk_worker_enq_cmd(node_id, &cmd))
            log_access("[%s (%s)]: ACLK synchronization thread is not active.", node_id, host->hostname);
    }
    return;
}
...
void aclk_ack_chart_sequence_id(char *node_id, uint64_t last_sequence_id)
{
    ...
    log_access("[%s (N/A)]: Node reports last sequence id received %"PRIu64, node_id, last_sequence_id);
...
void aclk_start_streaming(char *node_id, uint64_t sequence_id, time_t created_at, uint64_t batch_id)
{
    ...
    log_access("-> [%s (N/A)]: Start streaming charts from sequence %"PRIu64" t=%ld, batch=%"PRIu64, node_id,
        ...
        if (host->node_id && !(uuid_compare(*host->node_id, node_uuid))) {
            ...
            if (likely(wc)) {
                ...
                if (sequence_id > wc->chart_sequence_id || wc->chart_reset_count > 10) {
                    log_access("-- [%s (%s)]: Requesting full resync from the cloud "
                    ...
                    if (sequence_id < wc->chart_sequence_id || !sequence_id) { // || created_at != wc->chart_timestamp) {
                        log_access("-- [%s (%s)]: Reset streaming charts from sequence %"PRIu64 \
                        ...
                        log_access("-- [%s (%s)]: Start streaming charts enabled -- last streamed sequence %"PRIu64 \
                                   " t=%ld (reset count=%d)", wc->node_id, wc->host->hostname, wc->chart_sequence_id,
                             wc->chart_timestamp, wc->chart_reset_count);
                ...
                log_access("-- [%s (%s)]: ACLK synchronization thread is not active.", wc->node_id, wc->host->hostname);
            return;
        }
        host = host->next;
    }
