...
{
    UNUSED(name);
    info("Running health migration %s", name);
    ...
    snprintfz(sql, 1023, " SELECT \"CREATE TABLE IF NOT EXISTS \"||name||\" AS SELECT * FROM meta.\"||name||\";\", \"DROP TABLE IF EXISTS meta.\"||name||\";\"  " \
        "FROM meta.sqlite_schema WHERE name LIKE \"health_log_%%\" AND type = \"table\";");
    rc = sqlite3_prepare_v2(database, sql, -1, &res, 0);
    if (rc != SQLITE_OK) {
        error_report("Failed to prepare statement to create new health_log tables");
        return 1;
    }
    ...
    rc = sqlite3_finalize(res);
    if (unlikely(rc != SQLITE_OK))
        error_report("Failed to finalize statement when migrating health_log tables, rc = %d", rc);
...
DATABASE_FUNC_MIGRATION_LIST health_migration_action[] = {
    {.name = "v0 to v1",  .func = do_migration_noop},
    {.name = "v1 to v2",  .func = do_health_migration_v1_v2},
    // the terminator of this array
    {.name = NULL, .func = NULL}
};
