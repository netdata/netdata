...
class Agent:
    def __init__(self, executable, uid, web_port, replication, rm):
...
@click.option('--replication', required=True, type=bool)
@click.option('--rm', required=True, type=bool)
def main(executable, childs, levels, active_active, replication, rm):
    child_agents = [
        Agent(executable, uid, 20000 + uid, replication, rm) for uid in range(childs)
    ]

    level_agents = [
        [
            Agent(executable, 100 * (level + 1), 20000 + 100 * (level + 1), replication, rm),
            Agent(executable, 100 * (level + 1) + 1, 20000 + 100 * (level + 1) + 1, replication, rm)
